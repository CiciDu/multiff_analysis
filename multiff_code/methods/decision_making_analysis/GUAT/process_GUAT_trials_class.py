...
class ProcessGUATtrials:
    ...
    def __init__(self, give_up_after_trying_info_bundle, PlotTrials_args, max_distance_to_stop_for_GUAT_target=50, max_allowed_time_since_last_vis=2.5) -> None:
        ...
        self.GUAT_cluster_index = np.unique(
            self.GUAT_indices_df['cluster_index'].values)
    ...
    def find_GUAT_ff_aimed_at_from_manual_anno(self, manual_anno):
        ...
        # actually this should be unnecessary, since cluster should go from 0 to n continously
        unique_stopping_clusters = np.unique(
            self.GUAT_indices_df['cluster_index'].values)
        # for each cluster
        ...
        self.GUAT_ff_aimed_at_from_manual_anno = np.array(
            GUAT_ff_aimed_at_from_manual_anno, dtype=object)
    ...
    def make_GUAT_plot(self, trial, ff_near_stops, relevant_indices, additional_kwargs=None):
        ...
        duration = [self.ff_caught_T_new[trial-num_trials],
                    self.ff_caught_T_new[trial]]
        ...
        rotation_matrix = returned_info['rotation_matrix']
        ...
        if rotation_matrix is not None:
            ...
            temp_cum_mx, temp_cum_my = np.array(self.monkey_information['monkey_x'].loc[relevant_indices]), np.array(
                self.monkey_information['monkey_y'].loc[relevant_indices])
            if len(temp_cum_mx) > 0:
                ...
                for i in range(len(temp_cum_mx)):
                    circle = plt.Circle((temp_cum_mxy_rotated[0, i], temp_cum_mxy_rotated[1, i]),
                                        self.max_distance_to_stop_for_GUAT_target, facecolor='yellow', edgecolor='red', alpha=0.01, zorder=1)
    ...
    def make_GUAT_cluster_df(self):
        ...
        self.GUAT_cluster_df = cluster_analysis.find_ff_cluster_last_vis_df(
            ff_indices_of_each_cluster, GUAT_last_stop_time, ff_dataframe=self.ff_dataframe, cluster_identifiers=self.GUAT_w_ff_df['cluster_index'].values)

        self.GUAT_cluster_df.rename(
            columns={'cluster_identifier': 'cluster_index'}, inplace=True)
    ...
    def check_GUAT_object_with_manual_anno(self, verbose=True):
        ...
        for i in range(len(self.GUAT_expanded_trials_df.cluster_index.values)):
            ...
            relevant_info = np.where(
                self.GUAT_indices_df['cluster_index'].values == counter)[0]
            relevant_indices = self.GUAT_indices_df['point_index'].values[relevant_info]

            trial = self.GUAT_expanded_trials_df.trial.values[i]
                    ...
                    if len(ff_aimed_at) > 0:
                        for ff in ff_aimed_at:
                            if ff in ff_near_stops:
                                if verbose:
                                    print(
                                        "Trial %d: Matched! Aimed at ff %d, which is near stops!" % (trial, ff))
                                flag = True
                                clusters_w_matching_ff_near_stops.append(
                                    counter)
                    if not flag:
                        if verbose:
                            print("Trial %d: no matching ff near stops!" %
                                  (trial))
        ...
        self.clusters_w_matching_ff_near_stops = np.array(
            clusters_w_matching_ff_near_stops)
        self.clusters_w_o_matching_ff_near_stops = np.array(
            clusters_w_o_matching_ff_near_stops)
    ...
    def inspect_clusters_w_o_matching_ff(self, verbose=True, make_plots=True, max_plots=5):
        ...
        for cluster in self.clusters_w_o_matching_ff_near_stops:
            ...
            cluster_index = np.where(
                self.GUAT_expanded_trials_df.cluster_index.values == cluster)[0][0]
            relevant_info = np.where(
                self.GUAT_indices_df['cluster_index'].values == cluster)[0]
            relevant_indices = self.GUAT_indices_df['point_index'].values[relevant_info]
            ...
            ff_aimed_at = np.array(
                self.GUAT_ff_aimed_at_from_manual_anno[cluster_index])
            if verbose:
            ...
            if make_plots:
                ...
                self.make_GUAT_plot(
                    trial, ff_near_stops, relevant_indices, additional_kwargs=additional_kwargs)
