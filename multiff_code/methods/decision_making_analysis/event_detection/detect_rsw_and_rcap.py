...
def make_rcap_events_df(stop_category_df):
    new_rcap_df = stop_category_df.loc[stop_category_df['attempt_type'] == 'rcap', [
    ...
    rcap_events_df['eventual_outcome'] = 'capture'
    return rcap_events_df


def make_rsw_events_df(stop_category_df, ff_real_position_sorted, monkey_information):

    new_rsw_df = stop_category_df.loc[stop_category_df['attempt_type'] == 'rsw', [
        'point_index', 'stop_id_duration', 'stop_cluster_id', 'stop_cluster_size', 'trial', 'time', 'candidate_target']].copy()
    ...
    rsw_w_ff_df = rsw_events_df.merge(new_rsw_df[[
        'stop_cluster_id', 'ff_index']].drop_duplicates(), on='stop_cluster_id', how='left')
    rsw_w_ff_df['ff_index'] = rsw_w_ff_df['ff_index'].astype(int)

    rsw_w_ff_df = rsw_vs_rcap_utils.add_cur_ff_info(
        rsw_w_ff_df, ff_real_position_sorted)

    rsw_w_ff_df = rsw_vs_rcap_utils.add_stop_point_index(
        rsw_w_ff_df, monkey_information, ff_real_position_sorted)
    ...
    rsw_events_df['event_type'] = 'rsw'
    rsw_events_df['eventual_outcome'] = 'switch'

    rsw_w_ff_df['event_type'] = 'rsw'
    rsw_w_ff_df['eventual_outcome'] = 'switch'
    return rsw_events_df, rsw_w_ff_df


def make_temp_rcap_events_df(monkey_information, ff_caught_T_new, ff_real_position_sorted, max_cluster_distance=50):

    # Extract a subset of monkey information that is relevant for rsw analysis
...
def _take_out_monkey_subset_for_rcap(monkey_information, ff_caught_T_new, ff_real_position_sorted, max_cluster_distance=50):
    ...
    monkey_sub = _take_out_monkey_subset_for_rsw_or_rcap(
...
def _take_out_monkey_subset_for_rsw_or_rcap(monkey_information, ff_caught_T_new, ff_real_position_sorted,
                                            min_stop_per_cluster=2):
    """
    Extract a subset of monkey stop events for rsw/rcap analysis.
...
def combine_rsw_and_one_stop_to_make_miss_to_switch_df(rsw_w_ff_df, one_stop_w_ff_df):
    ...
    cols_to_keep = [
        col for col in one_stop_w_ff_df.columns if col in rsw_w_ff_df.columns]
    one_stop_w_ff_df = one_stop_w_ff_df[cols_to_keep].copy()

    rsw_w_ff_df['event_type'] = 'rsw'
    one_stop_w_ff_df['event_type'] = 'dsw'

    miss_to_switch_df = pd.concat(
        [rsw_w_ff_df, one_stop_w_ff_df], axis=0
    ).reset_index(drop=True)

    # Cast index-like columns to nullable Int64
    for col in miss_to_switch_df.columns:
        if col.endswith('_index'):
            try:
                miss_to_switch_df[col] = miss_to_switch_df[col].astype('Int64')
            except Exception:
                pass
