...
def add_cur_ff_info(rsw_w_ff_df, ff_real_position_sorted):
    if 'candidate_target' not in rsw_w_ff_df.columns:
        rsw_w_ff_df['candidate_target'] = rsw_w_ff_df['ff_index'].astype(int)
    rsw_w_ff_df['cur_ff_index'] = rsw_w_ff_df['candidate_target']
    rsw_w_ff_df[['cur_ff_x', 'cur_ff_y']
                ] = ff_real_position_sorted[rsw_w_ff_df['candidate_target']]
    return rsw_w_ff_df
...
def deal_with_duplicated_stop_point_index(
    ...
    missing = required - set(rsw_w_ff_df.columns)
    if missing:
...
def process_events_df(events_df, monkey_information, ff_dataframe, ff_real_position_sorted, stop_period_duration):
    ...
    ff_info = ff_data_utils.find_many_ff_info_anew(
...
def combine_relevant_features(x_features_df, only_cur_ff_df, plan_features_df, drop_columns_w_na=False):
    ...
    if drop_columns_w_na:
        ...
        print(
            f'When preparing x_var to predict whether rcap or rsw, there are {len(columns_with_na)} columns with NA that are dropped. {x_df.shape[1]} columns are left.')
        print('Columns with NA that are dropped:', np.array(columns_with_na))

    else:
        print(
            f'When preparing x_var to predict whether rcap or rsw, there are {len(columns_with_na)} out of {x_df.shape[1]} columns with NA.')
...
def extract_binary_event_label(df, point_index_array, event_col='event_type',
    ...
    # Check for non-unique mapping (same point_index → multiple event_types)
    conflicts = (
        df_sub.groupby(point_col)[event_col]
        .nunique()
        .loc[lambda x: x > 1]
    )
    ...
    if not conflicts.empty:
        raise ValueError(
            f'Found point_index values with multiple event_types: {conflicts.to_dict()}'
        )
    ...
    y_aligned = df_unique.reindex(point_index_array)['whether_rcap']

    # Return tidy DataFrame
    return pd.DataFrame({'whether_rcap': y_aligned.values})
