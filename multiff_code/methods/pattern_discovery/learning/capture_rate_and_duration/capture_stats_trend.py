...
def plot_poisson_rate_fit(
    df_sessions: pd.DataFrame,
    po,  # either a single fitted model OR a dict {monkey_name: fitted_model}
    *,
    session_col: str = 'session',
    count_col: str = 'captures',
    exposure_col: str = 'total_duration',
    ...
    """
    If df_sessions has a `monkey` column and `po` is a dict keyed by monkey,
    this will draw side-by-side panels for the requested `monkeys`.
    Otherwise, it behaves like the original single-plot version.

    `po` models are expected to include a 'session' coefficient for p-values.
    """
    ...
    def _fit_curve(_po, _df: pd.DataFrame):
        ...
        # p-value for session slope; default to 1.0 if not present
        p_value = getattr(_po, 'pvalues', {}).get('session', 1.0) if hasattr(_po, 'pvalues') else 1.0
        return fit_df, p_value

    def _plot_on_ax(ax, obs: pd.DataFrame, fit_df: pd.DataFrame, panel_title: str, p_value: float):
        ax.errorbar(
            obs[session_col],
            obs['rate'],
        ...
        ax.set_title(panel_title, fontsize=14, y=1.02)

    # --- Single-monkey mode (original behavior) --------------------------------
    if monkey_col not in df_sessions.columns or not isinstance(po, dict):
        ...
        ax = plt.gca()
        _plot_on_ax(ax, obs, fit_df, f'{title_prefix}{title}' if title_prefix else title, p_value)
        # y-lims
    ...
    for m in monkeys:
        ...
        panels.append((m, obs_m, fit_m, p_m))
    ...
    global_min = np.inf
    global_max = -np.inf
    for _, obs_m, fit_m, _ in panels:
        global_min = min(global_min, obs_m['p_lo'].min(), fit_m['p_lo'].min())
        global_max = max(global_max, obs_m['p_hi'].max(), fit_m['p_hi'].max())

    # Font & marker sizes (larger but balanced)
    marker_size = 6
    font_tick = 12
    font_axis = 13
    font_title = 15
    ...
    for i, (ax, (m, obs_m, fit_m, p_m)) in enumerate(zip(axes, panels)):
        ...
        ax.legend(fontsize=font_legend)  # per-panel legend

        analyze_proportion_trend.add_pval_to_plot(ax, p_m)

        ax.set_title(panel_title, fontsize=font_title, y=1.02)
        ax.tick_params(axis='both', labelsize=font_tick)
        ax.set_ylim(global_min * 0.9, global_max * 1.32)
        ax.yaxis.set_tick_params(labelleft=True)

    plt.tight_layout()
    # Increase horizontal space between the two subplots
