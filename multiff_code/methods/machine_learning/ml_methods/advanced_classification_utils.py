...
def _proba_or_score(mdl, X, n_classes: int):
    ...
    if hasattr(final_est, "predict_proba"):
        ...
        if n_classes == 2:
            if proba.ndim == 2 and proba.shape[1] == 2: return proba[:, 1]
            if proba.ndim == 1: return proba
            return None
        else:
            if proba.ndim == 2 and proba.shape[1] == n_classes: return proba
            return None
    ...
    if hasattr(final_est, "decision_function"):
        ...
        if n_classes == 2:
            if dec.ndim == 1: return dec
            if dec.ndim == 2 and dec.shape[1] == 1: return dec.ravel()
            if dec.ndim == 2 and dec.shape[1] == 2: return dec[:, 1]
            return None
        else:
            if dec.ndim == 2 and dec.shape[1] == n_classes: return dec
            return None
    return None
...
def _param_spaces(name, n_classes):
    ...
    Lists work with RandomizedSearchCV.
    """
    spaces = {
        "logreg": {
...
def use_advanced_model_for_classification(
    ...
    if kfold_cv:
        ...
        # sensible default for ~3k samples
    ...
    for name, base_mdl in all_models:
        try:
            if checkpoint_dir and resume and name in state["completed"]:
                ...
                # Collect saved metrics back into metrics_list for final table
            ...
            if tune:
                ...
                if len(space) > 0:
                    search = RandomizedSearchCV(
                        ...
                        n_jobs=n_jobs,
                        random_state=random_state,
                        refit=True,            # keep the best model fitted on full train
                        verbose=0
                    )
                    ...
                    # No space defined -> just fit
                    mdl.fit(X_train, y_train_enc)
            else:
                # No tuning -> straight fit
                mdl.fit(X_train, y_train_enc)
    ...
    if checkpoint_dir and os.path.isfile(os.path.join(checkpoint_dir, f"{best_name}.joblib")):
        ...
        if tune and len(_param_spaces(best_name, n_classes)) > 0:
            # quick refit without re-search (uses defaults); acceptable fallback
