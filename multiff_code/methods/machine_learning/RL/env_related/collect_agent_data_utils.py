...
def collect_agent_data_func(env, sac_model, n_steps=15000, LSTM=False, hidden_dim=128, deterministic=True, first_obs=None):
    ...
    obs_ff_indices_in_ff_dataframe = obs_ff_indices_in_ff_dataframe.merge(pd.DataFrame(
        pd.Series(range(n_steps), name='point_index')), on='point_index', how='right')
    # make obs_ff_indices_in_ff_dataframe a list
    obs_ff_indices_in_ff_dataframe = obs_ff_indices_in_ff_dataframe['ff_index'].tolist(
    )
    obs_ff_indices_in_ff_dataframe = [np.array(x) if isinstance(
        x, list) else np.array([]) for x in obs_ff_indices_in_ff_dataframe]
...
def find_corresponding_info_of_agent(info_of_monkey, currentTrial, num_trials, sac_model, agent_dt, LSTM=False, env_kwargs=None):
    ...
    # Make ff_dataframe
    ff_dataframe_args = (monkey_information, ff_caught_T_new,
    ...
    if len(ff_dataframe) > 0:
        ...
        cluster_around_target_trials, used_cluster_trials, cluster_around_target_indices, cluster_around_target_positions = pattern_by_trials.cluster_around_target_func(
