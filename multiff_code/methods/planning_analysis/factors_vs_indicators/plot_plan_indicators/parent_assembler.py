...
def _axes_domains(fig):
    ...
    if not xmap: xmap['1'] = (0.0, 1.0)
    if not ymap: ymap['1'] = (0.0, 1.0)
    return xmap, ymap
...
def make_parent_figure_from_children(main_fig, diff_fig,
    ...
    if interleave_if_grid_match and grids_match:
        ...
        row_heights = sum(([top_share / max(m_rows, 1), bot_share / max(m_rows, 1)] for _ in range(m_rows)), [])
...
def _adaptive_vspace(n_rows):
    ...
    if n_rows <= 2:   return 0.10
    if n_rows <= 4:   return 0.06
    if n_rows <= 6:   return 0.05
    return 0.04
...
class ParentFigureAssembler:
    ...
    def assemble(self, main_fig, diff_fig, *,
                 main_y_title, diff_y_title, overall_title):
        ...
        if grids_match:
            ...
            # 1) Put y-titles ONLY on the first pair to reduce clutter
            fig.update_yaxes(title_text=main_y_title, row=1, col=1)  # first "main" row
            fig.update_yaxes(title_text=diff_y_title, row=2, col=1)  # first "diff" row
            ...
            for c in range(1, parent_cols + 1):
                fig.update_xaxes(title_text=self.x_title, row=parent_rows, col=c)

            # 3) Zero reference line on every even row (all diff rows)
            for rr in range(2, parent_rows + 1, 2):
                fig.add_hline(y=0, line_dash='dash', line_color='gray', row=rr, col=1)
        ...
        for r in range(1, parent_rows + 1):
            for c in range(1, parent_cols + 1):
                fig.update_xaxes(automargin=True, title_standoff=8, row=r, col=c)
                fig.update_yaxes(automargin=True, title_standoff=8, row=r, col=c)
