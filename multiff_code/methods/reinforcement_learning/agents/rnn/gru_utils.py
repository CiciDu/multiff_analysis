...
class SAC_PolicyNetworkGRU(lstm_utils.PolicyNetworkBase):
    ...
    def get_action(self, state, last_action, hidden_in, deterministic=True):
        dev = self._dev()
        state = torch.FloatTensor(state).unsqueeze(0).unsqueeze(
            0).to(dev)  # increase 2 dims to match with training data
        last_action = torch.FloatTensor(
            last_action).unsqueeze(0).unsqueeze(0).to(dev)
        mean, log_std, hidden_out = self.forward(state, last_action, hidden_in)
        mean = torch.nan_to_num(mean, nan=0.0, posinf=10.0, neginf=-10.0)
...
class ReplayBufferGRU:
    ...
    def sample(self, batch_size,
        ...
        core_T = min(T_full_list) if seq_len is None else int(
            min(seq_len, min(T_full_list)))
        max_prefix_allowed = min(
            [T - core_T for T in T_full_list]) if core_T > 0 else 0
        ...
        for h_in, h_out, state, action, last_action, reward, next_state, done in batch:
            ...
            t0_core = random.randint(low, high) if (
                random_window and high > low) else low
            t0_b = t0_core - prefix
            t1 = t0_core + core_T
            ...
            if policy_net is not None and burn_in > 0 and t0_b > 0:
                with torch.no_grad():
                    ...
                    for t in range(t0_b):
                        s_t = torch.as_tensor(
                            state[t], dtype=torch.float32, device=h.device).unsqueeze(0).unsqueeze(0)
                        la_t = torch.as_tensor(
                            last_action[t], dtype=torch.float32, device=h.device).unsqueeze(0).unsqueeze(0)
        ...
        state = torch.as_tensor(
            np.array(s_lst), dtype=torch.float32, device=dev)
        action = torch.as_tensor(
            np.array(a_lst), dtype=torch.float32, device=dev)
        last_action = torch.as_tensor(
            np.array(la_lst), dtype=torch.float32, device=dev)
        reward = torch.as_tensor(
            np.array(r_lst), dtype=torch.float32, device=dev)
        next_state = torch.as_tensor(
            np.array(ns_lst), dtype=torch.float32, device=dev)
        done = torch.as_tensor(
            np.array(d_lst), dtype=torch.float32, device=dev)
...
class GRU_SAC_Trainer():
    def __init__(self, **kwargs):
        ...
        self.device = kwargs.get('device', "cuda" if torch.cuda.is_available() else (
            "mps" if torch.backends.mps.is_available() else "cpu"),)
        ...
        self.soft_q_net1 = QNetworkGRU(
            state_space, action_space, self.hidden_dim).to(self.device)
        self.soft_q_net2 = QNetworkGRU(
            state_space, action_space, self.hidden_dim).to(self.device)
        self.target_soft_q_net1 = QNetworkGRU(
            state_space, action_space, self.hidden_dim).to(self.device)
        self.target_soft_q_net2 = QNetworkGRU(
            state_space, action_space, self.hidden_dim).to(self.device)
        self.policy_net = SAC_PolicyNetworkGRU(
    ...
    def update(self, batch_size, reward_scale=1, auto_entropy=True, target_entropy=-2, gamma=0.975, soft_tau=1e-2):
        ...
        next_state = torch.nan_to_num(
            next_state, nan=0.0, posinf=0.0, neginf=0.0)
        action = torch.nan_to_num(
            action, nan=0.0, posinf=0.0, neginf=0.0).clamp_(-1.0, 1.0)
        last_action = torch.nan_to_num(
            last_action, nan=0.0, posinf=0.0, neginf=0.0).clamp_(-1.0, 1.0)
        reward = torch.nan_to_num(reward, nan=0.0, posinf=0.0, neginf=0.0)
        done = torch.nan_to_num(done, nan=1.0, posinf=1.0, neginf=1.0)

        # Sanitize hidden states
        hidden_in = torch.nan_to_num(
            hidden_in, nan=0.0, posinf=0.0, neginf=0.0)
        hidden_out = torch.nan_to_num(
            hidden_out, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        predicted_q_value2, _ = self.soft_q_net2(
            state, action, last_action, hidden_in)
        new_action, log_prob, z, mean, log_std, _ = self.policy_net.evaluate(
    ...
    # Updating alpha wrt entropy
        ...
        if auto_entropy is True:
            alpha_loss = - \
                (self.log_alpha *
                 (log_prob[:, tr, :] + target_entropy).detach()).mean()
    ...
    # Training Q Function
        ...
        grad_norm_q1 = torch.nn.utils.clip_grad_norm_(
            self.soft_q_net1.parameters(), max_norm=5.0)
        self.soft_q_optimizer1.step()
        self.soft_q_optimizer2.zero_grad()
        q_value_loss2.backward()
        grad_norm_q2 = torch.nn.utils.clip_grad_norm_(
            self.soft_q_net2.parameters(), max_norm=5.0)
    ...
    # Training Policy Function
        ...
        policy_loss = (
            self.alpha * log_prob[:, tr, :] - predicted_new_q_value[:, tr, :]).mean()
    ...
    # Soft update the target value net
        ...
        with torch.no_grad():
            ...
            alpha_val = float(self.alpha.detach().item()) if torch.is_tensor(
                self.alpha) else float(self.alpha)

        # Prepare scalar grad norms
        grad_norm_policy_value = float(grad_norm_policy.item()) if torch.is_tensor(
            grad_norm_policy) else float(grad_norm_policy)
        grad_norm_q1_value = float(grad_norm_q1.item()) if torch.is_tensor(
            grad_norm_q1) else float(grad_norm_q1)
        grad_norm_q2_value = float(grad_norm_q2.item()) if torch.is_tensor(
            grad_norm_q2) else float(grad_norm_q2)
    ...
    def load_model(self, path):
        ...
        self.soft_q_net1.load_state_dict(
            torch.load(path+'/gru_q1', map_location=device))
        self.soft_q_net2.load_state_dict(
            torch.load(path+'/gru_q2', map_location=device))
        self.policy_net.load_state_dict(torch.load(
            path+'/gru_policy', map_location=device))
...
def _train_gru_episode(env, rl_agent, max_steps_per_eps):
    state, _ = env.reset()
    ...
    dev = next(rl_agent.policy_net.parameters()).device
    hidden_out = _initialize_gru_hidden(rl_agent.hidden_dim, dev)
    ...
    for step in range(max_steps_per_eps):
        ...
        if (not torch.isfinite(hidden_in).all()):
            hidden_in = _initialize_gru_hidden(rl_agent.hidden_dim, dev)
        ...
        current_as = getattr(rl_agent.policy_net, 'anneal_step', 0)
        setattr(rl_agent.policy_net, 'anneal_step', int(current_as) + 1)
        ...
        if step % rl_agent.train_freq == 0 and len(rl_agent.replay_buffer) > rl_agent.batch_size:
            for _ in range(rl_agent.update_itr):
                rl_agent.update(batch_size=rl_agent.batch_size,
                                reward_scale=rl_agent.reward_scale,
                                auto_entropy=True,
                                target_entropy=rl_agent.target_entropy,
                                gamma=rl_agent.gamma,
                                soft_tau=rl_agent.soft_tau)

        if done:
            break

    rl_agent.replay_buffer.push(ini_hidden_in, ini_hidden_out, episode_data['state'], episode_data['action'],
                                episode_data['last_action'], episode_data['reward'], episode_data['next_state'], episode_data['done'])
    return np.sum(episode_data['reward'])


def evaluate_gru_agent(env, rl_agent, max_steps_per_eps, num_eval_episodes, deterministic=True):
    was_training = rl_agent.policy_net.training
    ...
    with torch.no_grad():
        for _ in range(num_eval_episodes):
            ...
            for step in range(max_steps_per_eps):
                hidden_in = hidden_out
                if not np.isfinite(state).all():
                    lstm_utils._maybe_warn_nans(
                        True, 'eval.state', epi_ctx, numerics_cfg)
                    state = np.nan_to_num(
                        state, nan=0.0, posinf=0.0, neginf=0.0)

                if (not torch.isfinite(hidden_in).all()):
                    lstm_utils._maybe_warn_nans(
                        True, 'eval.hidden_in', epi_ctx, numerics_cfg)
                    hidden_in = _initialize_gru_hidden(
                        rl_agent.hidden_dim, dev)
