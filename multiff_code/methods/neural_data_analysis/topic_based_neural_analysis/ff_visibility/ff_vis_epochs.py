...
def compute_visibility_runs_and_clusters(
    ...
    *,
    ...
    # merge chunks into clusters if gap <= this
    cluster_merge_gap: Optional[float] = None,
    # 'gap' = start - prev_end; 'start' = start - prev_start
    merge_mode: str = 'gap',
    verbose: bool = True,
    # skip sort if df is already [ff, time, t] sorted
    assume_sorted: bool = False,
    # cast IDs to pandas Int64 at the end (slower)
    nullable_int: bool = False
) -> pd.DataFrame:
    # 0) order & dtypes
    out = df if assume_sorted else df.sort_values(
        [ff_col, time_col, t_col], kind='quicksort')
    out = out.copy()
    ...
    out['raw_is_start'] = (out[vis_col] & ~prev_vis).astype(
        np.int64)  # <-- FIX: real column
    raw_run_id = out.groupby(ff_col, sort=False)[
        'raw_is_start'].cumsum().to_numpy()
    # sentinel for invisible rows
    raw_run_id = np.where(vis_mask, raw_run_id, -1)
    ...
    raw_tbl['vis_chunk_id'] = new_chunk.astype(
        np.int64).groupby(raw_tbl[ff_col], sort=False).cumsum()
    ...
    chunks['ff_vis_duration'] = chunks['ff_vis_end_time'] - \
        chunks['ff_vis_start_time']
    chunks['ff_prev_vis_start_time'] = chunks.groupby(
        ff_col, sort=False)['ff_vis_start_time'].shift()
    chunks['ff_prev_vis_end_time'] = chunks.groupby(
        ff_col, sort=False)['ff_vis_end_time'].shift()
    chunks['ff_next_vis_start_time'] = chunks.groupby(
        ff_col, sort=False)['ff_vis_start_time'].shift(-1)

    # -------------------------
    # 3) Merge CHUNKS → visible CLUSTERS (gap <= cluster_merge_gap)
    # -------------------------
    if cluster_merge_gap is None:
        chunks['vis_cluster_id'] = (chunks.groupby(
            ff_col, sort=False).cumcount() + 1).astype(np.int64)
        chunks['vis_cluster_idx'] = 1
    else:
        if merge_mode == 'gap':
            prev_c_end = chunks.groupby(ff_col, sort=False)[
                'ff_vis_end_time'].shift()
            sep_c = chunks['ff_vis_start_time'] - prev_c_end
        else:  # 'start'
            prev_c_start = chunks.groupby(ff_col, sort=False)[
                'ff_vis_start_time'].shift()
            sep_c = chunks['ff_vis_start_time'] - prev_c_start

        new_cluster = sep_c.isna() | (sep_c > float(cluster_merge_gap))
        chunks['vis_cluster_id'] = new_cluster.astype(
            np.int64).groupby(chunks[ff_col], sort=False).cumsum()
        chunks['vis_cluster_idx'] = chunks.groupby(
            [ff_col, 'vis_cluster_id'], sort=False).cumcount() + 1
    ...
    cluster_tbl['vis_cluster_duration'] = cluster_tbl['vis_cluster_end_time'] - \
        cluster_tbl['vis_cluster_start_time']
    cluster_tbl['vis_cluster_prev_start_time'] = cluster_tbl.groupby(
        ff_col, sort=False)['vis_cluster_start_time'].shift()
    cluster_tbl['vis_cluster_prev_end_time'] = cluster_tbl.groupby(
        ff_col, sort=False)['vis_cluster_end_time'].shift()
    cluster_tbl['vis_cluster_next_start_time'] = cluster_tbl.groupby(
        ff_col, sort=False)['vis_cluster_start_time'].shift(-1)

    chunks = chunks.merge(
        cluster_tbl, on=[ff_col, 'vis_cluster_id'], how='left', sort=False)
    ...
    out_vis = out_vis.merge(
        raw_to_chunk, on=[ff_col, 'raw_run_id'], how='left', sort=False)
    ...
    out.loc[vis_mask, ['vis_chunk_id'] +
            attach_cols] = out_vis[['vis_chunk_id'] + attach_cols].to_numpy()
    ...
    if nullable_int:
        ...
        out['vis_chunk_id'] = out['vis_chunk_id'].fillna(
            -1).astype(np.int64, copy=False)
        out['vis_cluster_id'] = out['vis_cluster_id'].fillna(
            -1).astype(np.int64, copy=False)
        out['vis_cluster_idx'] = out['vis_cluster_idx'].fillna(
            0).astype(np.int64, copy=False)

    if verbose:
        # how many raw runs collapsed into chunks >1
        raw_to_chunk_sizes = raw_tbl.groupby(
            [ff_col, 'vis_chunk_id'], sort=False).size()
        ...
        chunk_to_cluster_sizes = chunks.groupby(
            [ff_col, 'vis_cluster_id'], sort=False).size()
        merged_chunks = int(
            chunk_to_cluster_sizes[chunk_to_cluster_sizes > 1].sum())
        n_chunks = len(chunks)
...
def add_global_visibility_bursts(
    ...
    *,
    ...
    runs = (
        out.loc[vis_mask, [ff_col, 'vis_chunk_id',
                           'ff_vis_start_time', 'ff_vis_end_time']]
    ...
    if runs.empty:
        for c in ('global_burst_start_time', 'global_burst_end_time', 'global_burst_duration',
            ...
            out[c] = np.nan
        out['global_burst_id'] = pd.Series(
            [pd.NA]*len(out), dtype='Int64') if nullable_int else -1
        return out
    ...
    bursts['global_burst_duration'] = bursts['global_burst_end_time'] - \
        bursts['global_burst_start_time']
    ...
    bursts['global_burst_next_start_time'] = bursts['global_burst_start_time'].shift(
        -1)
    ...
    if nullable_int:
        ...
        out['global_burst_id'] = out['global_burst_id'].fillna(
            -1).astype(np.int64, copy=False)
    ...
    if verbose:
        ...
        print(
            f'[visibility][global] total runs={total_runs}, merged into bursts={runs_in_merged_bursts} ({pct:.1f}%)')
...
def _add_global_index_over(
    ...
    *,
    ...
    # e.g., ['session_id'] to restart numbering per session
    group_cols: Optional[list[str]] = None,
    nullable_int: bool = True,
    # extra sort keys after start/end
    ...
    def _one(g: pd.DataFrame) -> pd.DataFrame:
        ...
        if uniq.empty:
            g[out_col] = pd.Series(
                [pd.NA]*len(g), dtype='Int64') if nullable_int else np.nan
            return g
        ...
        uniq = uniq.sort_values(
            sort_keys, kind='quicksort').reset_index(drop=True)
    ...
    if group_cols:
        out = out.groupby(group_cols, group_keys=False, sort=False).apply(
            _one).reset_index(drop=True)
    else:
        out = _one(out)
