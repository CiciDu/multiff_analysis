...
Same interface as `stop_design_for_decoding.build_stop_design`, but implemented
...
# One_ff_gam tuning covariates (assemble_one_ff_gam_design finalize_one_ff_gam_design)
# Only columns present in monkey_information are aggregated and added.
ONE_FF_STYLE_EXTRA_COLS = [
    'v', 'w', 'd', 'phi',
    'r_targ', 'theta_targ',
    'eye_ver', 'eye_hor',
    'accel', 'ang_accel_deg', # though they are not in one_ff_gam, we add them to the design
]
...
DEFAULT_TUNING_VARS_NO_WRAP = [
    ...
    'accel', 'ang_accel_deg',  # though these are not in one_ff_gam, we add them to the design
]
...
def monkey_information_for_encoding(
    ...
    def _derive_eye_channel(
        ...
        if has_valid and (has_left or has_right):
            ...
            print('Warning: columns valid_view_point_l or valid_view_point_r does not exist')
    ...
    if 'ang_accel' in out.columns:
        out['ang_accel_deg'] = out['ang_accel'].to_numpy(float, copy=False) * _RAD_TO_DEG
...
def _add_retries_info_to_binned_feats(
    ...
    event_tbl = stop_design.build_per_event_table(new_seg_info, extras=RETRY_CATEGORIES)
    ...
    binned_feats['whether_in_retry_series'] = binned_feats[retry_cols].any(axis=1).astype('int8')
...
def _build_stop_design_base_for_encoding(
    new_seg_info: pd.DataFrame,
    ...
    *,
    ...
    binned_spikes = pd.DataFrame(spike_counts[pos, :], columns=cluster_ids).reset_index(drop=True)
    ...
    if not for_decoding:
        cluster_feats = cluster_feats + ['cluster_progress_c2', 'event_t_from_cluster_start_s']
    ...
    # groups are rebuilt later in build_stop_design_for_encoding
...
def build_temporal_design_base_stop(
    ...
    Mirror of assemble_one_ff_gam_design.build_temporal_design_base for stop-aligned data:
...
def build_temporal_design_summed_stops(
    ...
    *all* stops within the kernel window. Column names are ``{name_prefix}_0``, ...
    ...
    if use_global:
        ...
        n_grid = max(1, int(np.floor((t_grid_end - t_grid_start) / bin_dt)) + 1)
    else:
    ...
    if use_global:
        ...
        if np.any(bin_idx < 0) or np.any(bin_idx >= n_grid):
            raise ValueError('bin_t_center outside convolution grid (check t_min/t_max/bin_dt)')
        X = X_grid[bin_idx, :]
...
def _build_feature_groups_for_encoding(
    ...
    """
    Build semantic feature groups (same style as stop_design_for_decoding._build_feature_groups).
    ...
    _add('cluster_gaps', ['prev_gap_s_z', 'next_gap_s_z'])
    ...
    _add('cluster_event_t', ['event_t_from_cluster_start_s'])
...
def build_stop_design_for_encoding(
    ...
    Same interface and return types as stop_design_for_decoding.build_stop_design.
    ...
    bin_dt : float
        Bin width (s).
    add_ff_visible_info, add_retries_info, datasets, global_bins_2d, for_decoding
        Passed through to build_stop_design.
    n_basis : int
    ...
    if use_planning_rename or custom_rename:
        monkey_for_encoding = monkey_information_for_encoding(
            monkey_information,
    ...
    if for_decoding:
        kinematic_cols = ['accel', 'ang_accel_deg']
        extra_agg_cols = []
    else:
        kinematic_cols = []
    ...
    binned_spikes, binned_feats, offset_log, meta_used, _groups_unused = (
        _build_stop_design_base_for_encoding(
    ...
    if 't_center' not in meta_used.columns:
        raise ValueError('meta_used missing required column "t_center" for summed-stop temporal design')
    if 'event_id_start_time' not in events_with_stats.columns:
        raise ValueError('events_with_stats missing required column "event_id_start_time" for summed-stop temporal design')
    ...
    if use_global:
        ...
        kwargs['used_bin_indices'] = meta_used['global_bin'].to_numpy(dtype=int)
    ...
    if mode != 'raw_only':
        if not binrange_dict:
            binrange_dict = stop_parameters.estimate_stop_binrange_from_binned_feats(
                binned_feats
            )
        if not binrange_dict:
            raise ValueError(
        ...
        if _linear or _angular:
            X_tuning, tuning_meta = build_tuning_design_stop(
                ...
                wrap_angular=True,  # phi and other wrap vars use [-180, 180) wrapping
            )
            if mode == 'boxcar_only':
                to_drop = [
    ...
    if defer_blocks:
        ...
        return binned_spikes, binned_feats, offset_log, meta_used, groups, deferred
    groups = _build_feature_groups_for_encoding(
        binned_feats,
        kinematic_cols=kinematic_cols,
