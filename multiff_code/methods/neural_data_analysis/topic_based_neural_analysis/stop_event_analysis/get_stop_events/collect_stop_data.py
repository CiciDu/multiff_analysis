...
def prepare_stop_design_inputs(raw_data_folder_path, bin_width=0.04):
    """
    Prepare shared inputs for stop design assembly (encoding or decoding).
    Returns (pn, new_seg_info, events_with_stats, datasets) for use by
    assemble_stop_design_encode / assemble_stop_design_decode.
    """
    from neural_data_analysis.design_kits.design_around_event import event_binning

    pn, datasets, _ = collect_stop_data_func(raw_data_folder_path, bin_width=bin_width)
    ...
    events_with_stats = stops_with_stats[[
        'stop_id', 'stop_cluster_id', 'stop_id_start_time', 'stop_id_end_time',
    ]].rename(columns={
        'stop_id': 'event_id',
    ...
    return pn, new_seg_info, events_with_stats, datasets
