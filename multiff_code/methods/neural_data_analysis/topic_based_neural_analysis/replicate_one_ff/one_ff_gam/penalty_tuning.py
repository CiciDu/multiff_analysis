...
def _load_existing_results(save_path, lam_grid):
    ...
    print(f'⚠ Found partial results: {n_completed}/{n_total} combinations tested ({100*n_completed/n_total:.1f}%)')
    print(f'⚠ Best so far: {saved_data["best_lams"]} (LL={saved_data["best_score"]:.6f})')
...
def _print_progress(lam_setting, score, best_score, is_new_best, 
    ...
    avg_time = np.mean(combo_times[-10:])  # Use last 10 for more recent estimate
    ...
    print(f'  Time:   {combo_time:.1f}s | Avg={avg_time:.1f}s | ETA={eta_str} | Elapsed={elapsed/60:.1f}m')
    print()
...
def tune_penalties(
    ...
    _print_grid_search_setup(lam_grid, group_name_map, n_combinations, n_folds, save_path)
