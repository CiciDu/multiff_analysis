...
from null_behaviors import find_best_arc, curv_of_traj_utils, optimal_arc_utils
...
def make_curvature_df(ff_dataframe_sub, curv_of_traj_df, ff_radius_for_optimal_arc=15, clean=True, 
    ...
    if remove_invalid_rows:
        ...
        if include_curv_to_ff_center:
            ...
            ff_dataframe_sub = ff_dataframe_sub[ff_dataframe_sub.ff_angle_boundary.between(-45*math.pi/180, 45*math.pi/180)]
...
def _make_curvature_df(ff_dataframe_sub, curv_of_traj, ff_radius_for_optimal_arc=15, clean=False, 
                       invalid_curvature_ok=False,
                       include_curv_to_ff_center=True, include_optimal_curvature=True,
                       optimal_arc_stop_at_visible_boundary=True, ignore_error=False):
...
def supply_with_ff_curvature_info(curvature_df, ff_radius_for_optimal_arc=15, invalid_curvature_ok=False,
                                  include_curv_to_ff_center=True, include_optimal_curvature=True,
                                  optimal_arc_stop_at_visible_boundary=True, ignore_error=False):
    if include_curv_to_ff_center:
        curvature_df = _supply_curvature_df_with_arc_to_ff_center_info(curvature_df, invalid_curvature_ok=invalid_curvature_ok)
    if include_optimal_curvature:
        curvature_df = optimal_arc_utils._supply_curvature_df_with_optimal_arc_info(curvature_df, ff_radius_for_optimal_arc, optimal_arc_stop_at_visible_boundary=optimal_arc_stop_at_visible_boundary,
...
def _find_arc_ending_xy_in_world_coord(arc_ff_xy, monkey_xy, monkey_angle, arc_radius, arc_end_direction):
    ...
    center_x, center_y = optimal_arc_utils.find_arc_center_in_world_coord(monkey_xy, monkey_angle, arc_radius, arc_end_direction)
    # the calculation for angle_from_center_to_stop_position is the same whether optimal arc or arc to ff center is used.
    angle_from_center_to_stop_position = optimal_arc_utils.find_angle_from_arc_center_to_ff(arc_ff_xy, center_x, center_y)
    arc_end_x = np.cos(angle_from_center_to_stop_position)*arc_radius + center_x
    arc_end_y = np.sin(angle_from_center_to_stop_position)*arc_radius + center_y
...
def add_d_heading_info(curvature_df, include_curv_to_ff_center=True, include_optimal_curvature=True):
    if include_curv_to_ff_center:
        curvature_df['arc_measure_to_center'], curvature_df['arc_length_to_center'] = find_arc_measure_and_length_to_ff_center(curvature_df['arc_radius_to_ff_center'], curvature_df['ff_distance'], curvature_df['ff_angle'])
...
def find_arc_measure_and_length_to_ff_center(all_arc_radius, all_ff_distance, all_ff_angle):
    ...
    ff_x_relative_to_monkey = np.cos(all_ff_angle+math.pi/2)*all_ff_distance # here we add math.pi/2 because originally ff_angle starts from 0 which is to the north of the monkey
...
def find_cartesian_arc_center_and_angle_for_arc_to_center(monkey_xy, monkey_angle, ff_distance, ff_angle, arc_radius, arc_ff_xy, arc_end_direction, whether_ff_behind=None, ignore_error=False):
    center_x, center_y = optimal_arc_utils.find_arc_center_in_world_coord(monkey_xy, monkey_angle, arc_radius, arc_end_direction)
    angle_from_center_to_monkey, angle_from_center_to_stop_position = optimal_arc_utils.find_angle_from_arc_center_to_monkey_and_stop_position(arc_ff_xy, ff_angle, monkey_xy, center_x, center_y)
    arc_starting_angle, arc_ending_angle = optimal_arc_utils._find_cartesian_arc_starting_and_ending_angle(angle_from_center_to_monkey, angle_from_center_to_stop_position, ff_distance, ff_angle, arc_end_direction, 
...
def _refine_arc_starting_and_ending_angles(arc_starting_angle, arc_ending_angle, arc_end_direction):
    ...
    for i in range(2): #repeat 2 times just to make sure
        ...
        arc_ending_angle[ff_at_right][arc_ending_angle[ff_at_right] > arc_starting_angle[ff_at_right]] -= 2*pi
    ...
    for i in range(2):
        ...
        if len(np.where(delta_angle > 2*pi)[0]) > 0:
            raise ValueError("At least one arc has an angle larger than 2*pi, which shouldn't be. Please check the input.")
        ff_left_too_big_angle = np.where((arc_end_direction >= 0) & (delta_angle > pi))[0]
        ff_right_too_big_angle = np.where((arc_end_direction < 0) & (delta_angle > pi))[0]
        arc_ending_angle[ff_left_too_big_angle] = arc_starting_angle[ff_left_too_big_angle] + (2*pi - delta_angle[ff_left_too_big_angle])
        arc_ending_angle[ff_right_too_big_angle] = arc_starting_angle[ff_right_too_big_angle] - (2*pi - delta_angle[ff_right_too_big_angle])
...
def find_arc_end_position(all_arc_measure, all_arc_radius, arc_end_direction, 
    ...
    arc_end_angles, arc_end_distances, arc_end_x, arc_end_y = find_arc_end_position(\
        curvature_df['optimal_arc_measure'].values, curvature_df['optimal_arc_radius'].values, curvature_df['optimal_arc_end_direction'].values,
    ...
    if np.any(np.abs(all_arc_measure) > math.pi/2):
        raise ValueError("At least one arc has a measure larger than 90 degrees, which is not possible. Please check the input.")
    if np.any(all_arc_radius < 0):
        raise ValueError("At least one arc has a arc radius smaller than 0, which is not possible. Please check the input.")
    ...
    arc_end_angle_from_monkey = pi/2 - np.arctan2(arc_end_y, np.abs(arc_end_x)) # we used pi/2 here since we want 0 to be pointing to the north for the monkey
    # but honestly arc_end_angle_from_monkey should just be half of arc measures

    # for ff to the right
    arc_end_angle_from_monkey[arc_end_direction < 0] = - arc_end_angle_from_monkey[arc_end_direction < 0]
    ...
    if use_world_coordinates:
        ...
        x0 = all_monkey_x
        y0 = all_monkey_y
        arc_end_x_rotated = arc_end_x * monkey_angles_cos + arc_end_y * (-monkey_angles_sin) + x0
        arc_end_y_rotated = arc_end_x * monkey_angles_sin + arc_end_y * monkey_angles_cos + y0
...
def find_polar_arc_center_and_angle(arc_radius, arc_measure, arc_end_direction):
    ...
    arc_starting_angle, arc_ending_angle = _find_polar_arc_starting_and_ending_angles(arc_radius, arc_measure, arc_end_direction)
    return center_x, center_y, arc_starting_angle, arc_ending_angle
...
def _find_polar_arc_starting_and_ending_angles(arc_radius, arc_measure, arc_end_direction):
    ...
    arc_ending_angle[arc_end_direction < 0] = math.pi - arc_measure[arc_end_direction < 0]
    return arc_starting_angle, arc_ending_angle
...
def clean_curvature_info(curvature_df, include_optimal_curvature=True):
    ...
    if include_optimal_curvature:
        ...
        curvature_df['optimal_curvature'] = curvature_df['optimal_curvature'].clip(lower=-0.2, upper=0.2)
...
def fill_up_NAs_in_columns_related_to_curvature(df, monkey_information=None, ff_caught_T_new=None, curv_of_traj_df=None):
    ...
    if 'optimal_curvature' in df.columns:
        ...
        within_ff_na_index = (df['ff_distance'] <= 25) & df['abs_curv_diff'].isna()
    else:
    ...
    for column in ['curvature_lower_bound', 'curvature_upper_bound', 'curv_diff', 'abs_curv_diff']:
        if column in df.columns:
            df.loc[within_ff_na_index, column] = 0 if 'curv' in column else -200 if 'lower' in column else 200
    ...
    if 'curvature_lower_bound' in df.columns: # this should mean that curvature_lower_bound is also in df.columns
        ff_left_na_index = (df['ff_angle_boundary'] > 0) & df['curvature_lower_bound'].isna()
        df.loc[ff_left_na_index, ['curvature_lower_bound', 'curvature_upper_bound']] = np.array([0, 200]) # note: positive number is to the left
        ff_right_na_index = (df['ff_angle_boundary'] < 0) & df['curvature_lower_bound'].isna()
        df.loc[ff_right_na_index, ['curvature_lower_bound', 'curvature_upper_bound']] = np.array([-200, 0])
        middle_ff_na_index = (df['ff_angle_boundary'] == 0) & df['optimal_curvature'].isna()
        df.loc[middle_ff_na_index, ['curvature_lower_bound', 'curvature_upper_bound']] = np.array([-200, 200])

    if 'optimal_curvature' in df.columns:
        ff_left_na_index = (df['ff_angle_boundary'] > 0) & df['optimal_curvature'].isna()
        df.loc[ff_left_na_index, 'optimal_curvature'] = 0.1     
        ff_right_na_index = (df['ff_angle_boundary'] < 0) & df['optimal_curvature'].isna()
...
def add_arc_info_to_df(df, curvature_df, arc_info_to_add=['optimal_curvature', 'curv_diff']):
    ...
    curvature_df_sub = curvature_df[['ff_index', 'point_index'] + arc_info_to_add].copy()
...
def add_column_monkey_passed_by_to_curvature_df(curvature_df, ff_dataframe, monkey_information):
    ...
    for i in range(len(unique_ff_index)):
        ...
        ff_dataframe_sub = ff_dataframe[(ff_dataframe['ff_index'] == ff_index) & (ff_dataframe.ff_distance <= pass_by_within_n_cm)].copy()
        # include those time points and 2.5s before those time points
        ff_dataframe_sub['start_of_duration'] = ff_dataframe_sub.time - pass_by_within_next_n_seconds
        duration_df = ff_dataframe_sub[['time', 'start_of_duration']].copy().rename(columns={'time': 'end_of_duration'})
        # find the overlaps of these durations defined by time and time_minus_2_5s
        duration_df['end_of_previous_duration'] = duration_df['end_of_duration'].shift(1).fillna(0)
        ...
        duration_df.loc[duration_df['overlap_with_previous_duration']==True, 'start_of_duration'] = np.nan
        # forward fill the column
        duration_df['start_of_duration'] = duration_df['start_of_duration'].ffill()
        
        # deal with them similarly
        duration_df['overlap_with_next_duration'] = duration_df['overlap_with_previous_duration'].shift(-1).fillna(False)        
        duration_df.loc[duration_df['overlap_with_next_duration']==True, 'end_of_duration'] = np.nan
        duration_df['end_of_duration'] = duration_df['end_of_duration'].bfill()
        
        # now, let's only keep unique rows
        duration_df = duration_df[['start_of_duration', 'end_of_duration']].drop_duplicates()
        ...
        ## Method 3
        bin_inserted_in_time_array = np.searchsorted(monkey_t_array, duration_df.values, side='right')
        within_any_duration = []
        for duration_in_point_index in bin_inserted_in_time_array:
            within_any_duration.extend(range(duration_in_point_index[0], duration_in_point_index[1]))

        if len(within_any_duration) > 0:
            index_to_include = point_index_array[np.array(within_any_duration)]
            index_to_include_df = pd.concat([index_to_include_df, pd.DataFrame({'ff_index': np.repeat(ff_index, len(index_to_include)), 'point_index': index_to_include})], ignore_index=True)
