...
def find_shortest_arc_among_all_available_ff(ff_x, ff_y, monkey_x, monkey_y, monkey_angle, verbose=True):
    ...
    ff_xy, ff_distance, ff_angle, ff_angle_boundary, arc_length, arc_radius = find_arc_length_and_radius(ff_x, ff_y, monkey_x, monkey_y, monkey_angle, verbose=verbose)
...
def find_arc_length_and_radius(ff_x, ff_y, monkey_x, monkey_y, monkey_angle, verbose=True, ignore_error=False):
    ...
    if np.any(np.abs(ff_angle) > math.pi/4):
        ...
        if not ignore_error:
            raise ValueError(f"At least one ff is more than 45 degrees away from the monkey. The max is {max_angle}. If this is not desired, please eliminate them before calling this function.")
        else:
            if verbose:
                warnings.warn(f"At least one ff is more than 45 degrees away from the monkey. The max is {max_angle}. If this is not desired, please eliminate them before calling this function.")

     # supress warnings because invalid values might occur
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        arc_angle = pi - 2* np.arcsin(np.abs(ff_y_relative/ff_distance)) # result will be between (0, pi)
    ...
    arc_length[na_index] = ff_distance[na_index] # because na occurs when the monkey can just go a straight line to the ff, instead of an arc
...
def find_and_package_optimal_arc_info_for_plotting(best_arc_df, monkey_information=None, column_for_color=None, mode='cartesian',
    ...
    # Note, here arc_ff_xy is replaced by arc ending xy, which produces the same result if the normal optimal arc is used; but if optimal_arc_stop_at_closest_point_to_monkey_stop was True, then arc ending xy has to be used to mimic a new ff center
    ...
    if mode == 'cartesian':
        ...
        center_x, center_y, arc_starting_angle, arc_ending_angle = curvature_utils.find_polar_arc_center_and_angle(arc_radius, arc_measure, arc_end_direction)
    else:
...
def update_null_arc_info_based_on_fixed_arc_length(fixed_arc_length, null_arc_info):
    null_arc_info['all_arc_measure'] = fixed_arc_length/null_arc_info['all_arc_radius']
    
    ff_to_the_left = null_arc_info['all_arc_end_direction'] == 1
    null_arc_info.loc[ff_to_the_left, 'arc_ending_angle'] = null_arc_info.loc[ff_to_the_left, 'arc_starting_angle'] + null_arc_info.loc[ff_to_the_left, 'all_arc_measure']

    ff_to_the_right = null_arc_info['all_arc_end_direction'] == -1
    null_arc_info.loc[ff_to_the_right, 'arc_ending_angle'] = null_arc_info.loc[ff_to_the_right, 'arc_starting_angle'] - null_arc_info.loc[ff_to_the_right, 'all_arc_measure']

    return null_arc_info


def find_monkey_angle_after_an_arc(ff_xy, min_arc_ff_angle, center_x, center_y, monkey_angle):
    angle_from_center_to_stop_position = np.arctan2(ff_xy[1]-center_y, ff_xy[0]-center_x)
    # if angle_from_center_to_stop_position is the starting angle, then monkey_angle at the end of the trajectory should be angle_from_center_to_stop_position - pi/2
    if min_arc_ff_angle < 0: # ff is to the right of the monkey
        monkey_angle = angle_from_center_to_stop_position - pi/2
    # otherwise, it should be angle_from_center_to_stop_position + pi/2
    else:
        monkey_angle = angle_from_center_to_stop_position + pi/2
    return monkey_angle
...
def find_arc_xy_rotated(center_x, center_y, min_arc_radius, arc_starting_angle, arc_ending_angle, rotation_matrix=None, num_points=100):
    arc_x_array, arc_y_array = find_arc_xy(center_x, center_y, min_arc_radius, arc_starting_angle, arc_ending_angle, num_points=num_points)
    ...
    if rotation_matrix is not None:
        if arc_xy_rotated.ndim == 3:
            ...
            arc_xy_rotated = np.transpose(np.matmul(rotation_matrix, arc_xy_rotated_T), (1, 2, 0))
...
def show_null_agent_trajectory_func(duration, null_agent_starting_time, monkey_information, ff_dataframe, ff_caught_T_sorted,
    ...
    monkey_xy, monkey_angle, point_index = find_most_recent_monkey_information(monkey_information, current_moment)
    print('1st arch:')
    if type == 'most_aligned':
        if null_arc_info_for_plotting is None:
            raise ValueError('null_arc_info_for_plotting must be provided if type is \'most_aligned\'.')
    ...
    if show_starting_point_of_show_null_trajectory:
        ...
        marker = axes.scatter(monkey_xy_rotated[0], monkey_xy_rotated[1], s=50, zorder=5, color='darkorange')   
    ...
    if show_landing_point_of_show_null_trajectory & (type == 'shortest'):
        if min_arc_ff_xy_rotated is not None:
            marker = axes.scatter(min_arc_ff_xy_rotated[0], min_arc_ff_xy_rotated[1], s=40, zorder=5, color='blue')   
            legend_markers.append(marker)
            legend_names.append('Landing point of null trajectory')

    if show_null_agent_trajectory_2nd_time:
        if type == 'most_aligned':
                    raise ValueError('If type = \'most_alignt\', show_null_agent_trajectory_2nd_time cannot be True')
        elif type == 'shortest':
            print('2nd arch:')
            if min_arc_ff_xy is not None:
                ...
                if center_x is not None:
                    monkey_angle = find_monkey_angle_after_an_arc(min_arc_ff_xy, min_arc_ff_angle, center_x, center_y, monkey_angle)
                ...
                if show_landing_point_of_show_null_trajectory:
                    if min_arc_ff_xy is not None:
                        min_arc_ff_xy_rotated = np.matmul(R, min_arc_ff_xy.reshape(2,1)) 
                        marker = axes.scatter(min_arc_ff_xy_rotated[0], min_arc_ff_xy_rotated[1], s=40, zorder=3, color='darkorange')   

    if show_ff_to_be_considered_by_first_null_trajectory & (type=='shortest'):
        if ff_xy_to_be_considered is not None:
            marker = axes.scatter(ff_xy_to_be_considered[0], ff_xy_to_be_considered[1], s=60, zorder=2, color='gold')
            legend_markers.append(marker)
            legend_names.append('FFs to be considered by\nthe first null trajectory')            
...
def plot_shortest_arc_from_null_condition(axes, current_moment, ff_dataframe, ff_caught_T_sorted, monkey_xy, monkey_angle, rotation_matrix=None, assumed_memory_duration=2, arc_color='black',
    ...
    if reaching_boundary_ok:
        ff_xy = find_best_arc.find_point_on_ff_boundary_with_smallest_angle_to_monkey(ff_xy[:,0], ff_xy[:,1], monkey_xy[0], monkey_xy[1], monkey_angle)
    ...
    min_arc_length, min_arc_radius, min_arc_ff_xy, min_arc_ff_distance, min_arc_ff_angle, \
            min_arc_ff_angle_boundary = find_shortest_arc_among_all_available_ff(ff_xy[ff_to_be_considered,0], ff_xy[ff_to_be_considered,1], monkey_x, monkey_y, monkey_angle)
    ...
    if min_arc_radius > 0:
        ...
        center_x, center_y, arc_starting_angle, arc_ending_angle = curvature_utils.find_cartesian_arc_center_and_angle(monkey_xy, monkey_angle, ff_distance, ff_angle, min_arc_radius, min_arc_ff_xy, np.sign(min_arc_ff_angle), whether_ff_behind=whether_ff_behind)
    ...
    axes.plot(arc_xy_rotated[0], arc_xy_rotated[1], linewidth=2.5, color=arc_color, zorder=6)
...
def plot_null_arcs_from_best_arc_df(axes, relevant_point_index, null_arc_info_for_plotting, x0=0, y0=0, rotation_matrix=None, polar=False, 
    ...
    if 'values_for_color' in null_arc_info_for_plotting.columns:
        if null_arc_info_for_plotting['values_for_color'].values[0] is not None:
            ...
            line_colors = viridis(null_arc_info_for_plotting['values_for_color'].values)
    ...
    for point_index in relevant_point_index:
        indices = np.where(null_arc_info_for_plotting['arc_point_index'].values == point_index)[0]
        if len(indices) > 0:
            for index in indices:
                ...
                if polar is True:
                    r, theta = animation_utils.change_xy_to_polar(arc_xy_to_plot[0]-x0, arc_xy_to_plot[1]-y0)
                    theta = theta - math.pi/2 # since 0 is to the north
                    if alpha is None:
                        alpha = 0.7
                    if marker_size is None:
                        marker_size = 2
                    axes.scatter(theta, r, s=marker_size, alpha=alpha, zorder=zorder, color=current_line_color)
                else:
                    if alpha is None:
                        alpha = 0.3
                    if marker_size is None:
                        marker_size = 0.1
                    axes.scatter(arc_xy_to_plot[0]-x0, arc_xy_to_plot[1]-y0, s=marker_size, alpha=alpha, zorder=zorder, color=current_line_color)
...
def find_ff_near_intended_target(intended_target_ff_index, duration, ff_dataframe, max_distance_from_intended_target):
    ...
    ff_dataframe_sub = ff_dataframe[ff_dataframe['ff_index'] == intended_target_ff_index]
    ...
    ff_dataframe_sub = ff_dataframe[ff_dataframe['time'].between(duration[0], duration[1])]
    unique_ff = ff_dataframe_sub[['ff_x', 'ff_y', 'ff_index']].drop_duplicates()
    unique_ff['distance_from_intended_target'] = unique_ff.apply(lambda row: np.linalg.norm(row[['ff_x', 'ff_y']].values - intended_target_xy), axis=1)
    selected_ff = unique_ff[unique_ff['distance_from_intended_target'] <= max_distance_from_intended_target].drop_duplicates()
...
def eliminate_invalid_ff_for_null_arc(all_ff_index, all_point_index, ff_real_position_sorted, monkey_information):
    ...
    print(round(len(np.where(np.abs(ff_angle) >= pi/4)[0])/len(ff_angle)*100, 2), '% of ff has ff angle that is too large')
    print('Both of these cases are eliminated.')

    remaining_index_of_array = np.where((ff_y_relative >= 0) & (np.abs(ff_angle) >= pi/4))[0]
...
def find_point_indices_to_plot_null_arc(duration_to_plot, monkey_information, time_to_begin_plotting_null_arc=None, time_to_end_plotting_null_arc=None, time_between_every_two_null_arcs=1):
    ...
    if time_to_begin_plotting_null_arc is None:
        ...
        time_to_begin_plotting_null_arc = duration_to_plot[0]
        warnings.warn('time_to_begin_plotting_null_arc cannot be smaller than duration_to_plot[0]. It is set to be duration_to_plot[0].')
    if time_to_end_plotting_null_arc is None:
        ...
        time_to_end_plotting_null_arc = duration_to_plot[1]
        warnings.warn('time_to_end_plotting_null_arc cannot be larger than duration_to_plot[1].')
...
def make_pretty_plot_for_a_duration(duration_to_plot, best_arc_df, monkey_information, ff_dataframe, null_arc_info_for_plotting, null_arc_to_center_info_for_plotting, PlotTrials_args, pretty_null_arc_plot_kwargs, intended_target_ff_index=None,
    ...
    if eliminate_irrelevant_points_for_intended_target_before_or_after_crossing_boundary & (intended_target_ff_index is not None):
        if intended_target_ff_index >= 0: 
            best_arc_df_for_intended_target = best_arc_df[(best_arc_df.ff_index == intended_target_ff_index) & (best_arc_df.time.between(duration_to_plot[0], duration_to_plot[1]))]
            relevant_point_indices = best_arc_df_for_intended_target.point_index.values 
            if len(relevant_point_indices) > 0:
                duration_to_plot = eliminate_irrelevant_points_before_or_after_crossing_boundary(duration_to_plot, relevant_point_indices, monkey_information)
    ...
    if show_intended_target == True:
        if intended_target_ff_index is not None:
            pretty_null_arc_plot_kwargs['indices_of_ff_to_mark'] = [intended_target_ff_index] 
        else:
            raise ValueError('if show_intended_target is True, then intended_target_ff_index cannot be None.')

    if plot_null_arcs:
        if point_indices_to_plot_null_arc is None:
            point_indices_to_plot_null_arc = find_point_indices_to_plot_null_arc(duration_to_plot, monkey_information, time_to_begin_plotting_null_arc=time_to_begin_plotting_null_arc, time_to_end_plotting_null_arc=time_to_end_plotting_null_arc, time_between_every_two_null_arcs=time_between_every_two_null_arcs)
        else:
            point_indices_to_plot_null_arc = np.array(point_indices_to_plot_null_arc)
    else:
        point_indices_to_plot_null_arc = [None]

    if ff_indices_to_plot_null_arc is None:
        ff_indices_to_plot_null_arc = find_ff_indices_near_intended_target_to_plot_null_arc(duration_to_plot, ff_dataframe, intended_target_ff_index=intended_target_ff_index, ff_max_distance_to_intended_target=ff_max_distance_to_intended_target, ff_max_distance_to_path=ff_max_distance_to_path)
    
    temp_null_arc_info_df = null_arc_info_for_plotting[null_arc_info_for_plotting['arc_ff_index'].isin(ff_indices_to_plot_null_arc)].copy()
    temp_null_arc_to_center_info_df = null_arc_to_center_info_for_plotting[null_arc_to_center_info_for_plotting['arc_ff_index'].isin(ff_indices_to_plot_null_arc)].copy()
    ...
    for point_index_to_plot_null_arc in point_indices_to_plot_null_arc:
        if skip_plots_with_no_null_arc:
            if (len(temp_null_arc_to_center_info_df[temp_null_arc_to_center_info_df['arc_point_index']==point_index_to_plot_null_arc]) == 0) & \
                ...
                print('No null arc for point_index_to_plot_null_arc: ', point_index_to_plot_null_arc, '. Skipping this plot, since skip_plots_with_no_null_arc is True.')
                continue
        ...
        if whether_mark_path_where_intended_target_has_best_arc_among_all_ff:
            if intended_target_ff_index is None:
                raise ValueError('if whether_mark_path_where_intended_target_has_best_arc_among_all_ff is True, then intended_target_id cannot be None.')
            elif intended_target_ff_index >= 0: 
                best_arc_df_for_intended_target = best_arc_df[(best_arc_df.ff_index == intended_target_ff_index) & (best_arc_df.time.between(duration_to_plot[0], duration_to_plot[1]))]
                relevant_point_indices = best_arc_df_for_intended_target.point_index.values 
            axes = mark_path_where_intended_target_has_best_arc_among_all_ff_func(axes, best_arc_df_for_intended_target, relevant_point_indices, monkey_information, rotation_matrix=R)
        ...
        if title is not None:
            ...
            if add_intended_target_ff_index_to_title & (intended_target_ff_index is not None):
                current_title = current_title + ', Intended Target ff index: ' + str(intended_target_ff_index)
            if add_duration_to_plot_to_title:
                current_title = current_title + ', Duration: ' + str(round(duration_to_plot[0], 2)) + '-' + str(round(duration_to_plot[1], 2))
...
def find_ff_indices_near_intended_target_to_plot_null_arc(duration_to_plot, ff_dataframe, intended_target_ff_index=None, ff_max_distance_to_intended_target=2000, ff_max_distance_to_path=100):
    ff_dataframe_in_duration = ff_dataframe[ff_dataframe['time'].between(duration_to_plot[0], duration_to_plot[1])]
    ff_near_trajectory = np.unique(ff_dataframe_utils.keep_only_ff_that_monkey_has_passed_by_closely(ff_dataframe_in_duration, max_distance_to_ff=ff_max_distance_to_path).ff_index.values)
    if intended_target_ff_index is not None:
        ff_near_intended_target = find_ff_near_intended_target(intended_target_ff_index, duration_to_plot, ff_dataframe_in_duration, ff_max_distance_to_intended_target)
        ff_indices_to_plot_null_arc = np.intersect1d(ff_near_intended_target, ff_near_trajectory)
    else:
...
def show_percentile_by_color_func(fig, axes, best_arc_sub_info_valid_ff, R):
    ...
    viridis = matplotlib.colormaps['viridis']
    line_colors = viridis(best_arc_sub_info_valid_ff.diff_percentile_in_decimal.values)
    temp_ff_positions, temp_monkey_positions = plot_behaviors_utils.find_lines_to_connect_path_ff(best_arc_sub_info_valid_ff, target_indices=None, rotation_matrix=R, target_excluded = False)
    x0, y0 = 0, 0
    ...
    cbar2 = fig.colorbar(cm.ScalarMappable(norm=norm2, cmap=cmap), cax=fig.add_axes([0.95, 0.4, 0.05, 0.43]), orientation='vertical')
...
def make_plots_to_show_monkey_reaction_time(curvature_df,
    ...
    if whether_plotted == True:
        best_arc_sub_info = curvature_df[curvature_df.time.between(duration_to_plot[0], time)].copy()
        ...
        if not show_percentile_by_color:
            ...
            fig, axes = show_percentile_by_color_func(fig, axes, best_arc_sub_info_valid_ff, R)
