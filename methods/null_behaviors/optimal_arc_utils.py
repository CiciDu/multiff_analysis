...
def update_curvature_df_to_let_optimal_arc_stop_at_closest_point_to_monkey_stop(curvature_df, stop_ff_df, stops_near_ff_df, 
    ...
    old_stop_null_arc_info = show_null_trajectory.find_and_package_optimal_arc_info_for_plotting(
    ...
    arc_rows_closest_to_stop = show_planning_utils.get_optimal_arc_landing_points_closest_to_stop(
        old_stop_null_arc_info, stops_near_ff_df
    )
    ...
    if len(new_ff_x) != len(stop_ff_df):
        raise ValueError('Number of new firefly x coordinates must match the number of stops')
    ...
    new_ff_x, new_ff_y = show_planning_utils.make_new_ff_at_monkey_xy_if_within_1_cm(new_ff_x, new_ff_y, 
        stop_ff_df['monkey_x'].values, stop_ff_df['monkey_y'].values
        )

    # Find and package arc to center info for plotting
    stop_null_arc_info = show_null_trajectory.find_and_package_arc_to_center_info_for_plotting(
        stop_ff_df.point_index.values, stop_ff_df.ff_index.values, monkey_information, 
    ...
    curvature_df['optimal_arc_measure'] = stop_null_arc_info['all_arc_measure'].values
    curvature_df['optimal_arc_length'] = curvature_df['optimal_arc_measure'] * curvature_df['optimal_arc_radius']
    ...
    curvature_df[['optimal_arc_end_x', 'optimal_arc_end_y']] = arc_rows_closest_to_stop[['x', 'y']].values
    
    return curvature_df
    

def add_optimal_arc_measure_and_length(curvature_df, optimal_arc_stop_at_visible_boundary=False, ignore_error=False):
    ...
    if (abs(curvature_df['ff_angle_boundary']) > math.pi/4).sum() > 0:
        raise ValueError("At least one ff has an angle to boundary larger than pi/4. This is invalid. Please check the input.")
    ...
    curvature_df['optimal_arc_measure'] = np.abs(arc_starting_angle - arc_ending_angle)
    curvature_df['optimal_arc_length'] = curvature_df['optimal_arc_measure'] * curvature_df['optimal_arc_radius']

    # also find optimal arc end x and y
    curvature_df['optimal_arc_end_x'] = np.cos(arc_ending_angle)*arc_radius + center_x
    curvature_df['optimal_arc_end_y'] = np.sin(arc_ending_angle)*arc_radius + center_y

    # make sure that all landing points are within the reward boundary
    arc_end_distance_to_ff_center = np.sqrt((curvature_df['optimal_arc_end_x'] - curvature_df['ff_x'])**2 + (curvature_df['optimal_arc_end_y'] - curvature_df['ff_y'])**2)
...
def find_arc_center_in_world_coord(monkey_xy, monkey_angle, arc_radius, arc_end_direction):
    if monkey_xy.ndim == 1:
        ...
        monkey_x, monkey_y = monkey_xy[0,:], monkey_xy[1,:]
    angle_to_center_of_arc_from_monkey = monkey_angle + pi/2*arc_end_direction # relative to the monkey, the angle can only be 90 degrees or -90 degrees
...
def find_angle_from_arc_center_to_monkey_and_stop_position(arc_ff_xy, ff_angle, monkey_xy, center_x, center_y):
    ...
    angle_from_center_to_stop_position = find_angle_from_arc_center_to_ff(arc_ff_xy, center_x, center_y)
    angle_from_center_to_monkey = find_angle_from_arc_center_to_monkey(monkey_xy, center_x, center_y)
    angle_from_center_to_stop_position[np.abs(ff_angle) > math.pi/2] = angle_from_center_to_monkey[np.abs(ff_angle) > math.pi/2]
    return angle_from_center_to_monkey, angle_from_center_to_stop_position
...
def find_angle_from_arc_center_to_ff(arc_ff_xy, center_x, center_y):
    # the calculation for angle_from_center_to_stop_position is the same whether optimal arc or arc to ff center is used
    arc_ff_xy = arc_ff_xy.reshape(-1,2)
    angle_from_center_to_stop_position = np.arctan2(arc_ff_xy[:,1]-center_y, arc_ff_xy[:,0]-center_x)
    return angle_from_center_to_stop_position
...
def find_cartesian_arc_center_and_angle_for_optimal_arc(arc_ff_xy, monkey_xy, monkey_angle, ff_distance, ff_angle, arc_radius, arc_end_direction, whether_ff_behind=None,
                                                        optimal_arc_stop_at_visible_boundary=False, ignore_error=False):
    # Note: sometimes arc_ff_xy is replaced by arc ending xy, which produces the same result if the normal optimal arc is used; but if optimal_arc_stop_at_closest_point_to_monkey_stop was True, then arc ending xy has to be used to mimic a new ff center
    
    center_x, center_y = find_arc_center_in_world_coord(monkey_xy, monkey_angle, arc_radius, arc_end_direction)
    angle_from_center_to_monkey, angle_from_center_to_stop_position = find_angle_from_arc_center_to_monkey_and_stop_position(arc_ff_xy, ff_angle, monkey_xy, center_x, center_y)
    
    if optimal_arc_stop_at_visible_boundary:
        temp_null_arc_info = pd.DataFrame({
                                        'arc_ff_index': np.arange(len(ff_angle)), # this is just for keeping the ff in sequence, not for merging, so it's fine to use np.arange
                                        ...
                                        'arc_ending_angle': angle_from_center_to_stop_position
                                        })
        arc_rows_to_first_reach_boundary = show_planning_utils.get_optimal_arc_landing_points_when_first_reaching_visible_boundary(temp_null_arc_info)
        angle_from_center_to_stop_position = arc_rows_to_first_reach_boundary['angle'].values
    
    arc_starting_angle, arc_ending_angle = _find_cartesian_arc_starting_and_ending_angle(angle_from_center_to_monkey, angle_from_center_to_stop_position, ff_distance, ff_angle, arc_end_direction, 
                                                                                         whether_ff_behind=whether_ff_behind, ignore_error=ignore_error)

    return center_x, center_y, arc_starting_angle, arc_ending_angle


def _supply_curvature_df_with_optimal_arc_info(curvature_df, ff_radius_for_optimal_arc, optimal_arc_stop_at_visible_boundary=True, ignore_error=False):
    all_ff_angle = curvature_df['ff_angle'].values.copy()
    all_ff_distance = curvature_df['ff_distance'].values.copy()
    curvature_lower_bound, curvature_upper_bound = find_curvature_lower_and_upper_bound(all_ff_angle, all_ff_distance, ff_radius=ff_radius_for_optimal_arc)
    ...
    curvature_df['optimal_curvature'] = abs_optimal_curvature * np.sign(curvature_df['ff_angle'])
    curvature_df['optimal_arc_end_direction'] = np.sign(curvature_df['optimal_curvature'])
    # if any optimal_arc_end_direction is 0, raise an error
    if np.any(curvature_df['optimal_arc_end_direction'] == 0):
        raise ValueError("At least one optimal_arc_end_direction is 0, which is not possible. Please check the code.")
...
def find_curvature_lower_and_upper_bound(ff_angle, ff_distance, ff_radius=10):
    ...
    all_ff_angle_lower_bound = np.clip(all_ff_angle_lower_bound, a_min=-pi/4, a_max=pi/4)
    all_ff_angle_upper_bound = np.clip(all_ff_angle_upper_bound, a_min=-pi/4, a_max=pi/4)
    ...
    with warnings.catch_warnings():
        ...
        curvature_lower_bound, _ = find_arc_curvature(all_ff_angle_lower_bound, ff_distance_to_edge_for_lower_bound)
        curvature_upper_bound, _ = find_arc_curvature(all_ff_angle_upper_bound, ff_distance_to_edge_for_upper_bound)
    ...
    curvature_lower_bound = np.where(np.sign(curvature_lower_bound) != np.sign(ff_angle), 0, curvature_lower_bound)
    curvature_upper_bound = np.where(np.sign(curvature_upper_bound) != np.sign(ff_angle), 0, curvature_upper_bound)
...
def find_arc_curvature(ff_angle, ff_distance, invalid_curvature_ok=False):
    ...
    if invalid_curvature_ok is False:
        ...
        if np.any(np.abs(ff_angle) > pi/4):
            ...
            raise ValueError(f"Note: at least one ff has an angle larger than pi/4. The max is {max_angle}, This is invalid. Please check the input.")
...
def _find_cartesian_arc_starting_and_ending_angle(angle_from_center_to_monkey, angle_from_center_to_stop_position, ff_distance, ff_angle, arc_end_direction, whether_ff_behind=None,
                                                  reward_boundary_radius=25, ignore_error=False):

    arc_starting_angle = angle_from_center_to_monkey.copy()
    arc_ending_angle = angle_from_center_to_stop_position.copy()
    ...
    if whether_ff_behind is not None:
        ...
        arc_starting_angle[whether_ff_behind] = angle_from_center_to_stop_position[whether_ff_behind].copy()
        arc_ending_angle[whether_ff_behind] = angle_from_center_to_stop_position[whether_ff_behind].copy()
    ...
    df = _deal_with_delta_angles_greater_than_180_degrees(df, reward_boundary_radius=reward_boundary_radius, ignore_error=ignore_error)

    # deal with delta angles greater than 45 degrees
    df = _deal_with_delta_angles_greater_than_90_degrees(df, reward_boundary_radius=reward_boundary_radius, ignore_error=ignore_error)
...
def _deal_with_delta_angles_greater_than_180_degrees(df, reward_boundary_radius=25, ignore_error=False):
    ...
    df['delta_angle'] = np.abs(df['arc_ending_angle'] - df['arc_starting_angle'])
    ...
    if df[df['delta_angle'] > pi].shape[0] > 0:
        if not ignore_error:
            raise ValueError(f"At least one arc has an angle larger than pi, which is not possible. The largest is {np.max(df['arc_ending_angle'] - df['arc_starting_angle'])}. Please check the input.")
        else:
            print(f"Warning: At least one arc has an angle larger than pi, which is not possible. The largest is {np.max(df['arc_ending_angle'] - df['arc_starting_angle'])}. We will adjust them by making them a little less than pi.")
    ...
    if too_big_angle.sum() > 0:
        ...
        if too_big_angle.sum() > 0:
            ...
            if max_big_angle > 150:
                if not ignore_error:
                    raise ValueError(f"Error: max_big_angle is {max_big_angle} when ff is to the left. There is a problem here. Please check the input.")
                else:
                    print(f"Warning: max_big_angle is {max_big_angle} when ff is to the left. There is a problem here. We will adjust them by making them a little less than 90.")
            ...
            df.loc[too_big_angle & ff_at_left, 'arc_ending_angle'] = df.loc[too_big_angle & ff_at_left, 'arc_starting_angle'] + pi/2 - 0.00001
            df.loc[too_big_angle & ff_at_right, 'arc_ending_angle'] = df.loc[too_big_angle & ff_at_right, 'arc_starting_angle'] - (pi/2 - 0.00001)
    return df
