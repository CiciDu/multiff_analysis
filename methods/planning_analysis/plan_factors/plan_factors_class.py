...
class PlanFactors(stops_near_ff_based_on_ref_class.StopsNearFFBasedOnRef):
    ...
    def _make_plan_x_and_y_test_OR_plan_x_and_y_ctrl(self, test_or_control='test', already_made_ok=True, plan_x_exists_ok=True, plan_y_exists_ok=True, heading_info_df_exists_ok=True,
        ...
        if already_made_ok:
            try:
                ...
                pass
        self._make_plan_y_test_OR_plan_y_ctrl(test_or_control=test_or_control, exists_ok=plan_y_exists_ok, heading_info_df_exists_ok=heading_info_df_exists_ok, stops_near_ff_df_exists_ok=stops_near_ff_df_exists_ok, save_data=save_data)
    ...
    def _prepare_plan_data(self, plan_type, test_or_control, exists_ok, df_name_func, make_plan_func, save_data):
        ...
        if plan_type == 'plan_x':
            ...
            folder_name = os.path.join(self.planning_data_folder_path, self.plan_y_partial_path, test_or_control)
        else:
        ...
        if exists_ok & os.path.exists(csv_path):
            ...
            print(f'Successfully retrieved {plan_type}_{test_or_ctrl} ({df_name})')
        else:
            ...
            if save_data:
                ...
                print(f'Made {plan_type}_{test_or_ctrl} and saved to {csv_path}')
        return plan_data
    ...
    def _make_plan_x_test_OR_plan_x_ctrl(self, test_or_control='test', exists_ok=True, use_eye_data=True, 
        ...
        def make_plan_x():
            self.get_stops_near_ff_df(test_or_control=test_or_control, exists_ok=stops_near_ff_df_exists_ok, save_data=save_data)
    ...
    def _make_plan_y_test_OR_plan_y_ctrl(self, test_or_control='test', exists_ok=True, heading_info_df_exists_ok=True, stops_near_ff_df_exists_ok=True, save_data=True):
        ...
        def make_plan_y():
            ...
            # prepare to add monkey_angle_when_stop_ff_first_seen
            self.stop_ff_df_modified = self.stop_ff_df_modified.merge(self.stop_ff_df[['stop_point_index', 'point_index_ff_first_seen']], on='stop_point_index', how='left').sort_values(by='stop_point_index')
            self.stop_ff_df_temp = find_stops_near_ff_utils.find_ff_info(self.stop_ff_df_modified.ff_index.values, self.stop_ff_df_modified['point_index_ff_first_seen'].values, 
                                                                         self.monkey_information, self.ff_real_position_sorted)
            
            plan_y = plan_factors_utils.make_plan_y_df(self.heading_info_df, self.curv_of_traj_df, self.curv_of_traj_df_w_one_sided_window)

            plan_y = plan_factors_utils.add_d_monkey_angle(plan_y, self.stop_ff_df_temp, self.stops_near_ff_df)
    ...
    def make_the_distributions_of_angles_more_similar(self):
        ...
        self.plan_x_ctrl, self.plan_x_test = test_vs_control_utils.make_the_distributions_of_angles_more_similar(self.plan_x_ctrl, self.plan_x_test, column_name='stop_ff_angle_at_ref')
        self.plan_y_ctrl = self.plan_y_ctrl[self.plan_y_ctrl['unique_id'].isin(self.plan_x_ctrl['unique_id'])].sort_values(by='unique_id').drop(columns=['unique_id'])
    ...
    def make_x_and_y_var_df(self, test_data_only=False, control_data_only=False, scale_x_var=True, use_pca=False):
        if test_data_only & control_data_only:
            raise ValueError('Both test_data_only and control_data_only are True')
        if test_data_only:
        ...
        self.x_var_df, self.y_var_df = ml_methods_utils.make_x_and_y_var_df(x_df, y_df, scale_x_var=scale_x_var, use_pca=use_pca)      


    def get_x_and_y_for_lr(self, test_or_control='test', scale_x_var=True, use_pca=False):
        if test_or_control == 'test':
            self.make_x_and_y_var_df(test_data_only=True, scale_x_var=scale_x_var, use_pca=use_pca)
        elif test_or_control == 'control':
            self.make_x_and_y_var_df(control_data_only=True, scale_x_var=scale_x_var, use_pca=use_pca)
        else:
            self.make_x_and_y_var_df(scale_x_var=scale_x_var, use_pca=use_pca)


    def _run_lr(self, y_var_column):
        self.ml_inst.use_train_test_split(self.x_var_df, self.y_var_df, y_var_column=y_var_column)
    ...
    def use_lr_on_all(self, test_or_control='test', y_var_column='d_monkey_angle2', use_pca=False):
        self.get_x_and_y_for_lr(test_or_control=test_or_control, use_pca=use_pca)
    ...
    def use_lr_on_specific_columns(self, specific_columns=None, test_or_control='test', y_var_column='d_monkey_angle2'):
        if specific_columns is None:
            self.specific_columns = self.summary_df[self.summary_df['p_value'] < 0.05].index.values
        else:
    ...
    def use_lr_on_one_column(self, column=None, test_or_control='test', y_var_column='d_monkey_angle2'):
        self.get_x_and_y_for_lr(test_or_control=test_or_control)
        if column is None:
            column = 'stop_ff_angle_at_ref'
        self.x_var_df = self.x_var_df[column].copy()
        self._run_lr(y_var_column)


    def get_alt_ff_at_stop_df(self):

        self.alt_ff_df_temp = find_stops_near_ff_utils.find_ff_info(self.alt_ff_df_modified.ff_index.values, self.alt_ff_df_modified['stop_point_index'].values, 
                                                                    self.monkey_information, self.ff_real_position_sorted)
        self.alt_ff_at_stop_df = self.alt_ff_df_temp[['ff_distance', 'ff_angle']].copy()
        self.alt_ff_at_stop_df.rename(columns={'ff_distance': 'alt_ff_distance_at_stop', 
                                               'ff_angle': 'alt_ff_angle_at_stop'}, inplace=True)

        return self.alt_ff_at_stop_df
    

    def get_both_ff_when_seen_df(self, crossing_ff=False, deal_with_rows_with_big_ff_angles=False):
        # This contains the planning-related information at specific time (such as when stop_ff was last visibl)
        # If crossing_ff is true, we'll get alt_ff_info when stop_ff was first/last seen, and vice versa
        
        print('Making both_ff_when_seen_df...')
        self.both_ff_when_seen_df = self.alt_ff_df[['stop_point_index']].copy().set_index('stop_point_index')
        for first_or_last in ['first', 'last']:
            for when_which_ff, ff_df in [('when_alt_ff', self.alt_ff_df),
                                         ('when_stop_ff', self.stop_ff_df)]:
                all_point_index = ff_df[f'point_index_ff_{first_or_last}_seen'].values
                self._find_alt_ff_df_2_and_stop_ff_df_2_based_on_specific_point_index(all_point_index=all_point_index)
                ...
                for which_ff_info in ['alt_', 'stop_']:
                    if (when_which_ff == 'when_stop_ff') & (first_or_last == 'first') & (which_ff_info =='stop_'):
                        continue  # because the information is already contained in stop ff info at ref point

                    if not crossing_ff:
                        if (which_ff_info == 'alt_') & (when_which_ff == 'when_stop_ff'):
                            continue
                        if (which_ff_info == 'stop_') & (when_which_ff == 'when_alt_ff'):
                            continue
                    if deal_with_rows_with_big_ff_angles:
                        ff_df_modified = self.alt_ff_df_modified if which_ff_info == 'alt_' else self.stop_ff_df_modified
                    else:
                        ff_df_modified = self.alt_ff_df2 if which_ff_info == 'alt_' else self.stop_ff_df2
                    ...
                    if len(curv_df) != len(ff_df_modified):
                        raise ValueError('The length of curv_df is not the same as the length of ff_df_modified')
    ...
    def _make_plan_x_df(self, use_eye_data=True, use_speed_data=True, stop_period_duration=2, ff_radius=10,
        ...
        self.both_ff_at_ref_df['stop_point_index'] = self.alt_ff_df2['stop_point_index']
