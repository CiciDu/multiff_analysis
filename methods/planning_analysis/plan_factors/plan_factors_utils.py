import sysfrom planning_analysis.show_planning import alt_ff_utils, show_planning_class, show_planning_utils
...
def drop_na_in_x_var(x_var_df, y_var_df):
    ...
    if x_var_df.isnull().any(axis=1).sum() > 0:
        print(f'Number of rows with NaN values in x_var_df: {x_var_df.isnull().any(axis=1).sum()} out of {x_var_df.shape[0]} rows. The rows with NaN values will be dropped.')
...
def drop_na_in_x_and_y_var(x_var_df, y_var_df):
    ...
    if y_var_df.isnull().any(axis=1).sum() > 0:
        print(f'Number of rows with NaN values in y_var_df (after cleaning x_var_df and the corresponding rows in y_var_df): {y_var_df.isnull().any(axis=1).sum()} out of {y_var_df.shape[0]} rows. The rows with NaN values will be dropped.')
...
def find_curv_of_traj_stat_df(df_to_iter, curv_of_traj_df, start_time_column='stop_time',
    ...
    if add_to_df_to_iter:
        df_to_iter = add_stat_columns_to_df_to_iter(curv_of_traj_stat_df, df_to_iter, stat_prefix, groupby_column)
...
def extend_df_based_on_groups(ori_df, all_start_time, all_end_time, all_group_id, group_id='stop_point_index'):
    ...
    for i in range(len(all_start_time)):
        ...
        cum_indices = ori_df.loc[ori_df['time'].between(start_time, end_time)].index.values
...
def add_stat_columns_to_df_to_iter(stat_df, df_to_iter, stat_prefix, groupby_column):
    for prefix in stat_prefix:
        ...
        df_to_iter = df_to_iter.merge(stat_df[columns_to_add + [groupby_column]], on=groupby_column, how='left')
    return df_to_iter
...
def find_stat_of_columns_after_groupby(df_for_stat_extended,
    ...
    dict_of_column_to_prefix = {column: prefix for column, prefix in zip(stat_columns, stat_prefix)}

    stat_df = df_for_stat_extended.groupby(groupby_column)[stat_columns].describe()
    stat_df.columns = [dict_of_column_to_prefix[column[0]] + column[1] for column in stat_df.columns]

    for prefix in stat_prefix:
        stat_df[prefix + 'range'] = stat_df[prefix + 'max'] - stat_df[prefix + 'min']
        stat_df[prefix + 'iqr'] = stat_df[prefix + '75%'] - stat_df[prefix + '25%']
...
def get_eye_perc_df(df_for_stat_extended, list_of_max_degrees=[5, 10]):
    ...
    for eye in ['left_eye', 'right_eye']:
        for stop_or_alt in ['stop', 'alt']:
            for max_degrees in list_of_max_degrees:
                new_column = f'{eye}_{stop_or_alt}_ff_time_perc_{max_degrees}'
                df[new_column] = df['dt']
                df.loc[(df[eye_to_gaze[eye]] - df[f'{stop_or_alt}_ff_angle']).abs() > max_degrees/180 * math.pi, new_column] = 0
                df.loc[df[eye_to_gaze[eye]].isnull(), new_column] = 0
                eye_columns.append(new_column)

    eye_perc_df = df[eye_columns + ['dt', 'stop_point_index']].groupby('stop_point_index').sum()
...
def make_plan_y_df(heading_info_df, curv_of_traj_df, curv_of_traj_df_w_one_sided_window):
    ...
    curv_of_traj_stat_df, _ = find_curv_of_traj_stat_df(heading_info_df, curv_of_traj_df, add_to_df_to_iter=False)
    ...
    if curv_of_traj_stat_df.isnull().any(axis=1).sum() > 0:
        print(f'Number of rows with NaN values in plan_y_df: {curv_of_traj_stat_df.isnull().any(axis=1).sum()} out of {curv_of_traj_stat_df.shape[0]} rows.')
    ...
    #                                     'alt_ff_cluster_last_flash_time_bbas']].copy()
    ...
    # drop the columns if already exist
    heading_info_df = heading_info_df.drop(columns=columns_to_add, errors='ignore')
...
def find_ff_visible_info_in_a_period(list_of_ff_index, ff_dataframe_visible, start_point_index=None, end_point_index=None, start_time=None, end_time=None):
    if (start_point_index is not None) & (end_point_index is not None):
        ff_info = ff_dataframe_visible[ff_dataframe_visible['point_index'].between(start_point_index, end_point_index)].copy()
    elif (start_time is not None) & (end_time is not None):
        ff_info = ff_dataframe_visible[ff_dataframe_visible['time'].between(start_time, end_time)].copy()
    else:
        raise ValueError('Please provide either start_point_index and end_point_index or start_time and end_time.')
...
def find_ff_visible_duration_in_a_period(list_of_ff_index, ff_dataframe_visible, start_point_index=None, end_point_index=None, start_time=None, end_time=None):
    ...
    ff_visible_duration = ff_visible_info[['ff_index', 'dt']].groupby('ff_index').sum()
    ff_visible_duration = ff_visible_duration.rename(columns={'dt': 'visible_duration'})
    ff_visible_duration = ff_visible_duration.copy()
    return ff_visible_duration


def get_other_factors(stops_near_ff_df):
    other_factors = stops_near_ff_df[['stop_point_index', 'd_from_stop_ff_to_alt_ff']].copy()
    # Get the radius of stop points
    radius = np.linalg.norm(stops_near_ff_df[['stop_x', 'stop_y']].values, axis=1)
    ...
    other_factors['time_between_two_stops'] = stops_near_ff_df['next_stop_time'] - stops_near_ff_df['stop_time']
    return other_factors


def get_point_index_of_alt_ff_last_seen_before_next_stop(ff_dataframe_visible, stops_near_ff_df):
    all_group_identifiers, all_stop_time, all_next_stop_time, all_ff_index = stops_near_ff_df['stop_point_index'].values, stops_near_ff_df['stop_time'].values, stops_near_ff_df['next_stop_time'].values, stops_near_ff_df['alt_ff_index'].values
    #time_between_stop_and_next_stop = stops_near_ff_df['next_stop_time'].values - stops_near_ff_df['stop_time'].values
    ff_last_seen_info = alt_ff_utils.find_info_when_ff_was_first_or_last_seen_between_start_time_and_end_time(all_ff_index, all_group_identifiers, all_stop_time-2.5, 
...
def get_alt_ff_last_seen_info_before_next_stop(alt_ff_df2, ff_dataframe_visible, monkey_information, stops_near_ff_df, ff_real_position_sorted):
    last_seen_point_index = get_point_index_of_alt_ff_last_seen_before_next_stop(ff_dataframe_visible, stops_near_ff_df)
    alt_ff_df2 = find_stops_near_ff_utils.find_ff_info(stops_near_ff_df.alt_ff_index.values, last_seen_point_index, monkey_information, ff_real_position_sorted)
    alt_ff_df2['stop_point_index'] = stops_near_ff_df['stop_point_index'].values
    alt_ff_df2['time_last_seen'] = monkey_information.loc[alt_ff_df2['point_index'].values, 'time'].values
    alt_ff_df2 = alt_ff_df2.merge(stops_near_ff_df[['stop_point_index', 'next_stop_time']], on='stop_point_index', how='left')
    alt_ff_df2['time_alt_ff_last_seen_before_next_stop'] = alt_ff_df2['next_stop_time'].values - alt_ff_df2['time_last_seen'].values

    alt_ff_last_seen_info = alt_ff_df2[['ff_distance', 'ff_angle', 'time_alt_ff_last_seen_before_next_stop']].copy()
    alt_ff_last_seen_info.rename(columns={'ff_distance': 'alt_ff_distance_when_alt_ff_last_seen_before_next_stop',
                            'ff_angle': 'alt_ff_angle_when_alt_ff_last_seen_before_next_stop'}, inplace=True)
...
def add_dir_from_stop_ff_same_side(plan_y_df):
...
def get_eye_data_etc(stops_near_ff_df, monkey_information, ff_real_position_sorted, max_degrees=5,
                      column_suffix=''):
    left_eye_alt_ff_time_perc = [] # meaning left_eye_within_n_deg_to_alt_ff_time
    right_eye_alt_ff_time_perc = []
    left_eye_stop_ff_time_perc = []
    right_eye_stop_ff_time_perc = []
    left_eye_alt_ff_after_stop_time_perc = []
    right_eye_alt_ff_after_stop_time_perc = []
    ...
    for index, row in stops_near_ff_df.iterrows():
        ...
        ff_index = row['stop_ff_index']
        left_eye_time, right_eye_time, monkey_sub = _get_left_and_right_eye_time(ff_index, duration, monkey_information, ff_real_position_sorted, max_degrees)
        left_eye_stop_ff_time_perc.append(left_eye_time/duration_length)
        right_eye_stop_ff_time_perc.append(right_eye_time/duration_length)  
        raw_eye_data_row = get_quartile_data_in_a_row(monkey_sub, ['LDy', 'LDz', 'RDy', 'RDz'], suffix='')
        monkey_speed_row = get_quartile_data_in_a_row(monkey_sub, ['monkey_speed', 'monkey_dw'], suffix='')

        ff_index = row['alt_ff_index']
        left_eye_time, right_eye_time, monkey_sub = _get_left_and_right_eye_time(ff_index, duration, monkey_information, ff_real_position_sorted, max_degrees)
        left_eye_alt_ff_time_perc.append(left_eye_time/duration_length)
        right_eye_alt_ff_time_perc.append(right_eye_time/duration_length) 

        duration = [row['stop_time'], row['next_stop_time']]
        duration_length = duration[1] - duration[0]
        left_eye_time, right_eye_time, monkey_sub = _get_left_and_right_eye_time(ff_index, duration, monkey_information, ff_real_position_sorted, max_degrees)
        left_eye_alt_ff_after_stop_time_perc.append(left_eye_time/duration_length)
        right_eye_alt_ff_after_stop_time_perc.append(right_eye_time/duration_length)  

        raw_eye_data_row2 = get_quartile_data_in_a_row(monkey_sub, ['LDy', 'LDz', 'RDy', 'RDz'], suffix='_after_stop')
        raw_eye_data_row = pd.concat([raw_eye_data_row, raw_eye_data_row2], axis=1)
        raw_eye_data_df = pd.concat([raw_eye_data_df, raw_eye_data_row], axis=0)

        monkey_speed_row2 = get_quartile_data_in_a_row(monkey_sub, ['monkey_speed', 'monkey_dw'], suffix='_after_stop')
        monkey_speed_row = pd.concat([monkey_speed_row, monkey_speed_row2], axis=1)
        monkey_speed_df = pd.concat([monkey_speed_df, monkey_speed_row], axis=0)

    raw_eye_data_df = raw_eye_data_df.reset_index(drop=True)
    monkey_speed_df = monkey_speed_df.reset_index(drop=True)

    eye_factor_df = pd.DataFrame({'left_eye_alt_ff_time_perc' + column_suffix: np.array(left_eye_alt_ff_time_perc), 
                                'right_eye_alt_ff_time_perc' + column_suffix: np.array(right_eye_alt_ff_time_perc),
                                'left_eye_stop_ff_time_perc' + column_suffix: np.array(left_eye_stop_ff_time_perc), 
                                'right_eye_stop_ff_time_perc' + column_suffix: np.array(right_eye_stop_ff_time_perc),
                                'left_eye_alt_ff_after_stop_time_perc' + column_suffix: np.array(left_eye_alt_ff_after_stop_time_perc),
                                'right_eye_alt_ff_after_stop_time_perc' + column_suffix: np.array(right_eye_alt_ff_after_stop_time_perc),
                                })
...
def get_quartile_data_in_a_row(monkey_sub, columns, suffix=''):
    ...
    for column in columns:
        quartile_data_dict[column + '_25%' + suffix] = monkey_sub[column].quantile(0.25)
        quartile_data_dict[column + '_50%' + suffix] = monkey_sub[column].quantile(0.5)
        quartile_data_dict[column + '_75%' + suffix] = monkey_sub[column].quantile(0.75)
...
def _get_left_and_right_eye_time(ff_index, duration, monkey_information, ff_real_position_sorted, max_degrees=5):
    ...
    right_eye = monkey_sub[(monkey_sub['gaze_monkey_view_angle_r'] - monkey_sub['ff_angle']).abs() <= max_degrees/180 * math.pi].copy()
...
def make_cluster_df_as_part_of_plan_factors(stops_near_ff_df, ff_dataframe_visible, monkey_information, ff_real_position_sorted, 
                                            ...
                                            guarantee_stop_ff_info_for_cluster=False,
                                            guarantee_alt_ff_info_for_cluster=False,
                                            flash_or_vis='vis',
                                            columns_not_to_include=[]           
                                            ):

    all_start_time = stops_near_ff_df['stop_time'].values - stop_period_duration
    all_end_time = stops_near_ff_df['next_stop_time'].values
    all_group_id = stops_near_ff_df['stop_point_index'].values

    monkey_info_in_all_stop_periods = only_stop_ff_utils.find_monkey_info_in_all_stop_periods(all_start_time, all_end_time, all_group_id, monkey_information)
    monkey_info_to_add = [column for column in monkey_info_in_all_stop_periods.columns if (column not in ff_dataframe_visible.columns)]
    ff_info_in_all_stop_periods = ff_dataframe_visible.merge(monkey_info_in_all_stop_periods[monkey_info_to_add + ['point_index']], on=['point_index'], how='right')
    ff_info_in_all_stop_periods = ff_info_in_all_stop_periods[~ff_info_in_all_stop_periods['ff_index'].isnull()].copy()
    ...
    vis_time_info = ff_info_in_all_stop_periods.groupby(['ff_index', 'stop_point_index']).agg(earliest_vis_point_index=('point_index', 'min'),
                                                                                            latest_vis_point_index=('point_index', 'max'),
                                                                                            earliest_vis_time=('time', 'min'),
                                                                                            latest_vis_time=('time', 'max'),
                                                                                            vis_duration=('dt', 'sum'))
    
    if guarantee_stop_ff_info_for_cluster:
        stop_ff_info = stops_near_ff_df[['stop_point_index', 'stop_ff_index']].rename(columns={'stop_ff_index': 'ff_index'}).copy()
        vis_time_info = vis_time_info.merge(stop_ff_info, on=['ff_index', 'stop_point_index'], how='outer')
    if guarantee_alt_ff_info_for_cluster:
        alt_ff_info = stops_near_ff_df[['stop_point_index', 'alt_ff_index']].rename(columns={'alt_ff_index': 'ff_index'}).copy()
        vis_time_info = vis_time_info.merge(alt_ff_info, on=['ff_index', 'stop_point_index'], how='outer')
    ...
    stops_near_ff_columns_to_add = [column for column in stops_near_ff_df.columns if (column not in vis_time_info.columns)]
    ff_info_in_all_stop_periods = vis_time_info.merge(stops_near_ff_df[stops_near_ff_columns_to_add + ['stop_point_index']], on='stop_point_index', how='left')
    ff_info_in_all_stop_periods.reset_index(drop=True, inplace=True)

    # add info at ref point
    _, _, stop_ff_df = alt_ff_utils.get_alt_ff_df_and_stop_ff_df(stops_near_ff_df)
    stop_ff_df2 = find_stops_near_ff_utils.find_ff_info_based_on_ref_point(stop_ff_df, monkey_information, ff_real_position_sorted,
                                                                    ref_point_mode=ref_point_mode, ref_point_value=ref_point_value)
    
    ref_info = stop_ff_df2[['stop_point_index', 'point_index', 'monkey_x', 'monkey_y', 'monkey_angle']].rename(columns={'point_index': 'ref_point_index'}).copy()
    ...
    ref_info_columns_to_add = [column for column in ref_info.columns if (column not in ff_info_in_all_stop_periods.columns)]
    ff_info_in_all_stop_periods = ff_info_in_all_stop_periods.merge(ref_info[ref_info_columns_to_add + ['stop_point_index']], on='stop_point_index', how='left')

    # add ff info
    ff_info_in_all_stop_periods['ff_x'], ff_info_in_all_stop_periods['ff_y'] = ff_real_position_sorted[ff_info_in_all_stop_periods['ff_index'].values].T
    ff_info_in_all_stop_periods = only_stop_ff_utils._add_basic_ff_info_to_df_for_ff(ff_info_in_all_stop_periods, ff_radius=ff_radius)
    ff_info_in_all_stop_periods = furnish_ff_info_in_all_stop_periods(ff_info_in_all_stop_periods)


    # identify clusters based on various criteria
    ff_info_in_all_stop_periods, all_cluster_names = find_clusters_in_ff_info_in_all_stop_periods(ff_info_in_all_stop_periods, list_of_stop_ff_cluster_radius=list_of_stop_ff_cluster_radius,
                                                                                                    list_of_alt_ff_cluster_radius=list_of_alt_ff_cluster_radius)

    # get cluster info
    cluster_factors_df, cluster_agg_df = only_stop_ff_utils.get_cluster_and_agg_df(ff_info_in_all_stop_periods, all_cluster_names,
                                                                                  flash_or_vis=flash_or_vis, columns_not_to_include=columns_not_to_include) 
    # combine cluster info
    cluster_df = cluster_factors_df.merge(cluster_agg_df, on='stop_point_index', how='outer').reset_index(drop=True)
...
def find_clusters_in_ff_info_in_all_stop_periods(ff_info_in_all_stop_periods,
    ...
    for n_cm in list_of_stop_ff_cluster_radius:
        ...
        ff_info_in_all_stop_periods.loc[ff_info_in_all_stop_periods['ff_distance_to_stop_ff'] <= n_cm, column] = True

    for n_cm in list_of_alt_ff_cluster_radius:
        column = f'alt_ff_cluster_{n_cm}'
        all_cluster_names.append(column)
        ff_info_in_all_stop_periods[column] = False
        ff_info_in_all_stop_periods.loc[ff_info_in_all_stop_periods['ff_distance_to_alt_ff'] <= n_cm, column] = True
    
    whether_each_cluster_has_enough = ff_info_in_all_stop_periods[all_cluster_names + ['stop_point_index']].groupby('stop_point_index').sum() == 0
...
def furnish_ff_info_in_all_stop_periods(df):
    ...
    df['stop_ff_distance'] = np.linalg.norm([df['stop_ff_x'] - df['ff_x'], df['stop_ff_y'] - df['ff_y']], axis=0)
    df['ff_distance_to_stop_ff'] = np.linalg.norm([df['stop_ff_x'] - df['ff_x'], df['stop_ff_y'] - df['ff_y']], axis=0)
    df['ff_distance_to_alt_ff'] = np.linalg.norm([df['alt_ff_x'] - df['ff_x'], df['alt_ff_y'] - df['ff_y']], axis=0)
    
    df['angle_diff_boundary'] = df['ff_angle'] - df['ff_angle_boundary']
    df['angle_diff_boundary'] = df['angle_diff_boundary'] % (2*math.pi)
    df.loc[df['angle_diff_boundary'] > math.pi, 'angle_diff_boundary'] = df.loc[df['angle_diff_boundary'] > math.pi, 'angle_diff_boundary'] - 2*math.pi
    
    return df


monkey_info_columns = ['left_eye_stop_ff_time_perc_5',
...
ff_at_ref_columns = ['alt_ff_distance_at_ref',
                    'alt_ff_angle_at_ref',
                    'stop_ff_distance_at_ref',
                    'stop_ff_angle_at_ref',
                    'stop_ff_angle_boundary_at_ref',
                    'stop_ff_angle_diff_boundary_at_ref']
...
def process_plan_x_to_predict_monkey_info(plan_x, for_classification=False):
    ...
    if for_classification:
        if 'dir_from_stop_ff_to_alt_ff' in plan_x.columns:
            cluster_columns_to_save.append('dir_from_stop_ff_to_alt_ff')
...
def process_plan_x_to_predict_ff_info(plan_x, plan_y):
    ...
    cluster_columns_to_save = [col for col in plan_x.columns if (col in non_cluster_columns_to_save) | ('cluster' in col)]
    # delete any column that contains 'alt'
    cluster_columns_to_save = [col for col in cluster_columns_to_save if 'alt' not in col]
...
def make_plan_xy_test_and_plan_xy_ctrl(plan_x_both, plan_y_both):
    ...
    plan_xy_test = plan_xy[plan_xy['whether_test'] == 1].reset_index(drop=True).copy()
    plan_xy_ctrl = plan_xy[plan_xy['whether_test'] == 0].reset_index(drop=True).copy()
...
def quickly_process_plan_xy_test_and_ctrl(plan_xy_test, plan_xy_ctrl, column_for_split, whether_filter_info, finalized_params):
    ...
    if whether_filter_info:
        test_df, ctrl_df = test_vs_control_utils.filter_both_df(test_df, ctrl_df, **finalized_params)
...
def make_plan_x_df(stops_near_ff_df, heading_info_df, both_ff_at_ref_df, ff_dataframe, monkey_information, ff_real_position_sorted,
    ...
    cluster_df = make_cluster_df_as_part_of_plan_factors(stops_near_ff_df, ff_dataframe_visible, monkey_information, ff_real_position_sorted, 
                                                        ...
                                                        guarantee_stop_ff_info_for_cluster=guarantee_stop_ff_info_for_cluster, 
                                                        guarantee_alt_ff_info_for_cluster=guarantee_alt_ff_info_for_cluster,
    ...
    plan_x_df = cluster_df.merge(other_factors, on='stop_point_index', how='left')
    plan_x_df = plan_x_df.merge(both_ff_at_ref_df, on='stop_point_index', how='left').reset_index(drop=True)
    
    # alt_ff_last_seen_info = get_alt_ff_last_seen_info_before_next_stop(alt_ff_df2, ff_dataframe_visible, monkey_information, 
    #                                                                                       stops_near_ff_df, ff_real_position_sorted)
    # plan_x_df = pd.concat([plan_x_df, alt_ff_last_seen_info])

    if use_speed_data or use_eye_data:
        df_for_stat_extended = get_df_for_stat_extended_for_eye_info(stops_near_ff_df, monkey_information)
    
    if use_speed_data:
        monkey_speed_stat_df = find_stat_of_columns_after_groupby(df_for_stat_extended,
                                                        groupby_column='stop_point_index',
                                                        stat_columns=['monkey_speed', 'monkey_dw'])
        columns_to_preserve = [column for column in monkey_speed_stat_df if ('iqr' in column) | ('range' in column) | ('std' in column) | (column=='stop_point_index')]
        monkey_speed_stat_df = monkey_speed_stat_df[columns_to_preserve].copy()

        plan_x_df = plan_x_df.merge(monkey_speed_stat_df, on='stop_point_index', how='left')

    if use_eye_data:
        eye_stat_df = find_stat_of_columns_after_groupby(df_for_stat_extended,
                                                        groupby_column='stop_point_index',
                                                        stat_columns=['LDy', 'LDz', 'RDy', 'RDz'])
        columns_to_preserve = [column for column in eye_stat_df if ('iqr' in column) | ('range' in column) | ('std' in column) | (column=='stop_point_index')]
        eye_stat_df = eye_stat_df[columns_to_preserve].copy()

        eye_perc_df = get_eye_perc_df(df_for_stat_extended)

        plan_x_df = plan_x_df.merge(eye_stat_df, on='stop_point_index', how='left').merge(eye_perc_df, on='stop_point_index', how='left').reset_index(drop=True)


    # only keep the rows with stop_point_index that are in heading_info_df
    plan_x_df = plan_x_df[plan_x_df['stop_point_index'].isin(heading_info_df['stop_point_index'])].copy()
    plan_x_df = plan_x_df.sort_values(by='stop_point_index').reset_index(drop=True)
