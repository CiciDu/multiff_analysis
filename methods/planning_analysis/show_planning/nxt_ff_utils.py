...
def get_all_nxt_ff_df_from_ff_dataframe(stops_near_ff_df, ff_dataframe_visible,
    ...
    all_nxt_ff_df = stops_near_ff_df[['cur_ff_index', 'stop_point_index',
                                      'stop_cum_distance', 'stop_time', 'stop_x', 'stop_y']].copy()
    all_nxt_ff_df['beginning_time'] = all_nxt_ff_df['stop_time'] - \
        stop_period_duration
    # can be 'test', 'control', or 'neither
    all_nxt_ff_df['data_category_by_vis'] = 'test'

    # Set 'nxt_ff_index' to the index of the caught ff after stop
    all_nxt_ff_df['nxt_ff_index'] = all_nxt_ff_df['cur_ff_index'] + 1
    all_nxt_ff_df = all_nxt_ff_df[all_nxt_ff_df['nxt_ff_index'] < len(
        ff_caught_T_new)].copy()
    all_nxt_ff_df['nxt_ff_caught_time'] = ff_caught_T_new[all_nxt_ff_df['nxt_ff_index'].values]

    # get next_stop_time and next_stop_point_index based on nxt ff
    closest_stop_to_capture_df2 = closest_stop_to_capture_df[[
    ...
    all_nxt_ff_df['cur_ff_capture_time'] = ff_caught_T_new[all_nxt_ff_df['cur_ff_index'].values]
    original_length = len(all_nxt_ff_df)
    all_nxt_ff_df = all_nxt_ff_df[all_nxt_ff_df['next_stop_time'] - all_nxt_ff_df['stop_time'] >=
                                  min_time_between_cur_and_nxt_ff_caught_time].copy()
    print(f'{original_length - len(all_nxt_ff_df)} rows out of {original_length} rows were removed from all_nxt_ff_df because the time between stop and next stop is less than {min_time_between_cur_and_nxt_ff_caught_time} seconds')

    all_nxt_ff_df[['ff_x', 'ff_y']
                  ] = ff_real_position_sorted[all_nxt_ff_df['nxt_ff_index'].values]

    all_nxt_ff_df['nxt_ff_distance_to_next_stop'] = np.linalg.norm(all_nxt_ff_df[['ff_x', 'ff_y']].values -
                                                                   monkey_information.loc[all_nxt_ff_df['next_stop_point_index'], ['monkey_x', 'monkey_y']].values, axis=1)

    # Calculate the distance from the cur ff to the nxt ff
    all_nxt_ff_df['d_from_cur_ff_to_nxt_ff'] = np.linalg.norm(
    ...
    all_nxt_ff_df['nxt_ff_last_seen_rel_time_bbas'] = all_nxt_ff_df['NXT_time_ff_last_seen_bbas'] - \
    ...
    all_nxt_ff_df.loc[(all_nxt_ff_df['nxt_ff_last_seen_rel_time_bbas'].isnull()) &
                      (all_nxt_ff_df['data_category_by_vis'] != 'neither'), 'data_category_by_vis'] = 'control'

    # Preserve necessary columns
    columns_to_preserve = ['nxt_ff_index', 'cur_ff_index', 'stop_point_index', 'data_category_by_vis', 'nxt_ff_caught_time',
                           'next_stop_point_index', 'next_stop_time', 'nxt_ff_distance_to_next_stop',
                           'd_from_cur_ff_to_nxt_ff', 'nxt_ff_cluster_last_seen_rel_time_bbas',
    ...
    all_nxt_ff_df = all_nxt_ff_df[columns_to_preserve].copy()

    all_nxt_ff_df['nxt_ff_index'] = all_nxt_ff_df['nxt_ff_index'].astype(int)
...
def _add_stop_or_nxt_ff_first_seen_and_last_seen_info_bbas(df, ff_dataframe_visible, monkey_information,
    ...
    nxt_ff_first_and_last_seen_info = get_first_seen_and_last_seen_info_for_ff_in_time_windows(df[ff_index_column].values, df['stop_point_index'].values, all_start_time,
...
def add_nxt_ff_first_seen_and_last_seen_info_bsans(all_nxt_ff_df, ff_dataframe_visible, monkey_information):
    ...
    nxt_ff_first_and_last_seen_info = get_first_seen_and_last_seen_info_for_ff_in_time_windows(all_nxt_ff_df['nxt_ff_index'].values, all_nxt_ff_df['stop_point_index'].values, all_nxt_ff_df['stop_time'].values,
...
def get_nxt_ff_last_seen_rel_time(all_nxt_ff_df, ff_dataframe, stop_period_duration=2):
    # See if nxt ff was visible before stop; if they are not, they will be assigned control rather than test
    all_ff_index = all_nxt_ff_df['nxt_ff_index'].values
    all_point_index = all_nxt_ff_df['stop_point_index'].values
    all_start_time = all_nxt_ff_df['stop_time'].values - stop_period_duration
    all_nxt_ff_df['beginning_time'] = all_start_time
    all_end_time = all_nxt_ff_df['stop_time'].values
    ff_dataframe_visible = ff_dataframe[ff_dataframe['visible'] == 1].copy()
    nxt_ff_last_seen_info = find_info_when_ff_was_first_or_last_seen_between_start_time_and_end_time(all_ff_index, all_point_index, all_start_time,
...
def get_first_seen_and_last_seen_info_for_ff_in_time_windows(all_ff_index, all_stop_point_index, all_start_time, all_end_time, ff_dataframe_visible, monkey_information, verbose=True, drop_na=False):
    ff_last_seen_info = find_info_when_ff_was_first_or_last_seen_between_start_time_and_end_time(
        all_ff_index, all_stop_point_index, all_start_time, all_end_time, ff_dataframe_visible, first_or_last='last')
    ff_first_seen_info = find_info_when_ff_was_first_or_last_seen_between_start_time_and_end_time(
        all_ff_index, all_stop_point_index, all_start_time, all_end_time, ff_dataframe_visible, first_or_last='first')

    ff_info = pd.DataFrame(
        {'ff_index': all_ff_index, 'stop_point_index': all_stop_point_index})
    ...
    if drop_na:
        if ff_info.isnull().values.any():
            ...
            if num_null_rows > 0:
                print(
                    f'Warning: There are {num_null_rows} rows out of {len(ff_info)} rows in ff_info that have null values because they were not visible in the stop period. They will be dropped')
                ff_info.dropna(axis=0, inplace=True)

    point_index_ff_first_seen = ff_info.loc[~ff_info['point_index_ff_first_seen'].isnull(
    ), 'point_index_ff_first_seen']
    ff_info.loc[~ff_info['point_index_ff_first_seen'].isnull(
    ), 'point_index_ff_first_seen'] = point_index_ff_first_seen.astype(int)
    point_index_ff_last_seen = ff_info.loc[~ff_info['point_index_ff_last_seen'].isnull(
    ), 'point_index_ff_last_seen']
    ff_info.loc[~ff_info['point_index_ff_last_seen'].isnull(
    ), 'point_index_ff_last_seen'] = point_index_ff_last_seen.astype(int)

    ff_info.loc[~ff_info['point_index_ff_first_seen'].isnull(
    ), 'time_ff_first_seen'] = monkey_information.loc[point_index_ff_first_seen.values, 'time'].values
    ff_info.loc[~ff_info['point_index_ff_last_seen'].isnull(
    ), 'time_ff_last_seen'] = monkey_information.loc[point_index_ff_last_seen.values, 'time'].values
