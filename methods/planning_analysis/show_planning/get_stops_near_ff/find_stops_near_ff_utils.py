...
def find_captured_ff_info_for_making_stops_near_ff_df(monkey_information, ff_dataframe_visible, closest_stop_to_capture_df, stop_period_duration=2, max_diff_between_caught_time_and_stop_time=0.2):
    ...
    captured_ff_info = nxt_ff_utils.get_all_captured_ff_first_seen_and_last_seen_info(closest_stop_to_capture_df, stop_period_duration,
    ...
    captured_ff_info = drop_rows_in_where_stop_time_and_capture_time_is_too_far_apart(
        captured_ff_info, closest_stop_to_capture_df, max_diff_between_caught_time_and_stop_time=max_diff_between_caught_time_and_stop_time)
    ...
    crossing_boundary_time = monkey_information.loc[
        monkey_information['crossing_boundary'] == 1, 'time'].values
    ...
    captured_ff_info = captured_ff_info[captured_ff_info['stop_point_index'].isin(
        selected_point_index)].copy()
...
def drop_rows_in_where_stop_time_and_capture_time_is_too_far_apart(captured_ff_info, closest_stop_to_capture_df, max_diff_between_caught_time_and_stop_time=0.2):
    closest_stop_to_capture_df = closest_stop_to_capture_df.rename(
        columns={'cur_ff_index': 'ff_index'})
    captured_ff_info = captured_ff_info.merge(closest_stop_to_capture_df[[
                                              'ff_index', 'caught_time', 'diff_from_caught_time']], on='ff_index', how='left')
    # if there's any NA, raise error
    if len(captured_ff_info[captured_ff_info['diff_from_caught_time'].isna()]) > 0:
        raise ValueError(
            'There are NA values in diff_from_caught_time. This should not happen.')
    if captured_ff_info['diff_from_caught_time'].abs().max() >= max_diff_between_caught_time_and_stop_time:
        ...
        num_rows_exceeding = len(captured_ff_info[captured_ff_info['diff_from_caught_time'].abs(
        ...
        percentage_of_rows_where_diff_is_positive = len(
            closest_stop_to_capture_df[closest_stop_to_capture_df['diff_from_caught_time'] > 0]) / len(closest_stop_to_capture_df)
        ...
        captured_ff_info = captured_ff_info[captured_ff_info['diff_from_caught_time'].abs(
        ) < max_diff_between_caught_time_and_stop_time].copy()
    return captured_ff_info
...
def make_shared_stops_near_ff_df(monkey_information, ff_dataframe_visible,
    ...
    shared_stops_near_ff_df = find_captured_ff_info_for_making_stops_near_ff_df(
        monkey_information, ff_dataframe_visible, closest_stop_to_capture_df, stop_period_duration=stop_period_duration)

    shared_stops_near_ff_df[['stop_x', 'stop_y', 'monkey_angle', 'stop_time', 'stop_cum_distance']
                            ] = monkey_information.loc[shared_stops_near_ff_df['stop_point_index'], ['monkey_x', 'monkey_y', 'monkey_angle', 'time', 'cum_distance']].values

    shared_stops_near_ff_df = nxt_ff_utils.rename_first_and_last_seen_info_columns(
        shared_stops_near_ff_df, prefix='CUR_')

    shared_stops_near_ff_df.rename(columns={'ff_index': 'cur_ff_index',
                                            'monkey_angle': 'stop_monkey_angle',
                                            }, inplace=True)
    shared_stops_near_ff_df['cur_ff_capture_time'] = ff_caught_T_new[shared_stops_near_ff_df['cur_ff_index'].values]

    # add nxt ff info
    all_nxt_ff_df = nxt_ff_utils.get_all_nxt_ff_df_from_ff_dataframe(shared_stops_near_ff_df, ff_dataframe_visible, closest_stop_to_capture_df, ff_real_position_sorted, ff_caught_T_new, ff_life_sorted, monkey_information,
    ...
    shared_stops_near_ff_df = shared_stops_near_ff_df.merge(all_nxt_ff_df.drop(
        columns=['stop_point_index']), on='cur_ff_index', how='inner')
    print(f'{original_length - len(shared_stops_near_ff_df)} rows out of {original_length} rows in shared_stops_near_ff_df have been removed because the nxt_ff was not found from ff_dataframe.')
    shared_stops_near_ff_df['next_stop_cum_distance'] = monkey_information.loc[
        shared_stops_near_ff_df['next_stop_point_index'], 'cum_distance'].values
    shared_stops_near_ff_df['cum_distance_between_two_stops'] = shared_stops_near_ff_df['next_stop_cum_distance'] - \
    ...
    shared_stops_near_ff_df, all_nxt_ff_df = process_instances_where_monkey_is_too_close_to_edge(
        shared_stops_near_ff_df, all_nxt_ff_df, monkey_information, remove_cases_where_monkey_too_close_to_edge=remove_cases_where_monkey_too_close_to_edge)
    shared_stops_near_ff_df = add_monkey_info_before_stop(
        monkey_information, shared_stops_near_ff_df)
    shared_stops_near_ff_df['cur_ff_x'], shared_stops_near_ff_df[
        'cur_ff_y'] = ff_real_position_sorted[shared_stops_near_ff_df['cur_ff_index'].values].T
    shared_stops_near_ff_df['d_from_cur_ff_to_stop'] = np.linalg.norm(shared_stops_near_ff_df[[
                                                                      'stop_x', 'stop_y']].values - shared_stops_near_ff_df[['cur_ff_x', 'cur_ff_y']].values, axis=1)

    shared_stops_near_ff_df = nxt_ff_utils.add_if_nxt_ff_and_nxt_ff_cluster_flash_bbas(shared_stops_near_ff_df, ff_real_position_sorted,
                                                                                       ff_flash_sorted, ff_life_sorted, stop_period_duration=stop_period_duration)

    shared_stops_near_ff_df = nxt_ff_utils.add_if_nxt_ff_and_nxt_ff_cluster_flash_bsans(shared_stops_near_ff_df, ff_real_position_sorted,
                                                                                        ff_flash_sorted, ff_life_sorted)
    ...
    shared_stops_near_ff_df = shared_stops_near_ff_df[shared_stops_near_ff_df['stop_time'] -
                                                      shared_stops_near_ff_df['CUR_time_ff_first_seen_bbas'] >= min_time_between_cur_ff_first_seen_time_and_stop].copy().reset_index(drop=True)
    print(f'{len_before - len(shared_stops_near_ff_df)} rows out of {len_before} rows in shared_stops_near_ff_df have been removed because the time between stop_time and CUR_time_ff_first_seen_bbas is less than {min_time_between_cur_ff_first_seen_time_and_stop} seconds.')
...
def modify_position_of_ff_with_big_angle_for_finding_null_arc(ff_df, remove_i_o_modify_rows_with_big_ff_angles=True, verbose=True):
    ...
    ff_x_relative, ff_y_relative = show_null_trajectory.find_relative_xy_positions(
        ff_df.ff_x.values, ff_df.ff_y.values, ff_df.monkey_x.values, ff_df.monkey_y.values, ff_df.monkey_angle.values)
    if np.any(ff_y_relative < 0):
        percent_negative = np.sum(ff_y_relative < 0) / len(ff_y_relative)
        if verbose:
            print('Warning: {}% of ff_y_relative are negative. This should not happen. These rows will be eliminated'.format(
                round(percent_negative*100, 3)))
        indices_of_kept_rows = indices_of_kept_rows[np.where(ff_y_relative > 0)[
            0]]
    ...
    if len(points_with_big_angle) > 0:
        ...
        if remove_i_o_modify_rows_with_big_ff_angles:
            if verbose:
                print('Warning: {}% of ff_x_relative are bigger than ff_y_relative. In these cases, the rows will be eliminated'.format(
                    round(percent_big_angle*100, 3)))
            points_with_not_big_angles = np.where(
                np.abs(ff_x_relative) <= ff_y_relative)[0]
            indices_of_kept_rows = indices_of_kept_rows[points_with_not_big_angles]
            ff_df = original_ff_df.iloc[indices_of_kept_rows].copy()
        else:
            if verbose:
                print('Warning: {}% of ff_x_relative are bigger than ff_y_relative. In these cases, the absolute values of ff_x_relative are changed to be the same as ff_y_relative'.format(
                    round(percent_big_angle*100, 3)))
            ff_x_relative[points_with_big_angle] = ff_y_relative[points_with_big_angle] * \
                np.sign(points_with_big_angle)
            ...
            ff_df['ff_angle_boundary'] = specific_utils.calculate_angles_to_ff_boundaries(
                angles_to_ff=ff_angle, distances_to_ff=ff_distance, ff_radius=10)

            ff_x, ff_y = show_null_trajectory.turn_relative_xy_positions_to_absolute_xy_positions(
                ff_x_relative, ff_y_relative, ff_df.monkey_x, ff_df.monkey_y, ff_df.monkey_angle)
            ff_df['ff_x'] = ff_x
            ff_df['ff_y'] = ff_y
...
def add_instances_to_polar_plot(axes, stops_near_ff_df, nxt_ff_df2, monkey_information, max_instances, color='green',
    ...
    traj_df, stop_point_df, next_stop_point_df = _get_important_df_for_polar_plot(
        stops_near_ff_df, nxt_ff_df2, monkey_information, max_instances, start, end)
...
def add_instances_to_plotly_polar_plot(fig, stops_near_ff_df, nxt_ff_df2, monkey_information, max_instances, color='green', point_color='blue',
...
def _get_important_df_for_polar_plot(stops_near_ff_df, nxt_ff_df2, monkey_information, max_instances, start, end):
    ...
    for index, row in stops_near_ff_df.iterrows():
        ...
        monkey_sub = _get_monkey_sub_for_polar_plot(
            monkey_information, row, nxt_ff_df2, start, end)
        traj_df = pd.concat(
