...
class StopsNearFFBasedOnRef(plot_stops_near_ff_class._PlotStopsNearFF):
        ...
        if exists_ok & os.path.exists(df_path):
            ...
            if not only_try_retrieving:
                ...
                raise FileNotFoundError(
                    f'{df_name} is not in the folder: {folder_path}')

        if merge_diff_in_curv_df_to_heading_info:
            if hasattr(self, 'heading_info_df'):
                self.heading_info_df = self.heading_info_df.merge(
                    self.diff_in_curv_df, on='stop_point_index', how='left')

        return self.diff_in_curv_df

    def make_diff_in_curv_df(self, curv_traj_window_before_stop=[-50, 0]):
        self.nxt_ff_info_for_null_arc = show_planning_utils.make_nxt_ff_info_for_null_arc(
            self.nxt_ff_df_modified, self.cur_ff_final_df, self.heading_info_df)
        self.nxt_ff_info_for_monkey = show_planning_utils.make_nxt_ff_info_for_monkey(self.nxt_ff_df_modified, self.heading_info_df, self.monkey_information,
                                                                                      self.ff_real_position_sorted, self.ff_caught_T_new,
                                                                                      curv_traj_window_before_stop=curv_traj_window_before_stop)
        self.diff_in_curv_df = show_planning_utils.make_diff_in_curv_df(
            self.nxt_ff_info_for_monkey, self.nxt_ff_info_for_null_arc)
        self.diff_in_curv_df = show_planning_utils.furnish_diff_in_curv_df(
            self.diff_in_curv_df)
        return self.diff_in_curv_df

    def _init_empty_vars(self):
        self.slope = None
        self.ff_dataframe = None
        self.nxt_ff_df2_test = None
        self.nxt_ff_df2_ctrl = None
    ...
    def find_nxt_ff_df_2_and_cur_ff_df_2(self, ref_point_value, ref_point_mode
    ...
    def find_relative_curvature(self):
        ...
        self.traj_curv_counted, self.nxt_curv_counted = find_stops_near_ff_utils.find_relative_curvature(
            self.nxt_ff_counted_df, self.cur_ff_counted_df, self.curv_of_traj_counted, self.overall_params['use_curvature_to_ff_center'])
    ...
    def find_relationships_from_info(self, normalize=False,
        ...
        ax_for_corr = find_stops_near_ff_utils.plot_relationship(
    ...
    def _find_relative_heading_info(self):
        nxt_ff_df = self.nxt_ff_final_df.copy()

        self.d_heading_of_traj = nxt_ff_df['d_heading_of_traj'].values
        self.d_heading_cur = self.cur_ff_final_df['d_heading_of_arc'].values
        self.d_heading_nxt = nxt_ff_df['d_heading_of_arc'].values

        self.d_heading_of_traj = find_stops_near_ff_utils.confine_angle_to_within_one_pie(
            self.d_heading_of_traj)
        self.d_heading_cur = find_stops_near_ff_utils.confine_angle_to_within_one_pie(
            self.d_heading_cur)
        self.d_heading_nxt = find_stops_near_ff_utils.confine_angle_to_within_one_pie(
            self.d_heading_nxt)
        ...
        self.rel_heading_traj = find_stops_near_ff_utils.confine_angle_to_within_one_pie(
            self.rel_heading_traj)
        self.rel_heading_alt = find_stops_near_ff_utils.confine_angle_to_within_one_pie(
    ...
    def _find_mheading_before_stop(self):
        ...
        self.mheading_before_stop_dict = monkey_heading_functions.find_mheading_in_xy(
    ...
    def _find_nxt_ff_df_2_and_cur_ff_df_2_based_on_specific_point_index(self, all_point_index=None):
        print('nxt_ff_df2 and cur_ff_df2 are based on specific point_index')
    ...
    def _make_heading_info_df(self, test_or_control='test', heading_info_df_exists_ok=True, diff_in_curv_df_exists_ok=True, save_data=True,
        ...
        try:
            ...
            if heading_info_df_exists_ok:
                print(
                    f'Failed to retrieve heading_info_df because of {e}; will make new heading_info_df')
            ...
            self.heading_info_df['rank_by_angle_to_nxt_ff'] = self.heading_info_df['angle_from_m_before_stop_to_nxt_ff'].rank(
                method='first')
            ...
            if 'nxt_ff_angle_at_ref' not in self.heading_info_df.columns:
                ...
                self.heading_info_df = self.heading_info_df.merge(
                    self.both_ff_at_ref_df, on='stop_point_index', how='left')
    ...
    def _deal_with_rows_with_big_ff_angles(self, remove_i_o_modify_rows_with_big_ff_angles=True, verbose=False, delete_the_same_rows=True):
        ...
        self.nxt_ff_df_modified, indices_of_kept_rows = find_stops_near_ff_utils.modify_position_of_ff_with_big_angle_for_finding_null_arc(
            self.nxt_ff_df2, remove_i_o_modify_rows_with_big_ff_angles=remove_i_o_modify_rows_with_big_ff_angles, verbose=verbose)
        self.cur_ff_df_modified = self.cur_ff_df2.copy()
        if delete_the_same_rows:
            self.cur_ff_df_modified = self.cur_ff_df_modified.iloc[indices_of_kept_rows].copy(
            )
        self.cur_ff_df_modified, indices_of_kept_rows = find_stops_near_ff_utils.modify_position_of_ff_with_big_angle_for_finding_null_arc(
    ...
    def _add_d_heading_info(self):
        ...
        if self.overall_params['use_curvature_to_ff_center']:
            self.nxt_ff_final_df['d_heading_of_arc'] = self.nxt_ff_final_df['d_heading_to_center']
            self.cur_ff_final_df['d_heading_of_arc'] = self.cur_ff_final_df['d_heading_to_center']
    ...
    def _take_out_info_counted(self):
        ...
        self.cur_ff_counted_df = self.cur_ff_final_df.copy().reset_index(drop=True)
        self.nxt_ff_counted_df = self.nxt_ff_final_df.copy().reset_index(drop=True)

        self.ref_point_index_counted = self.nxt_ff_final_df.point_index.values.copy()
        self.stop_point_index_counted = self.nxt_ff_final_df.stop_point_index.values.copy()
    ...
    def _eliminate_outliers_in_cur_ff_curv(self):
        ...
        self.outlier_positions, self.non_outlier_positions = find_stops_near_ff_utils.find_outliers_in_a_column(
        ...
        self.nxt_ff_counted_df = self.nxt_ff_final_df.iloc[self.non_outlier_positions].copy(
        ).reset_index(drop=True)
        self.cur_ff_counted_df = self.cur_ff_final_df.iloc[self.non_outlier_positions].copy(
        ).reset_index(drop=True)
        self.stops_near_ff_df_counted = self.stops_near_ff_df.set_index(
            'stop_point_index').loc[self.stop_point_index_counted].reset_index()

    def _prepare_data_to_compare_test_and_control(self):
        if self.nxt_ff_df2_test is None:
            self.make_stops_near_ff_df_test(exists_ok=True)
            self._find_nxt_ff_df_2_and_cur_ff_df_2_based_on_specific_point_index(
                all_point_index=self.cur_ff_df['point_index_ff_first_seen'].values)
            self.nxt_ff_df2_test = self.nxt_ff_df2.copy()
        if self.nxt_ff_df2_ctrl is None:
            self.make_stops_near_ff_df_ctrl(exists_ok=True)
            self._find_nxt_ff_df_2_and_cur_ff_df_2_based_on_specific_point_index(
                all_point_index=self.cur_ff_df['point_index_ff_first_seen'].values)
            self.nxt_ff_df2_ctrl = self.nxt_ff_df2.copy()
    ...
    def _make_info_based_on_monkey_angle(self):
        self.info_based_on_monkey_angle_before_stop = find_stops_near_ff_utils.calculate_info_based_on_monkey_angles(
            self.stops_near_ff_df, self.stops_near_ff_df.monkey_angle_before_stop.values)
        self.info_based_on_monkey_angle_when_nxt_ff_last_seen = find_stops_near_ff_utils.calculate_info_based_on_monkey_angles(
            self.stops_near_ff_df, self.nxt_ff_df.monkey_angle_ff_last_seen.values)
