...
def _extend_arc_length_by_increasing_arc_ending_angle(null_arc_info):
    abs_delta_angle = abs(null_arc_info['arc_ending_angle'] - null_arc_info['arc_starting_angle'])
    ...
    angle_to_add = angle_to_add * np.sign(null_arc_info['arc_ending_angle'] - null_arc_info['arc_starting_angle'])
...
def get_points_on_each_arc(null_arc_info, num_points_on_each_arc=2000, extend_arc_angle=False):
    ...
    if extend_arc_angle:
        null_arc_info = _extend_arc_length_by_increasing_arc_ending_angle(null_arc_info)

    angle_array = np.linspace(null_arc_info['arc_starting_angle'].values, null_arc_info['arc_ending_angle'].values, num_points_on_each_arc).T.reshape(-1)
    ...
    arc_df = pd.DataFrame({
        'stop_ff_index': repeated_values['arc_ff_index'],
    ...
    arc_df['x'] = arc_df['center_x'] + arc_df['radius'] * np.cos(arc_df['angle'])
    arc_df['y'] = arc_df['center_y'] + arc_df['radius'] * np.sin(arc_df['angle'])

    # Calculate distance to firefly
    arc_df['distance_to_ff'] = np.sqrt((arc_df['x'] - arc_df['arc_ff_x'])**2 + (arc_df['y'] - arc_df['arc_ff_y'])**2)

    arc_df['delta_angle_from_starting_angle'] = np.abs(arc_df['angle'] - arc_df['arc_starting_angle'])
    return arc_df
...
def get_optimal_arc_landing_points_closest_to_stop(null_arc_info, stops_near_ff_df, reward_boundary_radius=25):
    if len(null_arc_info) != len(stops_near_ff_df):
        raise ValueError('The number of rows in null_arc_info and stops_near_ff_df do not match.')
    
    null_arc_info = null_arc_info.merge(stops_near_ff_df[['stop_ff_index', 'stop_ff_x', 'stop_ff_y']], left_on='arc_ff_index', right_on='stop_ff_index', how='left')
    null_arc_info.rename(columns={'stop_ff_x': 'arc_ff_x', 'stop_ff_y': 'arc_ff_y'}, inplace=True)
    ...
    arc_df = arc_df.merge(stops_near_ff_df[['stop_ff_index', 'stop_x', 'stop_y']], on='stop_ff_index', how='left')
    # Calculate distance to stop
    arc_df['distance_to_stop'] = np.sqrt((arc_df['x'] - arc_df['stop_x'])**2 + (arc_df['y'] - arc_df['stop_y'])**2)
    
    # Find the arc points closest to each stop
    index_of_arc_rows_closest_to_stop = arc_df.groupby('stop_ff_index')['distance_to_stop'].idxmin()
    arc_rows_closest_to_stop = arc_df.loc[index_of_arc_rows_closest_to_stop].reset_index(drop=True)
...
def get_optimal_arc_landing_points_when_first_reaching_visible_boundary(null_arc_info,
    ...
    for i in range(2): # do the following twice, just in case
        ...
        greater_than_pi = null_arc_info['arc_ending_angle'] - null_arc_info['arc_starting_angle'] > math.pi
        ...
        less_than_minus_pi = null_arc_info['arc_starting_angle'] - null_arc_info['arc_ending_angle'] > math.pi
        null_arc_info.loc[less_than_minus_pi, 'arc_ending_angle'] += 2 * math.pi


    arc_df_original = get_points_on_each_arc(null_arc_info, extend_arc_angle=True)

    # Find the arc points that first reach the visible boundary of the firefly
    arc_df = arc_df_original[arc_df_original['distance_to_ff'] < visible_boundary_radius + 0.1].copy()
    arc_df = arc_df.sort_values(by=['stop_ff_index', 'point_id_on_arc']).reset_index(drop=True)
    arc_rows_to_first_reach_boundary = arc_df.groupby('stop_ff_index').first().reset_index(drop=False)

    too_big_angle_rows = arc_rows_to_first_reach_boundary[arc_rows_to_first_reach_boundary['delta_angle_from_starting_angle'] > math.pi/2].copy()
    if len(too_big_angle_rows) > 0:
        ...
        arc_df_sub = arc_df_original[arc_df_original['stop_ff_index'].isin(too_big_angle_rows['stop_ff_index'])].copy()
        arc_df_sub = arc_df_sub[arc_df_sub['distance_to_ff'] <= reward_boundary_radius].copy()
        arc_df_sub = arc_df_sub[arc_df_sub['delta_angle_from_starting_angle'] <= math.pi/2].copy()
        arc_df_sub = arc_df_sub.sort_values(by=['stop_ff_index', 'distance_to_ff'], ascending=[True, True]).reset_index(drop=True)
        new_too_big_arc_rows = arc_df_sub.groupby('stop_ff_index').first().reset_index(drop=False)
        if len(new_too_big_arc_rows) != len(too_big_angle_rows):
            raise ValueError('The number of rows in new_too_big_arc_rows and too_big_angle_rows do not match.')
        # else, let arc_rows_to_first_reach_boundary drop the old rows and concatenate the new rows, and then sort by stop_ff_index
        arc_rows_to_first_reach_boundary = pd.concat([arc_rows_to_first_reach_boundary[~arc_rows_to_first_reach_boundary['stop_ff_index'].isin(too_big_angle_rows['stop_ff_index'])], new_too_big_arc_rows], axis=0)
        arc_rows_to_first_reach_boundary = arc_rows_to_first_reach_boundary.sort_values(by='stop_ff_index').reset_index(drop=True)
    ...
    if len(arc_rows_to_first_reach_boundary) != len(null_arc_info):
        ...
        arc_rows_to_first_reach_boundary = _get_missed_arc_info(null_arc_info, arc_df_original, arc_rows_to_first_reach_boundary, reward_boundary_radius=reward_boundary_radius)
...
def _get_missed_arc_info2(null_arc_info, arc_rows_to_first_reach_boundary, reward_boundary_radius=25):
    ...
    null_arc_info_sub = null_arc_info[~null_arc_info['arc_ff_index'].isin(arc_rows_to_first_reach_boundary['stop_ff_index'].values)].copy()
    arc_df_extended = get_points_on_each_arc(null_arc_info_sub, extend_arc_angle=True)
    missed_arc = arc_df_extended[(arc_df_extended['distance_to_ff'] <= reward_boundary_radius)].copy()
    arc_rows_to_first_reach_boundary = _add_info_to_arc_rows_to_first_reach_boundary(arc_rows_to_first_reach_boundary, missed_arc, null_arc_info)
    return arc_rows_to_first_reach_boundary
...
def _get_missed_arc_info(null_arc_info, arc_df_original, arc_rows_to_first_reach_boundary, reward_boundary_radius=25):
    ...
    missed_arc = arc_df_original[~arc_df_original['stop_ff_index'].isin(arc_rows_to_first_reach_boundary['stop_ff_index'].values)].copy()
...
def _add_info_to_arc_rows_to_first_reach_boundary(arc_rows_to_first_reach_boundary, missed_arc, null_arc_info):
    ...
    missed_arc.sort_values(by=['stop_ff_index', 'distance_to_ff'], ascending=[True, True], inplace=True)
    missed_arc = missed_arc.groupby('stop_ff_index').first().reset_index(drop=False)
    arc_rows_to_first_reach_boundary = pd.concat([arc_rows_to_first_reach_boundary, missed_arc], axis=0)
    arc_rows_to_first_reach_boundary.sort_values(by='stop_ff_index', inplace=True)
    if len(arc_rows_to_first_reach_boundary) != len(null_arc_info):
        raise ValueError('The number of rows in arc_rows_to_first_reach_boundary and stops_near_ff_df do not match.')
    return arc_rows_to_first_reach_boundary
...
def make_new_ff_at_monkey_xy_if_within_1_cm(new_ff_x, new_ff_y, monkey_x, monkey_y):
    ...
    if np.where(distance < 1)[0].size > 0:
        print(f'Number of new ff xy within 1 cm of monkey: {np.where(distance < 1)[0].size} out of {len(distance)}. Setting them to monkey position.')
...
def make_alt_and_stop_ff_df(alt_ff_final_df, stop_ff_final_df):
    ...
    # Create a copy of the shared columns from alt_ff_final_df and rename them
    alt_and_stop_ff_df = pd.concat([alt_ff_final_df[shared_columns], stop_ff_final_df[shared_columns]], axis=0)
    # drop duplicate rows
    alt_and_stop_ff_df = alt_and_stop_ff_df.drop_duplicates(subset=['stop_point_index']).reset_index(drop=True)

    alt_and_stop_ff_df.rename(columns={'point_index': 'ref_point_index',
    ...
    # Create copies of the relevant columns from alt_ff_final_df2 and stop_ff_final_df2 and rename them
    alt_ff_final_df2 = alt_ff_final_df[relevant_columns].copy()
    alt_ff_final_df2.columns = ['alt_'+col for col in alt_ff_final_df2.columns.tolist()]
    alt_ff_final_df2['stop_point_index'] = alt_ff_final_df['stop_point_index']

    stop_ff_final_df2 = stop_ff_final_df[relevant_columns].copy()
    stop_ff_final_df2.columns = ['stop_'+col for col in stop_ff_final_df2.columns.tolist()]
    stop_ff_final_df2['stop_point_index'] = stop_ff_final_df['stop_point_index']

    # Merge alt_and_stop_ff_df, alt_ff_final_df2, and stop_ff_final_df2
    alt_and_stop_ff_df = alt_and_stop_ff_df.merge(alt_ff_final_df2, how='left', on='stop_point_index')
    alt_and_stop_ff_df = alt_and_stop_ff_df.merge(stop_ff_final_df2, how='left', on='stop_point_index')

    # Calculate landing headings
    if 'stop_d_heading_of_arc' in alt_and_stop_ff_df.columns:
        alt_and_stop_ff_df['stop_arc_end_heading'] = alt_and_stop_ff_df['ref_monkey_angle'] + alt_and_stop_ff_df['stop_d_heading_of_arc']
        alt_and_stop_ff_df['alt_arc_end_heading'] = alt_and_stop_ff_df['ref_monkey_angle'] + alt_and_stop_ff_df['alt_d_heading_of_arc']
...
def make_heading_info_df(alt_and_stop_ff_df, stops_near_ff_df, monkey_information, ff_real_position_sorted):
    ...
    heading_info_df = stops_near_ff_df[['stop_point_index', 'stop_x', 'stop_y', 'stop_time', 
                                        'stop_ff_index', 'stop_ff_x', 'stop_ff_y', 'stop_ff_cluster_50_size',
                                        'point_index_before_stop',  'monkey_angle_before_stop',
                                        'next_stop_point_index', 'next_stop_time', 'cum_distance_between_two_stops',
                                        'curv_range', 'curv_iqr', 'alt_ff_index', 'alt_ff_x', 'alt_ff_y',
                                        'ALT_time_ff_last_seen_bbas', 
                                        ...
                                        'alt_ff_cluster_last_flash_time_bsans']].copy()

    # Add monkey's position before stop from monkey_information
    heading_info_df['mx_before_stop'], heading_info_df['my_before_stop'] = monkey_information.loc[heading_info_df['point_index_before_stop'], ['monkey_x', 'monkey_y']].values.T

    # Add alternative ff position from ff_real_position_sorted
    heading_info_df[['alt_ff_x', 'alt_ff_y']] = ff_real_position_sorted[heading_info_df['alt_ff_index']]
...
def get_alt_traj_and_alt_stop(heading_info_df):
    heading_info_df = heading_info_df.copy()
    # print the number of rows in heading_info_df that contain NaN values
    print(f'Number of rows with NaN values in heading_info_df: {heading_info_df.isnull().any(axis=1).sum()} out of {heading_info_df.shape[0]} rows, but they are not dropped. The columns with NaN values are:')
    ...
    alt_traj = heading_info_df['angle_from_m_before_stop_to_alt_ff'].values.reshape(-1)
    alt_stop = heading_info_df['angle_from_stop_ff_landing_to_alt_ff'].values.reshape(-1)

    #heading_info_df_no_na = heading_info_df.copy()
    return alt_traj, alt_stop, heading_info_df


def conduct_linear_regression(alt_traj, alt_stop, fit_intercept=True):
    # calculate r and slope of alt_traj and alt_stop
    if fit_intercept:
        X = sm.add_constant(alt_traj)
        model = sm.OLS(alt_stop, X)
        results = model.fit()
        slope = results.params[1]
        p_value = results.pvalues[1]
        intercept = results.params[0]
    else:
        model = sm.OLS(alt_stop, alt_traj)
        results = model.fit()
...
def omit_outliers_from_linear_regression_results(alt_traj, alt_stop, results):
    # Calculate residuals
    ...
    # Remove outliers
    alt_traj_no_outliers = alt_traj[~outliers]
    alt_stop_no_outliers = alt_stop[~outliers]
    return alt_traj_no_outliers, alt_stop_no_outliers

def conduct_linear_regression_to_show_planning(alt_traj, alt_stop, use_abs_values=False, fit_intercept=True, omit_outliers=False, q13_only=False, show_plot=True,
                                               hue=None):
    if q13_only:
        # retain only the values in the first and third quadrants
        same_sign = np.sign(alt_traj) == np.sign(alt_stop)
        alt_traj = alt_traj[same_sign]
        alt_stop = alt_stop[same_sign]
    
    if use_abs_values:
        alt_traj = np.abs(alt_traj)
        alt_stop = np.abs(alt_stop)
    
    
    # calculate r and slope of alt_traj and alt_stop
    slope, intercept, r_value, p_value, results = conduct_linear_regression(alt_traj, alt_stop, fit_intercept=fit_intercept)
    sample_size = len(alt_stop)

    
    if omit_outliers:     
        alt_traj, alt_stop = omit_outliers_from_linear_regression_results(alt_traj, alt_stop, results)        
        slope, intercept, r_value, p_value, results = conduct_linear_regression(alt_traj, alt_stop, fit_intercept=fit_intercept)
    ...
    if show_plot:
        plot_stops_near_ff_utils.plot_alt_traj_vs_alt_stop(alt_traj, alt_stop, hue, title, slope, intercept)
...
def make_diff_and_ratio_stat_df(test_df, ctrl_df):
    ...
    test_stat = test_df[['diff', 'ratio', 'diff_in_abs',
                         'diff_in_abs_d_curv']].describe()
    test_stat.columns = ['test_' + col for col in test_stat.columns]
    
    ctrl_stat = ctrl_df[['diff', 'ratio', 'diff_in_abs',
                        'diff_in_abs_d_curv']].describe()
    ctrl_stat.columns = ['ctrl_' + col for col in ctrl_stat.columns]

    # ratio_stat = pd.concat([test_df['ratio'].describe(), ctrl_df['ratio'].describe()], axis=1)
    # ratio_stat.columns = ['test_ratio', 'ctrl_ratio']
    # diff_stat = pd.concat([test_df['diff'].describe(), ctrl_df['diff'].describe()], axis=1)
    # diff_stat.columns = ['test_diff', 'ctrl_diff']
    # diff_in_abs_stat = pd.concat([test_df['diff_in_abs'].describe(), ctrl_df['diff_in_abs'].describe()], axis=1)
    # diff_in_abs_stat.columns = ['test_diff_in_abs', 'ctrl_diff_in_abs']
...
def remove_outliers(x_var_df, y_var):
    ...
    print(f'Number of outliers dropped before train_test_split: {len(outliers) - len(non_outlier_index)} out of {len(outliers)} samples.')
    return x_var_df, y_var


def make_alt_ff_info_for_null_arc(alt_ff_df_modified, stop_ff_final_df, heading_info_df):
    # To get # curv of null arc from monkey stop to alt ff

    # use 'stop_arc_end_x', 'stop_arc_end_y', 'stop_arc_end_heading'
    # to replace 'monkey_x', 'monkey_y', 'monkey_angle'
    alt_ff_info_for_null_arc = alt_ff_df_modified.copy()
    alt_ff_info_for_null_arc['monkey_x'] = heading_info_df['stop_arc_end_x'].values
    alt_ff_info_for_null_arc['monkey_y'] = heading_info_df['stop_arc_end_y'].values
    alt_ff_info_for_null_arc['monkey_angle'] = heading_info_df['stop_arc_end_heading'].values
    ...
    # curv_of_traj will be stop null curv's optimal_curvature
    alt_ff_info_for_null_arc['curv_of_traj'] = stop_ff_final_df['optimal_curvature'].values
...
def make_alt_ff_info_for_monkey(alt_ff_df_modified, heading_info_df, monkey_information, ff_real_position_sorted, ff_caught_T_new,
    ...
    return alt_ff_info_for_monkey


def make_diff_in_curv_df(alt_ff_info_for_monkey, alt_ff_info_for_null_arc):
    """
    ...
    null_arc_outside_boundary = alt_ff_info_for_null_arc[
        (alt_ff_info_for_null_arc['ff_angle_boundary'] < angle_boundary[0]) | 
        (alt_ff_info_for_null_arc['ff_angle_boundary'] > angle_boundary[1])
    ]

    monkey_outside_boundary = alt_ff_info_for_monkey[
        (alt_ff_info_for_monkey['ff_angle_boundary'] < angle_boundary[0]) | 
        (alt_ff_info_for_monkey['ff_angle_boundary'] > angle_boundary[1])
    ]

    # Get the union of the indices of the rows outside the boundary
    union_indices = null_arc_outside_boundary.index.union(monkey_outside_boundary.index)

    # Calculate the percentage of these rows out of all rows for both DataFrames
    total_rows = len(alt_ff_info_for_null_arc) + len(alt_ff_info_for_monkey)
    percentage_outside_boundary = len(union_indices) / total_rows * 100

    # Print the percentage
    print(f"Percentage of rows outside of [-45, 45]: {percentage_outside_boundary:.2f}%")

    # Drop the union of these rows from both DataFrames
    alt_ff_info_for_null_arc_cleaned = alt_ff_info_for_null_arc.drop(null_arc_outside_boundary.index)
    alt_ff_info_for_monkey_cleaned = alt_ff_info_for_monkey.drop(monkey_outside_boundary.index)

    # Generate curvature DataFrames
    null_arc_curv_df = curvature_utils._make_curvature_df(
        alt_ff_info_for_null_arc_cleaned, 
        alt_ff_info_for_null_arc_cleaned['curv_of_traj'].values, 
        ff_radius_for_optimal_arc=15, 
        clean=False, 
        invalid_curvature_ok=False,
        include_curv_to_ff_center=False,
        optimal_arc_stop_at_visible_boundary=False, # this doesn't matter since we only care about the curvature to alt ff, not the null arc landing point inside alt ff
    )

    monkey_curv_df = curvature_utils._make_curvature_df(
        alt_ff_info_for_monkey_cleaned, 
        alt_ff_info_for_monkey_cleaned['curv_of_traj'].values, 
        ff_radius_for_optimal_arc=15,
        clean=False,
    ...
    null_arc_curv_df = null_arc_curv_df[['optimal_curvature']].reset_index(drop=True)
    null_arc_curv_df[['stop_point_index', 'curv_of_traj']] = alt_ff_info_for_null_arc_cleaned[['stop_point_index', 'curv_of_traj']].values
    null_arc_curv_df.rename(columns={'curv_of_traj': 'null_arc_curv_to_stop_ff', 
                                     'optimal_curvature': 'null_arc_curv_to_alt_ff'}, inplace=True)

    monkey_curv_df = monkey_curv_df[['optimal_curvature']].reset_index(drop=True)
    monkey_curv_df[['stop_point_index', 'curv_of_traj']] = alt_ff_info_for_monkey_cleaned[['stop_point_index', 'curv_of_traj']].values
    monkey_curv_df.rename(columns={'curv_of_traj': 'monkey_curv_to_stop_ff', 
                                   'optimal_curvature': 'monkey_curv_to_alt_ff'}, inplace=True)
    
    diff_in_curv_df = monkey_curv_df.merge(null_arc_curv_df, how='outer', on='stop_point_index')

    return diff_in_curv_df


def furnish_diff_in_curv_df(diff_in_curv_df):
    diff_in_curv_df['d_curv_null_arc'] = (180/math.pi * 100) * (diff_in_curv_df['null_arc_curv_to_alt_ff'] - diff_in_curv_df['null_arc_curv_to_stop_ff'])
    diff_in_curv_df['d_curv_monkey'] = (180/math.pi * 100) * (diff_in_curv_df['monkey_curv_to_alt_ff'] - diff_in_curv_df['monkey_curv_to_stop_ff'])
    diff_in_curv_df['abs_d_curv_null_arc'] = np.abs(diff_in_curv_df['d_curv_null_arc'])
    diff_in_curv_df['abs_d_curv_monkey'] = np.abs(diff_in_curv_df['d_curv_monkey'])   

    diff_in_curv_df['diff_in_curv_to_stop'] = diff_in_curv_df['null_arc_curv_to_stop_ff'] - diff_in_curv_df['monkey_curv_to_stop_ff']
    diff_in_curv_df['diff_in_curv_to_alt'] = diff_in_curv_df['null_arc_curv_to_alt_ff'] - diff_in_curv_df['monkey_curv_to_alt_ff']
    diff_in_curv_df['diff_in_d_curv'] = diff_in_curv_df['d_curv_null_arc'] - diff_in_curv_df['d_curv_monkey']
    diff_in_curv_df['diff_in_abs_d_curv'] = np.abs(diff_in_curv_df['d_curv_null_arc']) - np.abs(diff_in_curv_df['d_curv_monkey'])
...
def retrieve_df_based_on_ref_point(ref_point_mode, ref_point_value, test_or_control, data_folder_name, df_partial_path, monkey_name):
    ...
    df_name = find_stops_near_ff_utils.find_df_name(monkey_name, ref_point_mode, ref_point_value)
    retrieved_df = pd.read_csv(os.path.join(df_path, df_name))
    print(f'Retrieving {df_name} from {os.path.join(df_path, df_name)} succeeded')
    return retrieved_df      
