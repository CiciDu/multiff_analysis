...
all_x_vars_of_interest=['test_or_control',
                        ...
                        'whether_filter_info',
                        'if_test_alt_ff_group_appear_after_stop',
                        'keep_monkey_info',
                        'id',
                        'num_obs_ff',
                        'max_in_memory_time',
...
def make_new_df_for_plotly_comparison(df, match_rows_based_on_ref_columns_only=True):
    ...
    try:
        ...
        df.loc[df['test_or_control']=='test', 'sample_size'] = df['test_sample_size']
    except KeyError:
        pass

    if match_rows_based_on_ref_columns_only & ('ref_columns_only' in df.columns):
        new_df = _supply_info_to_ref_column_subset_to_match_more_columns_subset(df)  
    else:
        new_df = df.copy()
...
def _supply_info_to_ref_column_subset_to_match_more_columns_subset(overall_df):
    ...
    if len(columns_to_match) > 0:
        ...
        for index, row in more_sub[columns_to_match].drop_duplicates().iterrows():
            ...
            more_sub2 = more_sub[more_sub[columns_to_match].eq(row).all(axis=1)].copy()
            ...
            if more_sub2.shape[0] != ref_sub2.shape[0]:
                ...
                raise ValueError('more_sub and new_ref_sub have different number of rows')
...
def get_combinations_from_changeable_variables(df, changeable_variables):
    if len(changeable_variables) > 0:
        changeable_variables = {key: df[key].unique() for key in changeable_variables}
...
def break_up_df_to_smaller_ones(df, fixed_variable_values_to_use, changeable_variables, var_to_determine_x_offset_direction=None, 
    ...
    combinations = get_combinations_from_changeable_variables(processed_df, changeable_variables)
...
def process_lr_or_ml_df_for_visualization(lr_or_ml_df,
    ...
    all_x_vars_of_interest = [col for col in all_x_vars_of_interest if col in lr_or_ml_df.columns]
    if var_to_split is None:
        vars_to_keep = list(set(all_x_vars_of_interest + ['sample_size', y_var_column]))
    else:
        all_x_vars_of_interest = [col for col in all_x_vars_of_interest if col != var_to_split]
        vars_to_keep = list(set(all_x_vars_of_interest + ['sample_size', var_to_split, y_var_column]))

    if se_column is not None:
        lr_or_ml_df['se_upper'] = lr_or_ml_df[y_var_column] + lr_or_ml_df[se_column] * 2
        lr_or_ml_df['se_lower'] = lr_or_ml_df[y_var_column] - lr_or_ml_df[se_column] * 2
    ...
    try:
        ...
        if 'ref_columns_only' in lr_or_ml_df.columns:
            ...
            lr_or_ml_df.loc[lr_or_ml_df['ref_columns_only']==False, 'ref_columns_only'] = 'more_columns' 
        if 'cluster_for_interaction' in lr_or_ml_df.columns:                                         
            lr_or_ml_df.loc[lr_or_ml_df['cluster_for_interaction'].isnull(), 'cluster_for_interaction'] = 'none'
    except KeyError:
        pass
    ...
    if var_to_split is not None:
        ...
        if var_to_split == 'test_or_control':
            ...
            sub_df.loc[sub_df[var_to_split]=='ref_columns_only', 'y1_or_y2'] = 'y2'
        else:
            sub_df.loc[sub_df[var_to_split]==sub_df[var_to_split].unique()[1], 'y1_or_y2'] = 'y2'
...
def assign_color_to_sub_df_based_on_unique_combinations(sub_df, columns_to_find_unique_combinations_for_color):
    ...
    if columns_to_find_unique_combinations_for_color is not None:
        ...
        color_map = {comb: colors[i % len(colors)] for i, comb in enumerate(sub_df['unique_combination'].unique())}
...
def assign_line_type_to_sub_df_based_on_unique_combinations(sub_df, columns_to_find_unique_combinations_for_line):
    ...
    if len(columns_to_find_unique_combinations_for_line) > 0:
        sub_df['unique_combination_for_line'] = sub_df.apply(lambda row: '; '.join(col + '=' + str(row[col]) for col in columns_to_find_unique_combinations_for_line), axis=1)

        lines = ['dash', 'solid', 'dot', 'dashdot', 'longdash', 'longdashdot']

        line_type_map = {comb: lines[i % len(lines)] for i, comb in enumerate(sub_df['unique_combination_for_line'].unique())}

        sub_df['line_type'] = sub_df['unique_combination_for_line'].map(line_type_map)
    else:
...
def get_smaller_dfs_to_plot_coeff(df, column_to_split_grouped_bars='test_or_control', fixed_variable_values_to_use={}):
    ...
    columns_accounted_for = [column_to_split_grouped_bars] + list(fixed_variable_values_to_use.keys())
    changeable_variables = [col for col in columns_with_variations if col not in columns_accounted_for]
...
def melt_df_by_test_and_control(df, test_column='test_perc', control_column='ctrl_perc', value_name='perc'):
    df = df.copy()
    df.rename(columns={test_column: 'test', control_column: 'control'}, inplace=True)
    ...
    df2['sample_size'] = -999
    df2.loc[df2['test_or_control'] == 'test', 'sample_size'] = df2.loc[df2['test_or_control'] == 'test', 'test_sample_size']
    df2.loc[df2['test_or_control'] == 'control', 'sample_size'] = df2.loc[df2['test_or_control'] == 'control', 'ctrl_sample_size']
...
def plot_heading_in_overall_all_median_info(overall_all_median_info, title_prefix=None):
    ...
    plot_variations_utils.streamline_making_plotly_plot_to_compare_two_sets_of_data(new_overall_all_median_info, 
                                                                ...
                                                                se_column='diff_in_abs_boot_med_std',
