import sysfrom planning_analysis.show_planning.get_stops_near_ff import stops_near_ff_based_on_ref_class, find_stops_near_ff_utils
from planning_analysis.show_planning import show_planning_class, alt_ff_utils
...
def _match_combo_of_columns(df1, df2, combo_of_columns_to_match):
    combo_of_columns_to_match = [col for col in combo_of_columns_to_match if (col in df1.columns) and (col in df2.columns)]
    ...
    for index, combo in df1[combo_of_columns_to_match].drop_duplicates().iterrows():
        ...
        if df2_sub.shape[0] == 0:
            continue
        new_df1 = pd.concat([new_df1, df1[df1[combo_of_columns_to_match].eq(combo).all(axis=1)]], axis=0)
...
def make_both_players_df(monkey_df, agent_df,
    ...
    columns_differed, shared_varients = examine_and_compare_columns_between_two_df(monkey_df, agent_df, verbose=False)
    ...
    agent_df_sub = _match_combo_of_columns(monkey_df_sub, agent_df_sub, combo_of_columns_to_match)
