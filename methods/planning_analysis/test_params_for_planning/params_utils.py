...
def add_values_and_marks_to_ref_point_info(ref_point_info):
    for key, value in ref_point_info.items():
        ref_point_all_values = np.arange(value['min'], value['max'], value['step']).tolist()
...
def put_down_the_dropdown_menu_for_time_or_distance(time_or_distance):
    return html.Div(children=[
                        html.Label(['Time or Distance'], style={'font-weight': 'bold', "text-align": "center"}),
                        dcc.Dropdown(
                            id='time_or_distance',
...
def get_subset_of_combo_df(combo_df, hyperparameter_dict):
    ...
    for key, value in hyperparameter_dict.items():
        if key in combo_df.columns:
            ...
            print('Warning: key ' + key + ' in hyperparameter_dict is not in combo_df.columns')
    return combo_df
...
def generate_distribution_of_correlation_after_shuffling_d_heading(d_heading_alt, d_heading_stop, d_heading_of_traj, sample_size):
    d_heading_alt = np.array(d_heading_alt)
    all_r_values = []
    all_p_values = []
    for i in range(sample_size):
        np.random.shuffle(d_heading_alt)
        rel_heading_traj = d_heading_of_traj - d_heading_stop
        rel_heading_alt = d_heading_alt - d_heading_stop
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(rel_heading_alt, rel_heading_traj)
...
def generate_distribution_of_correlation_after_shuffling_alt_ff_curv(alt_ff_counted_df, stop_ff_counted_df, curv_of_traj_counted, use_curvature_to_ff_center, sample_size):
    ...
    for i in range(sample_size):
        ...
        traj_curv_counted, alt_curv_counted = find_stops_near_ff_utils.find_relative_curvature(alt_ff_counted_df, stop_ff_counted_df, curv_of_traj_counted, use_curvature_to_ff_center)
...
def generate_possible_combos_for_planning(
    ...
    for index, row in combo_df.iterrows():
        ...
        ref_point_value = np.round(ref_point_value_based_on_mode[ref_point_mode], 2)
        curv_of_traj_lower_end = np.round(curv_of_traj_lower_end_based_on_mode[curv_of_traj_mode], 2)
        curv_of_traj_upper_end = np.round(curv_of_traj_upper_end_based_on_mode[curv_of_traj_mode], 2)
...
def plot_tested_heading_df_or_curv_df(sub_df):
    ...
    plot_to_add = go.Scatter(x=sub_df['curv_of_traj_lower_end'], 
                            ...
                            customdata=sub_df[['r', 'shuffled_r_mean', 'shuffled_r_std']],
    ...
    fig.update_layout(title='Compare Correlations', xaxis_title='Curv of traj lower end', yaxis_title='Curv of traj upper end')
...
def process_tested_combo_df(tested_combo_df):
    tested_combo_df['shuffled_heading_r_mean_plus_2_std'] = tested_combo_df['shuffled_heading_r_mean'] + 2*tested_combo_df['shuffled_heading_r_std']
    tested_combo_df['shuffled_curv_r_mean_plus_2_std'] = tested_combo_df['shuffled_curv_r_mean'] + 2*tested_combo_df['shuffled_curv_r_std']

    tested_combo_df['heading_r_z_score'] = (tested_combo_df['heading_r'] - tested_combo_df['shuffled_heading_r_mean'])/tested_combo_df['shuffled_heading_r_std']
    tested_combo_df['curv_r_z_score'] = (tested_combo_df['curv_r'] - tested_combo_df['shuffled_curv_r_mean'])/tested_combo_df['shuffled_curv_r_std']

    tested_combo_df['heading_r_z_score'] = np.round(tested_combo_df['heading_r_z_score'], 2)
    tested_combo_df['curv_r_z_score'] = np.round(tested_combo_df['curv_r_z_score'], 2)
...
def plot_tested_heading_df_or_curv_df2(sub_df, 
    ...
    norm = mplc.Normalize(vmin=sub_df2['r_z_score'].min(), vmax=sub_df2['r_z_score'].max())
    ...
    for index, row in sub_df2.iterrows():
        customdata = np.array([[row['r_z_score']]*100, [row['curv_of_traj_lower_end']]*100, [row['curv_of_traj_upper_end']]*100]).T
        fig_lines.add_trace(go.Scatter(x=np.linspace(row['curv_of_traj_lower_end'], row['curv_of_traj_upper_end'], 100)+ref_point_value_to_add,
                                        y=[index]*100, 
                                        mode='lines', 
                                        line={'color': f'rgba{cmap(norm(row["r_z_score"]))}'},
                                        showlegend=False,
        ...
        if row['r_z_score'] > 2:
            fig_lines.add_annotation(x=row['curv_of_traj_upper_end']+ref_point_value_to_add, y=index+0.2, text=f'{row["r_z_score"]:.2f}', showarrow=False, font=dict(size=10, color='red'))
        else:
