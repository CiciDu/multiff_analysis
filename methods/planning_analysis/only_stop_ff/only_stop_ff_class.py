...
class OnlyStopFF(base_processing_class.BaseProcessing):
    ...
    def __init__(self, monkey_name='monkey_Bruno', 
        ...
        if raw_data_folder_path is not None:
            super().__init__()   
            self.load_raw_data(raw_data_folder_path=raw_data_folder_path, window_for_curv_of_traj=window_for_curv_of_traj, curv_of_traj_mode=curv_of_traj_mode)
    ...
    def make_only_stop_ff_df(self, exists_ok=True, stop_period_duration=2, ref_point_mode='distance', ref_point_value=-150):
        df_name = find_stops_near_ff_utils.find_df_name(self.monkey_name, ref_point_mode, ref_point_value)
        df_path = os.path.join(self.only_stop_ff_folder_path, df_name + '.csv')
        if exists_ok:
            if exists(df_path):
                self.only_stop_ff_df = pd.read_csv(df_path).drop(columns=['Unnamed: 0'])
                print(f'Successfully retrieved only_stop_ff_df from {df_path}')
                return
            else:
                print(f'Failed to retrieve only_stop_ff_df from {df_path}; will make a new one')
        ...
        self.all_closest_point_to_capture_df = alt_ff_utils.get_closest_stop_time_to_all_capture_time(self.ff_caught_T_sorted, self.monkey_information, stop_ff_index_array=np.arange(len(self.ff_caught_T_sorted)),
                                                                                                      )
        self.only_stop_ff_df = only_stop_ff_utils.get_only_stop_ff_df(self.all_closest_point_to_capture_df, self.ff_real_position_sorted, self.ff_caught_T_sorted, self.monkey_information, 
        ...
        self.only_stop_ff_df.to_csv(df_path)
        print(f'New only_stop_ff_df was stored in {df_path}.')


    def _make_ff_info_at_start_df(self, stop_period_duration=2):
        self.all_closest_point_to_capture_df = alt_ff_utils.get_closest_stop_time_to_all_capture_time(self.ff_caught_T_sorted, self.monkey_information, stop_ff_index_array=np.arange(len(self.ff_caught_T_sorted)),
                                                                                                      )
        self.monkey_info_in_all_stop_periods = only_stop_ff_utils.make_monkey_info_in_all_stop_periods(self.all_closest_point_to_capture_df, self.monkey_information, stop_period_duration=stop_period_duration)
    ...
    def make_x_features_df(self, exists_ok=True, ref_point_mode='distance', ref_point_value=-150,
        ...
        if exists_ok:
            if exists(df_path):
                self.x_features_df = pd.read_csv(df_path).drop(columns=['Unnamed: 0'])
                print(f'Successfully retrieved x_features_df from {df_path}')
                return
            else:
                print(f'Failed to retrieve x_features_df from {df_path}; will make a new one')
            
        if self.monkey_information is None:
            self.load_raw_data(raw_data_folder_path=self.raw_data_folder_path, window_for_curv_of_traj=self.window_for_curv_of_traj, curv_of_traj_mode=self.curv_of_traj_mode, truncate_curv_of_traj_by_time_of_capture=False)
    ...
    def make_x_and_y_var_df(self, scale_x_var=True, use_pca=False, n_components_for_pca=None):
        ...
        self.x_var_df, self.y_var_df = ml_methods_utils.make_x_and_y_var_df(x_features_df_temp, self.only_stop_ff_df, scale_x_var=scale_x_var, 
                                                                            use_pca=use_pca, n_components_for_pca=n_components_for_pca)      



    def use_ref_columns_only(self):
        self.x_features_df = self.x_features_df[[column for column in self.x_features_df.columns if 'ref' in column]]


    def prepare_only_stop_ff_data_for_ml(self):

        self.only_stop_ff_df_for_ml = self.only_stop_ff_df.copy()
        self.x_features_df_for_ml = self.x_features_df.copy()

        self._prepare_only_stop_ff_data_for_ml()
    ...
    def streamline_preparing_for_ml(self, y_var_column, 
        ...
        self.x_var_df, self.y_var_df = ml_methods_utils.streamline_preparing_for_ml(self.x_features_df_for_ml, 
                                                                                    ...
                                                                                    ensure_stop_ff_at_front=ensure_stop_ff_at_front,
                                                                                    use_pca=use_pca,
