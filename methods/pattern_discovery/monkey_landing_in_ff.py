...
def make_scatter_around_target_df(monkey_information, closest_stop_to_capture_df, ff_real_position_sorted, data_folder_name=None, make_plot=False):
    ...
    scatter_around_target_df = valid_subset[['distance_from_ff_to_stop']].describe().T[['mean', 'std', '25%', '50%', '75%']]
    scatter_around_target_df['iqr'] = scatter_around_target_df['75%'] - scatter_around_target_df['25%']
    scatter_around_target_df.columns = ['distance_' + i for i in scatter_around_target_df.columns]
    scatter_around_target_df.reset_index(inplace=True, drop=True)

    scatter_around_target_df_angle = valid_subset[['angle_in_degrees_from_ff_to_stop']].describe().T[['mean', 'std', '25%', '50%', '75%']]
    scatter_around_target_df_angle['iqr'] = scatter_around_target_df_angle['75%'] - scatter_around_target_df_angle['25%']
    scatter_around_target_df_angle.columns = ['angle_' + i for i in scatter_around_target_df_angle.columns]
    scatter_around_target_df_angle.reset_index(inplace=True, drop=True)
    scatter_around_target_df = pd.concat([scatter_around_target_df, scatter_around_target_df_angle], axis=1)

    scatter_around_target_df_abs_angle = valid_subset[['angle_in_degrees_from_ff_to_stop']].abs().describe().T[['mean', 'std', '25%', '50%', '75%']]
    scatter_around_target_df_abs_angle['iqr'] = scatter_around_target_df_abs_angle['75%'] - scatter_around_target_df_abs_angle['25%']
    scatter_around_target_df_abs_angle.columns = ['abs_angle_' + i for i in scatter_around_target_df_abs_angle.columns]
    scatter_around_target_df_abs_angle.reset_index(inplace=True, drop=True)
    scatter_around_target_df = pd.concat([scatter_around_target_df, scatter_around_target_df_abs_angle], axis=1)
