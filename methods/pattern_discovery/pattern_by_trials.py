...
def cluster_around_target_func(ff_dataframe, caught_ff_num, ff_caught_T_new, ff_real_position_sorted, 
  ...
  ff_sub = ff_dataframe[['ff_index', 'target_index', 'ff_distance', 'visible', 'time']]
  ...
  for i in range(caught_ff_num):
    ...
    ff_sub2 = ff_sub[(ff_sub['time'] > duration[0])&(ff_sub['time'] < duration[1])]
...
def find_instances_of_sudden_flash_ff(ff_dataframe, max_ff_distance_from_monkey=50):
  ...
  start_index0 = np.where(np.ediff1d(np.array(ff_dataframe['visible'])) == 1)[0]+1
  ...
  start_index1 = np.where(np.ediff1d(np.array(ff_dataframe['point_index'])) > 1)[0]+1
  ...
  sudden_flash_sub = ff_sub[ff_sub['ff_distance'] < max_ff_distance_from_monkey].copy()
...
def ignore_sudden_flash_func(ff_dataframe, max_point_index, max_ff_distance_from_monkey = 50):
  ...
  sudden_flash_sub = find_instances_of_sudden_flash_ff(ff_dataframe,  max_ff_distance_from_monkey)
  ...
  cases_w_sudden_flash_non_target = sudden_flash_sub[(sudden_flash_sub['ff_index'] != sudden_flash_sub['target_index']) & (sudden_flash_sub['ff_index'] != sudden_flash_sub['target_index']+1)]

  # Only keep the cases where the suddenly visible firefly is closer than the target
  cases_w_sudden_flash_non_target = cases_w_sudden_flash_non_target[cases_w_sudden_flash_non_target['ffdistance2target'] > cases_w_sudden_flash_non_target['ff_distance']]
  ...
  ignored_ff_target_pairs = cases_w_sudden_flash_non_target[['ff_index', 'target_index', 'ff_distance', 'ff_angle', 'ff_angle_boundary']].drop_duplicates()
  ...
  ignore_sudden_flash_indices = np.sort(cases_w_sudden_flash_non_target['point_index'].unique())
  ...
  ignore_sudden_flash_indices_for_anim = np.unique(np.array(ignore_sudden_flash_indices_for_anim))
  ignore_sudden_flash_indices_for_anim = ignore_sudden_flash_indices_for_anim[ignore_sudden_flash_indices_for_anim <= max_point_index]
