...
def make_target_closest_or_target_angle_smallest(ff_dataframe, max_point_index, column='ff_distance'):
    ...
    target_closest_point_index = np.array(min_distance_subset[min_distance_subset['ff_index'] == min_distance_subset['target_index']].point_index)
    # Below are point indices where the target is not the closest; they the complement of target_closest_point_index
    target_not_closest_point_index = np.delete(np.arange(max_point_index+1), target_closest_point_index)
...
def make_target_closest(ff_dataframe, max_point_index, data_folder_name=None):
    ...
    if data_folder_name:
        np.savetxt(os.path.join(data_folder_name, 'target_closest.csv'), target_closest.tolist(), delimiter=',')
...
def make_target_angle_smallest(ff_dataframe, max_point_index, data_folder_name=None):
    ...
    if data_folder_name:
        np.savetxt(os.path.join(data_folder_name, 'target_angle_smallest.csv'), target_angle_smallest.tolist(), delimiter=',')
