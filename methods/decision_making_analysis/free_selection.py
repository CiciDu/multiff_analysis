...
def find_info_of_n_ff_per_point(free_selection_df, ff_dataframe, ff_real_position_sorted, monkey_information, num_ff_per_row=3, guarantee_including_target_info=True, only_select_n_ff_case=None, selection_criterion_if_too_many_ff='abs_ff_angle',
    ...
    if only_select_n_ff_case is not None:
        ...
        if isinstance(only_select_n_ff_case, int) is False:
            raise ValueError('only_select_n_ff_case needs to be either None or an integer')
    ...
    if guarantee_including_target_info:
        free_selection_df_temp = free_selection_df[['starting_point_index', 'ff_index']].copy()
        free_selection_df_temp.rename(columns={'starting_point_index': 'point_index'}, inplace=True)
    ...
    info_of_n_ff_per_point['order'] = np.tile(range(num_ff_per_row), int(len(info_of_n_ff_per_point)/num_ff_per_row))
...
def find_label_of_ff_in_obs_ff(sequence_of_obs_ff_indices, ff_index_to_label, na_filler=0):
    # find labels
    matched_rows = np.where(sequence_of_obs_ff_indices - ff_index_to_label.reshape(-1,1) == 0)
    labels_df = pd.DataFrame({'row': matched_rows[0], 'label': matched_rows[1]})
...
def find_free_selection_inputs_from_info_of_n_ff_per_point(info_of_n_ff_per_point, 
    ...
    point_index_array = info_of_n_ff_per_point[info_of_n_ff_per_point['order']==0].point_index.values
    free_selection_inputs = pd.DataFrame() 
    free_selection_inputs_df_for_plotting = pd.DataFrame()
    pred_var = []
    ...
    for i in range(num_ff_per_row):
        ...
        if np.any(current_info.point_index.values != point_index_array):
            raise ValueError('point_index_array is not in the right order for order number: ', i)
    ...
    return free_selection_inputs, free_selection_inputs_df_for_plotting, sequence_of_obs_ff_indices, point_index_array, pred_var
...
def organize_free_selection_data(free_selection_df, ff_dataframe, ff_real_position_sorted, monkey_information, only_select_n_ff_case=None, num_ff_per_row=5, 
    ...
    if info_of_n_ff_per_point is None:
        free_selection_df = free_selection_df.sort_values(['starting_point_index'], ascending=[True]).reset_index(drop=True)
    ...
    rows_with_no_matching_labels = np.where(free_selection_labels == num_ff_per_row)[0]
    point_index_with_no_matching_labels = point_index_array[rows_with_no_matching_labels]
    non_chosen_rows_of_df = free_selection_df[free_selection_df['starting_point_index'].isin(point_index_with_no_matching_labels)].copy()
    ...
    chosen_rows_of_df = free_selection_df[~free_selection_df['starting_point_index'].isin(point_index_with_no_matching_labels)].copy()
    chosen_row_indices = chosen_rows_of_df.index.values.tolist()
    sequence_of_obs_ff_indices = [sequence_of_obs_ff_indices[i] for i in chosen_row_indices]
    free_selection_inputs_df['point_index'] = point_index_array
    free_selection_inputs_df = free_selection_inputs_df[~free_selection_inputs_df['point_index'].isin(point_index_with_no_matching_labels)].reset_index(drop=True)
    free_selection_labels = free_selection_labels[free_selection_inputs_df.index.values]
    free_selection_inputs_df.drop(columns=['point_index'], inplace=True)

    return free_selection_inputs_df, free_selection_labels, cases_for_inspection, chosen_rows_of_df, sequence_of_obs_ff_indices, free_selection_inputs_df_for_plotting
...
def make_free_selection_predictions_using_trained_model(trained_model, ff_indices, target_indices, ff_dataframe, ff_real_position_sorted, ff_caught_T_new, monkey_information, time_of_evaluation=None):
    ...
    inputs, labels, cases_for_inspection, chosen_rows_of_df, sequence_of_obs_ff_indices, free_selection_inputs_df_for_plotting \
        = organize_free_selection_data(substitute_free_selection_df, ff_dataframe, 
...
def guarantee_ff_dataframe_include_target_info(target_info, ff_dataframe_sub, ff_real_position_sorted, ff_dataframe, monkey_information):
    ...
    ff_dataframe_w_targets = pd.merge(target_info[['point_index', 'ff_index']], ff_dataframe_sub, on=['point_index', 'ff_index'], how='outer')
    ff_dataframe_w_targets['selection_criterion'] = ff_dataframe_w_targets['selection_criterion'].fillna(-9999)
    ...
    ff_dataframe_w_targets_na = ff_dataframe_w_targets[ff_dataframe_w_targets['ff_angle'].isna()].copy()    
    ff_info = decision_making_utils.find_many_ff_info_anew(ff_dataframe_w_targets_na.ff_index.values, ff_dataframe_w_targets_na.point_index.values, ff_real_position_sorted, ff_dataframe, monkey_information)
...
def guarantee_n_ff_per_point_index_in_ff_dataframe(ff_dataframe_sub, all_point_index, num_ff_per_row=3, only_select_n_ff_case=None, selection_criterion_if_too_many_ff='abs_ff_angle',
    ...
    count_of_ff = pd.merge(point_index_df, count_of_ff, on='point_index', how='left').fillna(0)
    ...
    if only_select_n_ff_case is not None:
        count_of_ff = count_of_ff[count_of_ff['ff_index'] == num_ff_per_row].copy()
        ff_dataframe_sub = ff_dataframe_sub[ff_dataframe_sub['point_index'].isin(count_of_ff.point_index.values)].copy()
    else:
        ...
        not_enough_ff = count_of_ff[count_of_ff['ff_index'] < num_ff_per_row].copy()
        not_enough_ff['num_missing'] = num_ff_per_row - not_enough_ff['ff_index']
        ...
        ff_dataframe_sub.sort_values(['point_index', 'selection_criterion'], ascending=True, inplace=True)
        ff_dataframe_sub = ff_dataframe_sub.groupby('point_index').head(num_ff_per_row)
...
def fill_up_additional_attributes_for_placeholders(placeholder_df, 
    ...
    placeholder_df = decision_making_utils.fill_new_columns_with_placeholder_values(placeholder_df, columns=attributes, additional_placeholder_mapping=additional_placeholder_mapping)
