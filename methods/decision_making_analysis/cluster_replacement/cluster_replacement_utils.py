...
def find_df_related_to_cluster_replacement(replacement_df, prior_to_replacement_df, non_chosen_df, manual_anno, ff_dataframe, monkey_information, ff_real_position_sorted, ff_life_sorted, sample_size=None, equal_sample_from_two_cases=False):
    ...
    old_ff_info, new_ff_info, all_point_index, all_time = replacement.get_old_and_new_ff_info(replacement_df, prior_to_replacement_df, ff_dataframe, monkey_information, ff_real_position_sorted)
    old_ff_info, new_ff_info, old_ff_positions, new_ff_positions, all_point_index_1, all_time_1 = eliminate_close_by_pairs_between_old_and_new_ff_info(old_ff_info, new_ff_info, all_time, ff_real_position_sorted)
    ...
    old_ff_cluster_df = cluster_analysis.turn_list_of_ff_clusters_info_into_dataframe(old_ff_cluster, all_point_index_1)
    old_ff_cluster_df = decision_making_utils.find_many_ff_info_anew(old_ff_cluster_df['ff_index'].values, old_ff_cluster_df['point_index'].values, ff_real_position_sorted, ff_dataframe_visible, monkey_information)

    new_ff_cluster_df = cluster_analysis.turn_list_of_ff_clusters_info_into_dataframe(new_ff_cluster, all_point_index_1)
    new_ff_cluster_df = decision_making_utils.find_many_ff_info_anew(new_ff_cluster_df['ff_index'].values, new_ff_cluster_df['point_index'].values, ff_real_position_sorted, ff_dataframe_visible, monkey_information)

    parallel_old_ff_cluster_df = cluster_analysis.turn_list_of_ff_clusters_info_into_dataframe(parallel_old_ff_cluster, all_point_index_2)
    parallel_old_ff_cluster_df = decision_making_utils.find_many_ff_info_anew(parallel_old_ff_cluster_df['ff_index'].values, parallel_old_ff_cluster_df['point_index'].values, ff_real_position_sorted, ff_dataframe_visible, monkey_information)

    non_chosen_ff_cluster_df = cluster_analysis.turn_list_of_ff_clusters_info_into_dataframe(non_chosen_ff_cluster, all_point_index_2)
    ...
    new_ff_cluster_df = mark_intended_target_in_df(new_ff_cluster_df, new_ff_info)

    # mark the current intended_target in parallel_old_ff_cluster_df
    parallel_old_ff_cluster_df = mark_intended_target_in_df(parallel_old_ff_cluster_df, parallel_old_ff_info)
...
def further_process_df_related_to_cluster_replacement(joined_old_ff_cluster_df, joined_new_ff_cluster_df, num_old_ff_per_row=3, num_new_ff_per_row=3, selection_criterion_if_too_many_ff='time_since_last_visible', sorting_criterion=None):
    ...
    if joined_old_ff_cluster_df[selection_criterion_if_too_many_ff].isnull().values.any():
        raise ValueError('The column "selection_criterion_if_too_many_ff" contains NA values in joined_old_ff_cluster_df')
    elif joined_new_ff_cluster_df[selection_criterion_if_too_many_ff].isnull().values.any():
        raise ValueError('The column "selection_criterion_if_too_many_ff" contains NA values in joined_new_ff_cluster_df')
    ...
    joined_old_ff_cluster_df.loc[joined_old_ff_cluster_df['whether_intended_target']==True, 'selection_criterion'] = -9999


    # make sure that there are num_old_ff_per_row or num_new_ff_per_row of ff for each point_index
    joined_old_ff_cluster_df = free_selection.guarantee_n_ff_per_point_index_in_ff_dataframe(joined_old_ff_cluster_df, np.unique(joined_old_ff_cluster_df.point_index.values), num_ff_per_row=num_old_ff_per_row, selection_criterion_if_too_many_ff=selection_criterion_if_too_many_ff)
    joined_new_ff_cluster_df = GUAT_and_TAFT.retain_rows_in_df1_that_share_or_not_share_columns_with_df2(joined_new_ff_cluster_df, joined_old_ff_cluster_df, columns=['point_index', 'ff_index'], whether_share=False)
    joined_new_ff_cluster_df = free_selection.guarantee_n_ff_per_point_index_in_ff_dataframe(joined_new_ff_cluster_df, np.unique(joined_old_ff_cluster_df.point_index.values), num_ff_per_row=num_new_ff_per_row, selection_criterion_if_too_many_ff=selection_criterion_if_too_many_ff)

    # fill out NAs
    joined_old_ff_cluster_df['whether_intended_target'] = joined_old_ff_cluster_df['whether_intended_target'].fillna(False).infer_objects(copy=False)
    joined_new_ff_cluster_df['whether_intended_target'] = joined_new_ff_cluster_df['whether_intended_target'].fillna(False).infer_objects(copy=False)
    joined_old_ff_cluster_df['whether_changed'] = joined_old_ff_cluster_df['whether_changed'].fillna(False).infer_objects(copy=False)
    joined_new_ff_cluster_df['whether_changed'] = joined_new_ff_cluster_df['whether_changed'].fillna(False).infer_objects(copy=False)
    
    # but whether_changed should depend on the point_index
    _point_index = joined_old_ff_cluster_df[joined_old_ff_cluster_df['whether_changed']==True]['point_index'].values
    joined_old_ff_cluster_df.loc[joined_old_ff_cluster_df['point_index'].isin(_point_index), 'whether_changed'] = True
    joined_new_ff_cluster_df.loc[joined_new_ff_cluster_df['point_index'].isin(_point_index), 'whether_changed'] = True

    if sorting_criterion is None:
        joined_old_ff_cluster_df.sort_values(by=['point_index', 'selection_criterion'], inplace=True)
        joined_new_ff_cluster_df.sort_values(by=['point_index', 'selection_criterion'], inplace=True)
    else:
        ...
        joined_old_ff_cluster_df.loc[joined_old_ff_cluster_df['whether_intended_target']==True, 'sorting_criterion'] = -9999
        joined_old_ff_cluster_df.sort_values(by=['point_index', 'sorting_criterion'], inplace=True)
        joined_new_ff_cluster_df.sort_values(by=['point_index', 'sorting_criterion'], inplace=True)

    # add order column
    joined_old_ff_cluster_df['order'] = np.tile(range(num_old_ff_per_row), int(len(joined_old_ff_cluster_df)/num_old_ff_per_row))
    joined_new_ff_cluster_df['order'] = np.tile(range(num_new_ff_per_row), int(len(joined_new_ff_cluster_df)/num_new_ff_per_row))
...
def eliminate_close_by_pairs_between_old_and_new_ff_info(old_ff_info, new_ff_info, all_time, ff_real_position_sorted, min_distance_between_old_and_new_ff = 50):
    ...
    old_ff_to_new_ff_distance = np.linalg.norm(old_ff_positions - new_ff_positions, axis=1)
    in_same_cluster = np.where(old_ff_to_new_ff_distance <= min_distance_between_old_and_new_ff)[0]
    not_in_same_cluster = np.where(old_ff_to_new_ff_distance > min_distance_between_old_and_new_ff)[0]
...
def eliminate_rows_with_large_value_in_shared_column_between_df(shared_column, max_value, df_1, df_2):
    ...
    shared_valid_point_index = np.intersect1d(df_1_valid_point_index, df_2_valid_point_index)
...
def mark_intended_target_in_df(df, intended_target_df):
    intended_target_df_sub = intended_target_df[['ff_index', 'point_index']].copy()
    ...
    df = pd.merge(df, intended_target_df_sub, on=['ff_index', 'point_index'], how='left')
...
def find_non_chosen_ff_info_and_parallel_old_ff_info_with_no_close_by_pairs(non_chosen_df, manual_anno, ff_real_position_sorted, ff_life_sorted, ff_dataframe, monkey_information):
    non_chosen_ff_info, all_point_index, all_time = replacement.get_non_chosen_ff_info(non_chosen_df, selection_criterion='abs_curv_diff', one_row_per_group=False)
    parallel_old_ff_info, position_indices_with_valid_ff = replacement.get_parallel_old_ff_info(all_point_index, all_time, manual_anno, ff_real_position_sorted, ff_dataframe, monkey_information)
    ...
    parallel_old_ff_info, non_chosen_ff_info, parallel_old_ff_positions, non_chosen_ff_positions, all_point_index, all_time = eliminate_close_by_pairs_between_old_and_new_ff_info(parallel_old_ff_info, non_chosen_ff_info, all_time, ff_real_position_sorted)

    # use merge to preserve only rows where the non-chosen ff and the old ff are not in the same cluster  
    non_chosen_df = pd.merge(non_chosen_df, non_chosen_ff_info[['ff_index', 'point_index']], on=['ff_index', 'point_index'], how='inner')
...
def find_more_ff_df(point_index_all, ff_dataframe, ff_real_position_sorted, monkey_information, all_available_ff_in_near_future=None, attributes_for_plotting=['ff_distance', 'ff_angle', 'time_since_last_visible']):
    ...
    if all_available_ff_in_near_future is not None:
        all_possible_ff = pd.concat([more_ff_df[['ff_index', 'point_index']], all_available_ff_in_near_future], axis=0).reset_index(drop=True)
...
def eliminate_part_of_more_ff_inputs_already_in_observation(more_ff_inputs, sequence_of_obs_ff_indices, point_index_all):
    ff_cluster_df = cluster_analysis.turn_list_of_ff_clusters_info_into_dataframe(sequence_of_obs_ff_indices, point_index_all)
    ...
    more_ff_inputs = pd.merge(more_ff_inputs, ff_cluster_df[['ff_index', 'point_index', 'in_obs']], on=['ff_index', 'point_index'], how='left')
...
def turn_more_ff_df_into_free_selection_inputs_df_for_plotting(more_ff_df, point_index_all, return_all_attributes=False, attributes_for_plotting=['ff_distance', 'ff_angle', 'time_since_last_visible']):
    ...
    more_ff_df = free_selection.guarantee_n_ff_per_point_index_in_ff_dataframe(more_ff_df, point_index_all, num_ff_per_row=num_ff_per_row)
    more_ff_df = more_ff_df.sort_values(by=['point_index']).copy()
    more_ff_df['order'] = np.tile(range(num_ff_per_row), int(len(more_ff_df)/num_ff_per_row))
    free_selection_inputs_df, free_selection_inputs_df_for_plotting, sequence_of_obs_ff_indices, point_index_array, pred_var = free_selection.find_free_selection_inputs_from_info_of_n_ff_per_point(more_ff_df, \
                    attributes_for_plotting=attributes_for_plotting, num_ff_per_row=num_ff_per_row)
    if return_all_attributes:
        return free_selection_inputs_df
    else:
        return free_selection_inputs_df_for_plotting
...
def supply_info_of_ff_last_seen_and_next_seen_to_df(df, ff_dataframe, monkey_information, ff_real_position_sorted, ff_caught_T_sorted, curv_of_traj_df=None,
    ...
    df = decision_making_utils.add_attributes_last_seen_or_next_seen_for_each_ff_in_df(df, ff_dataframe, attributes=attributes_to_add)
    df = trajectory_info.add_distance_and_angle_from_monkey_now_to_monkey_when_ff_last_seen_or_next_seen(df, monkey_information, ff_dataframe, monkey_xy_from_other_time=df[['last_seen_monkey_x', 'last_seen_monkey_y']].values)
    df = decision_making_utils.add_attributes_last_seen_or_next_seen_for_each_ff_in_df(df, ff_dataframe, attributes=attributes_to_add, use_last_seen=False)
    df = trajectory_info.add_distance_and_angle_from_monkey_now_to_monkey_when_ff_last_seen_or_next_seen(df, monkey_information, ff_dataframe, monkey_xy_from_other_time=df[['next_seen_monkey_x', 'next_seen_monkey_y']].values, use_last_seen=False)
