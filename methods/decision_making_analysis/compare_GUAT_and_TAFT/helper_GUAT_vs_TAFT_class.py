...
class HelperGUATandTAFTclass():
    ...
    def _get_stops_near_ff_df(self, 
        ...
        self.stops_near_ff_df = self.TAFT_df.copy() if (self.GUAT_or_TAFT == 'TAFT') else self.GUAT_df.copy()
    ...
    def _get_GUAT_or_TAFT_x_df(self, save_data=True):
        self.GUAT_or_TAFT_x_df = GUAT_vs_TAFT_utils.combine_relevant_features(self.x_features_df, self.only_stop_ff_df, self.plan_x_df, self.plan_y_df)

        # add num_stops
        trials_df = self.TAFT_trials_df if self.GUAT_or_TAFT == 'TAFT' else self.GUAT_w_ff_df
        self.GUAT_or_TAFT_x_df = self.GUAT_or_TAFT_x_df.merge(trials_df[['stop_point_index', 'num_stops']], on='stop_point_index', how='left')

        # also clean out unnecesary columns especially in clusters
        new_df_columns = features_to_keep_utils.get_minimal_features_to_keep(self.GUAT_or_TAFT_x_df, for_classification=False, deal_with_combd_min_ff_distance=True)
