...
class DecisionMaking():
    ...
    def __init__(self, ff_dataframe, ff_caught_T_new, ff_real_position_sorted, monkey_information, raw_data_folder_path=None,
        ...
        self.trajectory_features = ['monkey_distance', 'monkey_angle_to_origin']
    ...
    def retrieve_manual_anno(self):
        ...
        wrong_indices = np.where(whether_the_same==False)[0] # this should return no index; otherwise the point_index/time is not strictly monotonously increasing
    ...
    def separate_manual_anno(self):
        ...
        self.manual_anno_trimmed_valid = self.manual_anno_trimmed[self.manual_anno_trimmed['ff_index'] >= 0]
       
        free_selection = self.manual_anno_trimmed_valid.groupby('target_index').first().reset_index()
        self.manual_anno_trimmed_invalid_index = self.manual_anno_trimmed[self.manual_anno_trimmed['ff_index'] < 0].index
        invalid_ff_indices_positions = np.where(np.array(self.manual_anno_trimmed.index.isin(self.manual_anno_trimmed_invalid_index))==True)[0]
        # to make sure it doesn't go out of index
        invalid_ff_indices_positions = invalid_ff_indices_positions[invalid_ff_indices_positions < len(self.manual_anno_trimmed)-1]
        more_free_selection = self.manual_anno_trimmed.iloc[invalid_ff_indices_positions+1]
        self.free_selection_df = pd.concat([free_selection, more_free_selection]).drop_duplicates()
        # On the other hand, all the ff after a negative value of ff_index can be considered as free_selection
        self.replacement_df = self.manual_anno_trimmed_valid[~self.manual_anno_trimmed_valid.starting_point_index.isin(self.free_selection_df.starting_point_index)]
        # find rows in manual_anno_trimmed_valid that are one row prior to the rows in replacement_ff
        replacement_df_positions = np.where(np.array(self.manual_anno_trimmed_valid.index.isin(self.replacement_df.index))==True)[0]
        self.prior_to_replacement_df = self.manual_anno_trimmed_valid.iloc[replacement_df_positions-1]
        ...
        for trial in range(self.manual_anno_trimmed.target_index.min()+1, self.manual_anno_trimmed.target_index.max()):
            ...
            self.non_chosen_df = pd.concat([self.non_chosen_df, ff_dataframe_subset[ff_dataframe_subset['ff_index'].isin(non_chosen_ff)]])

        # see data span
        span_in_min = (self.manual_anno.loc[self.manual_anno.index.max(), 'time'].item() - self.manual_anno.loc[self.manual_anno.index.min(), 'time'].item())/60
    ...
    def determine_n_seconds_before_or_after_crossing_boundary(self, n_seconds_before_crossing_boundary=None, n_seconds_after_crossing_boundary=None):
        if n_seconds_before_crossing_boundary is None:
            n_seconds_before_crossing_boundary = max(0, self.time_range_of_trajectory[1])
        if n_seconds_after_crossing_boundary is None:
            n_seconds_after_crossing_boundary = max(0, - self.time_range_of_trajectory[0])
    ...
    def eliminate_crossing_boundary_cases(self, n_seconds_before_crossing_boundary=None, n_seconds_after_crossing_boundary=None):
        ...
        print("replacement_df:", self.replacement_df.shape[0], "out of", original_length, "rows remains")
        ...
        print("free_selection_df:", self.free_selection_df.shape[0], "out of", original_length, "rows remains")
        ...
        print("non_chosen_df:", self.non_chosen_df.shape[0], "out of", original_length, "rows remains")
    ...
    def get_replacement_data(self, add_arc_info=False, add_current_curvature_of_traj=False, curvature_df=None, curv_of_traj_df=None, replacement_inputs_format = 'old_plus_diff_between_old_and_new', 
        ...
        replacement_data: df, containing the input features for both original and alternative ff, for visualization
        '''
        ...
        if (replacement_inputs_format == 'old_plus_diff_between_old_and_new') or (replacement_inputs_format == 'diff_between_old_and_new'):
            replacement_data = self.changing_pursued_ff_data_diff.copy()
        elif replacement_inputs_format == 'both_old_and_new':
            replacement_data = self.changing_pursued_ff_data.copy()
        else:
            raise ValueError("replacement_inputs_format can only be 'old_plus_diff_between_old_and_new', 'diff_between_old_and_new', or 'both_old_and_new'")
        
        self.replacement_inputs = replacement_data.drop(['whether_changed'], axis=1).values
        self.replacement_labels = replacement_data['whether_changed'].values 
        


    def get_free_selection_data(self, num_ff_per_row=5, keeping_1_out_of_n_rows=1, add_arc_info=False, arc_info_to_add=['optimal_curvature', 'curv_diff'],
                                curvature_df=None, curv_of_traj_df=None, **kwargs):
        '''
        free_selection_inputs_df: df, the input features for machine learning
        ...
        self.free_selection_inputs_df, self.free_selection_labels, self.cases_for_inspection, self.chosen_rows_of_df, self.sequence_of_obs_ff_indices, self.free_selection_inputs_df_for_plotting = free_selection.organize_free_selection_data(self.free_selection_df_sample, self.ff_dataframe, self.ff_real_position_sorted, self.monkey_information,
        ...
        self.free_selection_point_index = self.chosen_rows_of_df.starting_point_index.values.astype(int)
    ...
    def prepare_data_for_machine_learning(self, kind="free selection", furnish_with_trajectory_data=True, trajectory_data_kind="position", add_traj_stops=True): 
        ...
        self.converting_multi_class_for_free_selection = False # if using multi_class later, this will be updated to be True
        self.add_traj_stops = add_traj_stops
        self.trajectory_data_kind = trajectory_data_kind

        if kind == "free selection":
            self.X_all = self.free_selection_inputs.copy()
            self.X_all_to_plot = self.free_selection_inputs_df_for_plotting.copy().values
            self.y_all = self.free_selection_labels.copy()
            self.indices = np.arange(len(self.free_selection_inputs))
            self.time_all = self.free_selection_time
            self.point_index_all = self.free_selection_point_index
            self.input_features = self.free_selection_inputs_df.columns

        elif kind == "replacement":          
            replacement_data = self.changing_pursued_ff_data_diff.drop(['whether_changed'], axis=1)
            self.X_all = replacement_data.values
            ...
            self.input_features = replacement_data.columns
        elif kind is None:
            pass
        else:
            raise ValueError("kind can only be 'free selection' or 'replacement'")

        if furnish_with_trajectory_data:
            self.furnish_machine_learning_data_with_trajectory_data(trajectory_data_kind=trajectory_data_kind, add_traj_stops=add_traj_stops)
    ...
    def turn_y_label_into_multi_class(self, allow_multi_label=True, manual_anno_mul = None):
        ...
        if self.data_kind != "free selection":
            raise ValueError("The function turn_y_label_into_multi_class is only for free selection data")
        ...
        self.y_all, self.anno_but_not_obs_ff_indices_dict = decision_making_utils.turn_labels_into_multi_label_format(y_all, manual_anno_mul, self.manual_anno_long, sequence_of_obs_ff_indices, sequence_of_original_starting_point_index, allow_multi_label=allow_multi_label)
        ...
        for key, row in self.non_chosen_rows_of_df.iterrows():
            ...
            self.anno_but_not_obs_ff_indices_dict[starting_point_index].append(int(row['ff_index']))
    ...
    def split_data_to_train_and_test(self, scaling_data=True, keep_whole_chunks=False, test_size=0.2):
        ...
        if keep_whole_chunks:
            ...
            self.indices_test = self.indices[test_indice_start:test_indice_start+num_test_points]
            ...
            self.X_train, self.X_test, self.y_train, self.y_test, self.indices_train, self.indices_test = train_test_split(X_all_to_use, self.y_all, self.indices, test_size=test_size)
        ...
        if self.furnish_with_trajectory_data:
            ...
            if self.monkey_information is not None:
                self.traj_distances, self.traj_angles, self.left_end_r, self.left_end_theta, self.right_end_r, self.right_end_theta = monkey_heading_functions.find_all_mheading_components_in_polar(self.monkey_information, self.time_all, self.time_range_of_trajectory, self.gc_kwargs['num_time_points_for_trajectory'])
    ...
    def furnish_machine_learning_data_with_trajectory_data(self, trajectory_data_kind="position", add_traj_stops=True):
        ...
        self.input_features = np.concatenate([self.input_features, self.trajectory_feature_names], axis=0)
    ...
    def use_neural_network(self, n_epochs = 200, batch_size = 100):
        ...
        if np.any(self.y_all > 1) or np.any(self.y_train > 1):
            ...
            self.split_data_to_train_and_test(scaling_data=self.scaling_data, keep_whole_chunks=self.keep_whole_chunks, test_size=self.test_size)
        ...
        self.nn_model, self.y_pred = machine_learning_utils.use_neural_network_on_classification_func(self.X_train, self.y_train, self.X_test, self.y_test, n_epochs = n_epochs, batch_size = batch_size)
    ...
    def use_knn(self):
        ...
        print("Accuracy:", accuracy) # In multilabel classification, this function computes subset accuracy: the set of free_selection_labels predicted for a sample must exactly match the corresponding set of free_selection_labels in y_true.
        print("Hamming Loss:", hamming_loss(y_test, y_pred)) # Hamming loss is the fraction of wrong free_selection_labels to the total number of free_selection_labels.
    ...
    def get_pred_results_df(self):
        ...
        if self.converting_multi_class_for_free_selection:
            ...
            indices_in_prob = np.stack([np.arange(len(self.y_pred)), self.y_pred]).T
            self.y_pred_prob_all = self.model.predict_proba(self.X_test)
            # take out only the probability of the predicted labels
            self.y_pred_prob = self.y_pred_prob_all[indices_in_prob[:,0], indices_in_prob[:,1]]
    ...
    def add_additional_info_to_plot(self, time_range_of_trajectory, num_time_points_for_trajectory, ff_attributes=['ff_distance', 'ff_angle', 'time_since_last_vis']):
        ...
        self.more_ff_df, self.more_ff_inputs_df_for_plotting = cluster_replacement_utils.find_more_ff_inputs_for_plotting(self.point_index_all, self.sequence_of_obs_ff_indices, self.ff_dataframe, self.ff_real_position_sorted, self.monkey_information, ff_attributes=ff_attributes)
    ...
    def prepare_to_plot_prediction_results(self, use_more_ff_inputs=False, use_more_traj_points=False, show_direction_of_monkey_on_trajectory=False):
        ...
        if show_direction_of_monkey_on_trajectory:
            if self.furnish_with_trajectory_data is True:
                ...
                warnings.warn("show_direction_of_monkey_on_trajectory is set to False because furnish_with_trajectory_data was False")
    ...
    def plot_prediction_results(self, selected_cases=None, PlotTrials_args=None, also_show_regular_plot=False, max_plot_to_make=40, show_direction_of_monkey_on_trajectory=False, use_more_ff_inputs=False, use_more_traj_points=False):
        ...
        if also_show_regular_plot:
            if PlotTrials_args is None:
                raise ValueError("PlotTrials_args cannot be None if also_show_regular_plot is True")
        else:
            all_selected_cases = [all_selected_cases] # so that when using the for loop later, it will still only call the function once



        for selected_cases in all_selected_cases: # break the cases down into case by case in order to plot the regular plot
            if also_show_regular_plot:
            ...
            if also_show_regular_plot:
                duration  = self.time_range_of_trajectory + self.time_to_plot[selected_cases[0]]
