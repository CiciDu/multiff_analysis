...
def find_time_since_last_visible_OR_time_till_next_visible(ff_indices, all_current_time, ff_dataframe, time_since_last_visible=True, return_duration_of_last_visible_period=False):
    ...
    ff_visible_df = ff_dataframe_visible[(ff_dataframe_visible['ff_index'].isin(ff_indices))]
    ...
    if time_since_last_visible:
        ...
        ff_visible_df['time_till_next_visible'] = ff_visible_df['time_till_next_visible'].fillna(5000)
...
def add_time_when_last_visible_period_began_when_calculating_time_since_last_visible(ff_visible_df):
    ...
    ff_visible_df2['point_index_diff'] = ff_visible_df2.groupby(['ff_index', 'current_time'])['point_index'].diff(periods=1).fillna(100)
...
def find_attributes_of_ff_when_last_visible_OR_next_visible(ff_indices, all_current_time, ff_dataframe, use_last_seen=True, attributes=['ff_distance', 'ff_angle', 'curv_diff'],
    ...
    ff_info = pd.merge(ff_info, current_time_df, how='right', on='unique_id').drop(columns=['unique_id'])

    ff_info = fill_NA_in_columns(ff_info, columns=attributes, additional_placeholder_mapping=additional_placeholder_mapping)
...
def find_placeholder_values_for_attributes(attributes=['ff_distance', 'ff_angle', 'curv_diff'], additional_placeholder_mapping=None):
    ...
    for attribute in attributes:
        ...
        whether_placeholder_using_both_signs.append(placeholder_mapping[attribute][1])
...
def fill_NA_in_columns(df, columns=['ff_distance', 'ff_angle'], additional_placeholder_mapping=None):
    df = df.copy()
    placeholder_values, placeholder_using_both_signs = find_placeholder_values_for_attributes(attributes=columns, additional_placeholder_mapping=additional_placeholder_mapping)    
    
    for i in range(len(columns)):
        na_index = df[columns[i]].isna()
        if placeholder_using_both_signs[i] == True:
            df.loc[na_index, columns[i]] = np.random.choice([-1, 1], size=np.sum(na_index)) * placeholder_values[i]
        else:
...
def fill_new_columns_with_placeholder_values(df, columns=['ff_distance', 'ff_angle'], additional_placeholder_mapping=None):
    ...
    for i in range(len(columns)):
        if placeholder_using_both_signs[i] == True:
            df[columns[i]] = np.random.choice([-1, 1], size=num_rows) * placeholder_values[i]
        else:
...
def add_curv_diff_to_df(df, monkey_information, curv_of_traj_df, ff_real_position_sorted=None, ff_radius_for_optimal_arc=15):
    ...
    curvature_df = curvature_utils.make_curvature_df(df, curv_of_traj_df, ff_radius_for_optimal_arc=ff_radius_for_optimal_arc)
    curvature_df = curvature_df[['ff_index', 'point_index', 'curv_diff', 'abs_curv_diff']].drop_duplicates()
...
def find_many_ff_info_anew(ff_indices, point_index, ff_real_position_sorted, ff_dataframe_visible, monkey_information, add_time_till_next_visible=False, add_curv_diff=False,
    ...
    if np.any(ff_indices < 0):
        warnings.warn('ff_indices should not contain negative values. Negative ff_indices are likely to be placeholders. They will be ignored.')
        point_index = point_index[ff_indices >= 0]
        ff_indices = ff_indices[ff_indices >= 0]
    if len(ff_indices) != len(point_index):
        raise ValueError('ff_indices and point_index should have the same length')
...
def find_ff_info_at_some_time(ff_index, point_index, ff_real_position_sorted, ff_dataframe, monkey_information):
    ...
    raw_ff_info = ff_dataframe[(ff_dataframe['ff_index']==ff_index) & (ff_dataframe['point_index']==point_index)].copy()
...
def turn_labels_into_multi_label_format(y_all, manual_anno_mul, manual_anno_long, sequence_of_obs_ff_indices, sequence_of_original_starting_point_index, allow_multi_label=True):
    ...
    anno_but_not_obs_ff_indices_dict = {} # store the ff_index that are in annotation but not in observation
    ...
    if allow_multi_label:
        ...
        for index, row in manual_anno_mul.iterrows():
            ...
            all_ff_indices = all_ff_indices[~np.isnan(all_ff_indices)].astype(int)
            manual_anno_mul_dict[row['starting_point_index'].astype(int)] = all_ff_indices


        # update y_all_multi
        for key, item in manual_anno_mul_dict.items():
            if key in sequence_of_original_starting_point_index:
                indices_in_chosen_rows = np.where(sequence_of_original_starting_point_index==key)[0]
...
def find_time_points_that_are_within_n_seconds_after_crossing_boundary(input_time, crossing_boundary_time, n_seconds_after_crossing_boundary=2, n_seconds_before_crossing_boundary=0):
    ...
    n_seconds_before_crossing_boundary = abs(n_seconds_before_crossing_boundary)

    corresponding_CB_time = crossing_boundary_time[np.searchsorted(crossing_boundary_time, input_time)-1]
    ...
    CB_indices = np.where(time_since_CB <= n_seconds_after_crossing_boundary)[0]
   
    if n_seconds_before_crossing_boundary > 0:
        corresponding_CB_time_indices = np.searchsorted(crossing_boundary_time, input_time)
        # make sure the indices for corresponding_CB_time don't go out of bound
        corresponding_CB_time_indices[corresponding_CB_time_indices >= len(crossing_boundary_time)] = len(crossing_boundary_time) - 1
        ...
        CB_indices = np.append(CB_indices, np.where(time_before_CB <= n_seconds_before_crossing_boundary)[0])
...
def make_pred_ff_indices_dict(moit2, y_pred_all):
    ...
    if y_pred_all is None:
        ...
        nn_model.eval()
        y_pred_all = nn_model(torch.tensor(moit2.X_all_sc, dtype=torch.float32))
        y_pred_all = y_pred_all > 0.5

    for i in range(len(moit2.chosen_rows_of_df)):
        starting_point_index = int(moit2.chosen_rows_of_df.iloc[i]['starting_point_index'].item())
...
def make_anno_ff_indices_dict(moit2, add_negative_labels=True):
    ...
    for i in range(len(moit2.chosen_rows_of_df)):
        ...
        if add_negative_labels:
            ...
            if anno_ff.size == 0:
                anno_ff = moit2.manual_anno_long.loc[moit2.manual_anno_long['starting_point_index'] == starting_point_index, 'ff_index'].item()               
                anno_ff = np.array([anno_ff])
                if len(anno_ff) > 1:
                    print('Problem! anno_ff = ', anno_ff, 'but there shouldn\'t be more than one ff')
...
def make_anno_and_pred_ff_indices_dict(moit2, y_pred_all=None, add_negative_labels=True):
    ...
    anno_ff_indices_dict = make_anno_ff_indices_dict(moit2, add_negative_labels=add_negative_labels)
...
def make_pseudo_manual_anno(best_arc_df, monkey_information, ff_caught_T_sorted):
    ...
    pseudo_manual_anno = pseudo_manual_anno[['ff_index', 'starting_point_index', 'time', 'target_index']]


    pseudo_manual_anno_long = pseudo_manual_anno_long[['ff_index', 'starting_point_index', 'time', 'target_index']]
    pseudo_manual_anno_long['original_starting_point_index'] = pseudo_manual_anno_long['starting_point_index'] # since all data are valid, each point is its own original starting point
