...
class ModelOfIntendedTargets(decision_making_class.DecisionMaking):
    ...
    def get_manual_anno_long(self):
        super().retrieve_manual_anno()
        self.manual_anno_trimmed = self.manual_anno[self.manual_anno.target_index != self.manual_anno.target_index.min()]
        self.manual_anno_trimmed = self.manual_anno_trimmed[self.manual_anno_trimmed.target_index != self.manual_anno_trimmed.target_index.min()]
        ...
        self.manual_anno_long = self.manual_anno_long.set_index('starting_point_index')
        self.manual_anno_long = self.manual_anno_long.reindex(range(min_point, max_point+1))
        ...
        self.manual_anno_long['starting_point_index'] = self.manual_anno_long['starting_point_index'].astype('int')
        self.manual_anno_long['target_index'] = self.manual_anno_long['target_index'].astype('int')
        self.manual_anno_long['ff_index'] = self.manual_anno_long['ff_index'].astype('int')
    ...
    def update_invalid_ff_indices_in_manual_anno_long(self):
        ...
        manual_anno_long.loc[manual_anno_long['sub_ff_index'] < 0, 'sub_ff_index'] = np.nan
        # fill NA with backward fill
        manual_anno_long['sub_ff_index'] = manual_anno_long['sub_ff_index'].fillna(method='bfill')
        # And then, in case there are leftover NAs near the end of the df, we fill it with -9
        manual_anno_long['sub_ff_index'] = manual_anno_long['sub_ff_index'].fillna(-9)
        manual_anno_long['sub_ff_index'] = manual_anno_long['sub_ff_index'].astype(int)
        ...
        for ff_index in valid_sub_ff_index:
            ...
            ff_time = self.ff_dataframe[self.ff_dataframe['ff_index'] == ff_index]['time']
            ...
            manual_anno_long_invalid_indices = manual_anno_long[(manual_anno_long['sub_ff_index'] == ff_index) & (~manual_anno_long['time'].isin(ff_time))].index.values
            if len(manual_anno_long_invalid_indices) > 0:
                ...
                manual_anno_long.loc[manual_anno_long_invalid_indices, 'sub_ff_index'] = manual_anno_long.loc[manual_anno_long_invalid_indices, 'ff_index']
            
        # Also change the invalid ff_index back to the original numbers
        manual_anno_long.loc[manual_anno_long['sub_ff_index'] < 0, 'sub_ff_index'] = manual_anno_long.loc[manual_anno_long['sub_ff_index'] < 0, 'ff_index']
        ...
        print('Percentage of manual_anno_long has invalid ff_index: ', round(len(manual_anno_long[manual_anno_long['ff_index'] < 0])/len(manual_anno_long)*100, 1), '%')
        print('Out of the above:')
        print('Percentage of invalid ff_index that has been updated: ', round(len(manual_anno_long[(manual_anno_long['ff_index'] < 0) & (manual_anno_long['sub_ff_index'] >= 0)])/len(manual_anno_long[manual_anno_long['ff_index'] < 0])*100, 1), '%')
        print('percentage of invalid ff_index that has not been updated: ', round(len(manual_anno_long[(manual_anno_long['ff_index'] < 0) & (manual_anno_long['sub_ff_index'] < 0)])/len(manual_anno_long[manual_anno_long['ff_index'] < 0])*100, 1), '%')        

        self.manual_anno_long['ff_index'] = manual_anno_long['sub_ff_index']



    def get_input_data(self, num_ff_per_row=5, keeping_1_out_of_n_rows=1, add_arc_info=False, arc_info_to_add=['optimal_curvature', 'curv_diff'], curvature_df=None, curv_of_traj_df=None, **kwargs):
        self.free_selection_df = self.manual_anno_long
        super().get_free_selection_data(num_ff_per_row=num_ff_per_row, keeping_1_out_of_n_rows=keeping_1_out_of_n_rows, add_arc_info=add_arc_info, arc_info_to_add=arc_info_to_add, curvature_df=curvature_df, curv_of_traj_df=curv_of_traj_df, **kwargs)
...
class ModelOfMultipleIntendedTargets(ModelOfIntendedTargets):
    ...
    def prepare_data_for_machine_learning(self, furnish_with_trajectory_data=True, trajectory_data_kind="position", allow_multi_label=True): 
        super().prepare_data_for_machine_learning(furnish_with_trajectory_data=furnish_with_trajectory_data, trajectory_data_kind=trajectory_data_kind)
...
def test_moit_hyperparameters(ff_dataframe, ff_caught_T_sorted, ff_real_position_sorted, monkey_information, ff_flash_sorted, ff_life_sorted, pseudo_manual_anno, pseudo_manual_anno_long,
    ...
    moit.split_data_to_train_and_test(scaling_data=True, keep_whole_chunks=keep_whole_chunks)
