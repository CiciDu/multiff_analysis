...
def make_one_polar_plot(max_time_before_next_visible_to_annotate=2.5, **current_polar_plot_kargs):
    ...
    if show_direction_of_monkey_on_trajectory:
        ax = monkey_heading_functions.plot_triangles_to_show_monkey_headings_in_polar(
            ax, current_mheading)
...
def get_current_polar_plot_kargs(i, max_time_since_last_vis=2.5, show_reward_boundary=False, ff_colormap='Greens', null_arcs_bundle=None,
    ...
    if show_direction_of_monkey_on_trajectory:
        current_mheading = monkey_heading_functions.find_current_mheading_for_the_point(
            mheading, i)
    else:
