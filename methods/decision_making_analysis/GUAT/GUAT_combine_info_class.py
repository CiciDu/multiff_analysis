...
class GUATCombineInfoAcrossSessions(GUAT_helper_class.GUATHelperClass):
    ...
    def retrieve_or_make_combined_info(self, gc_kwargs, 
        ...
        if combined_info_exists_ok:
            self.combined_info, self.collect_info_flag = combine_info_utils.try_to_retrieve_combined_info(self.combd_GUAT_info_folder_path, df_names=self.df_names)
        else:
    ...
    def collect_combined_info_for_GUAT(self, gc_kwargs, 
        ...
        self.combined_info = combine_info_utils.turn_all_important_info_into_combined_info(self.all_important_info, self.combd_GUAT_info_folder_path, save_each_df_as_csv=True)
    ...
    def find_all_traj_feature_names(self, gc_kwargs, traj_point_features=['monkey_distance', 'monkey_angle']):
        all_traj_feature_names = trajectory_info.make_all_traj_feature_names(time_range_of_trajectory=gc_kwargs['time_range_of_trajectory'], num_time_points_for_trajectory=gc_kwargs['num_time_points_for_trajectory'], 
                                                                time_range_of_trajectory_to_plot=gc_kwargs['time_range_of_trajectory_to_plot'], num_time_points_for_trajectory_to_plot=gc_kwargs['num_time_points_for_trajectory_to_plot'],
...
class GUATProcessCombinedInfo(GUATCombineInfoAcrossSessions):


    def __init__(self, gc_kwargs, monkey_name='monkey_Bruno'):
        super().__init__(monkey_name=monkey_name)
        self.gc_kwargs = gc_kwargs


    def reload_combined_info_back_to_self(self, combined_info=None, all_traj_feature_names=None):
        ...
        self.relevant_curv_of_traj_df = self.traj_data_df.loc[:, self.all_traj_feature_names['relevant_curv_of_traj']]
        self.relevant_curv_of_traj_df['point_index'] = self.traj_data_df['point_index'].copy()
        self.more_traj_points_df = self.more_traj_data_df.loc[:, self.all_traj_feature_names['more_traj_points']]
        self.more_traj_stops_df = self.more_traj_data_df.loc[:, self.all_traj_feature_names['more_traj_stops']]

        self.num_stops_df = self.GUAT_current_ff_info_reloaded[['point_index', 'num_stops']].drop_duplicates().copy()
    ...
    def process_current_and_alternative_ff_info(self):
        ...
        self.GUAT_current_ff_info = curvature_utils.fill_up_NAs_for_placeholders_in_columns_related_to_curvature(self.GUAT_current_ff_info, curv_of_traj_df=self.relevant_curv_of_traj_df)
        self.GUAT_alt_ff_info = curvature_utils.fill_up_NAs_for_placeholders_in_columns_related_to_curvature(self.GUAT_alt_ff_info, curv_of_traj_df=self.relevant_curv_of_traj_df)
    ...
    def find_input_and_output(self,
        ...
        # find free_selection_inputs_df and use it as input for machine learning
        ...
        if add_num_ff_in_cluster:
            ...
            self.free_selection_inputs_df['num_alt_ff_in_cluster'] = self.GUAT_alt_ff_info.groupby('point_index').first()['num_ff_in_cluster'].values

        self.free_selection_time = self.GUAT_joined_ff_info.set_index('point_index').loc[self.point_index_array, 'time'].values
    ...
    def prepare_data_for_machine_learning(self, furnish_with_trajectory_data=True,
        ...
        super().prepare_data_for_machine_learning(furnish_with_trajectory_data=False, add_traj_stops=False)

        if furnish_with_trajectory_data: # manually add trajectory and stops info
            self.furnish_with_trajectory_data = True
            self.X_all = np.concatenate([self.X_all, self.traj_points_df.values], axis=1)
            self.X_all = np.concatenate([self.X_all, self.traj_stops_df.values], axis=1)
            # add the feature names
            self.input_features = np.concatenate([self.input_features, self.traj_points_df.columns], axis=0)
            self.input_features = np.concatenate([self.input_features, self.traj_stops_df.columns], axis=0)
        ...
        if traj_ff_attributes_to_add is not None:
            ...
            for attribute in traj_ff_attributes_to_add:
                ...
                trajectory_feature_names = trajectory_info.generate_feature_names_given_relative_time_points(relative_time_points_of_trajectory, num_time_points_for_trajectory, original_feature_names=[attribute+'_current', attribute+'_alt'])
                ...
                traj_ff_info_temp = self.traj_ff_info[self.traj_ff_info['ff_number'].isin([1, 101])]
                # collapse the data belonging to the same point_index into the same row, in the wide format
                traj_ff_info_temp = traj_ff_info_temp.pivot(index='point_index', columns='traj_relative_index', values=attribute)
                self.X_all = np.concatenate([self.X_all, traj_ff_info_temp.values], axis=1)
                self.input_features = np.concatenate([self.input_features, trajectory_feature_names], axis=0)


    def add_additional_info_to_plot(self):
        self.more_ff_df = cluster_replacement_utils.eliminate_part_of_more_ff_inputs_already_in_observation(self.more_ff_df, self.sequence_of_obs_ff_indices, self.point_index_all)
        self.more_ff_inputs_df_for_plotting = cluster_replacement_utils.turn_more_ff_df_into_free_selection_inputs_df_for_plotting(self.more_ff_df, self.point_index_all, attributes_for_plotting=self.attributes_for_plotting)
