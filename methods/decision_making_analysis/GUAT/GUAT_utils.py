...
def make_one_stop_w_ff_df(one_stop_df):
    ...
    one_stop_w_ff_df['stop_indices'] = one_stop_w_ff_df['point_index'].apply(lambda x: [x])
...
def get_filtered_stops_df(distinct_stops_df, min_distance_from_other_stops=100):
    # take out stops that are not within 75 cm distance of any other stop 
    delta_x_from_last_stop = distinct_stops_df['monkey_x'].diff().fillna(min_distance_from_other_stops * 2)
    delta_y_from_last_stop = distinct_stops_df['monkey_y'].diff().fillna(min_distance_from_other_stops * 2)
    delta_x_from_next_stop = - distinct_stops_df['monkey_x'].diff(-1).fillna(min_distance_from_other_stops * 2)
    delta_y_from_next_stop = - distinct_stops_df['monkey_y'].diff(-1).fillna(min_distance_from_other_stops * 2)
    distinct_stops_df['distance_from_last_stop'] = np.sqrt(delta_x_from_last_stop ** 2 + delta_y_from_last_stop ** 2)
    distinct_stops_df['distance_from_next_stop'] = np.sqrt(delta_x_from_next_stop ** 2 + delta_y_from_next_stop ** 2)
    filtered_stops_df = distinct_stops_df[(distinct_stops_df['distance_from_last_stop'] > min_distance_from_other_stops) & \
                                            (distinct_stops_df['distance_from_next_stop'] > min_distance_from_other_stops)].copy()
...
def filter_stops_based_on_distance_to_ff_capture(filtered_stops_df, monkey_information, ff_caught_T_new, min_cum_distance_to_ff_capture=100):
    ...
    filtered_stops_df = filtered_stops_df[filtered_stops_df['closest_cum_distance_to_ff_capture'] > min_cum_distance_to_ff_capture].copy()
...
def get_one_stop_df(filtered_stops_df, ff_dataframe, min_distance_to_ff=25, max_distance_to_ff=50):
    ...
    # from the filtered stops, take out the stops that are within n cm of a ff (use ff_dataframe to check)

    # drop the columns in ff_dataframe that are also in one_stop_df except for 'point_index'
    ff_dataframe_temp = ff_dataframe.drop(columns=ff_dataframe.columns.intersection(filtered_stops_df.columns).difference(['point_index']))

    one_stop_df = filtered_stops_df.merge(ff_dataframe_temp, on='point_index', how='left')

    # eliminate point_index in one_stop_df if min ff_distance < min_distance_to_ff
    grouped_df = one_stop_df.groupby('point_index').min()
    point_index_to_eliminate = grouped_df[grouped_df['ff_distance'] < min_distance_to_ff].index.values
    one_stop_df = one_stop_df[~one_stop_df['point_index'].isin(point_index_to_eliminate)].copy()

    # eliminate ff that are too far away from the stop
...
def get_GUAT_w_ff_df(GUAT_indices_df, 
    ...
    GUAT_ff_info2 = GUAT_ff_info[['cluster_index', 'ff_index']].drop_duplicates().groupby('cluster_index')['ff_index'].apply(list).reset_index(drop=False)
    ...
    GUAT_expanded_trials_df['whether_w_ff_near_stops'] =  (~GUAT_expanded_trials_df['nearby_alive_ff_indices'].isna()).values.astype(int)
    GUAT_w_ff_df = GUAT_expanded_trials_df[GUAT_expanded_trials_df['whether_w_ff_near_stops']==1].reset_index(drop=True)
