...
def streamline_getting_one_stop_df(monkey_information, ff_dataframe, ff_caught_T_new, min_distance_from_adjacent_stops=75, 
    ...
    filtered_stops_df = get_filtered_stops_df(distinct_stops_df, min_distance_from_adjacent_stops)
...
def get_filtered_stops_df(distinct_stops_df, min_distance_from_adjacent_stops):
    ...
    delta_x_from_last_stop = distinct_stops_df['monkey_x'].diff().fillna(min_distance_from_adjacent_stops * 2)
    delta_y_from_last_stop = distinct_stops_df['monkey_y'].diff().fillna(min_distance_from_adjacent_stops * 2)
    delta_x_from_next_stop = - distinct_stops_df['monkey_x'].diff(-1).fillna(min_distance_from_adjacent_stops * 2)
    delta_y_from_next_stop = - distinct_stops_df['monkey_y'].diff(-1).fillna(min_distance_from_adjacent_stops * 2)
    ...
    distinct_stops_df['min_distance_from_adjacent_stops'] = distinct_stops_df[['distance_from_last_stop', 'distance_from_next_stop']].min(axis=1)
    filtered_stops_df = distinct_stops_df[(distinct_stops_df['min_distance_from_adjacent_stops'] > min_distance_from_adjacent_stops)].copy()
