...
def find_GUAT_current_ff_info(GUAT_w_ff_df, ff_real_position_sorted, ff_life_sorted, ff_dataframe, monkey_information, include_ff_in_near_future=False, 
    ...
    GUAT_current_ff = pd.DataFrame({'ff_index': list_of_ff_index,
                                'point_index': list_of_point_index,
                                'target_index': list_of_target_index
                                })
    
    GUAT_current_ff = GUAT_current_ff.astype(int)
    
    GUAT_current_ff = GUAT_current_ff.merge(GUAT_w_ff_df[['target_index', 'point_index', 'time', 'num_stops', 'first_stop_point_index', 'total_stop_time']], 
    ...
    if include_ff_in_near_future:
        ...
        GUAT_current_ff_info = decision_making_utils.find_many_ff_info_anew(GUAT_current_ff_info['ff_index'].values, GUAT_current_ff_info['point_index'].values, ff_real_position_sorted, ff_dataframe_visible, monkey_information, add_time_till_next_visible=True)

        # now we need to add back some columns
        columns_to_add_back = GUAT_current_ff[['point_index', 'num_stops', 'total_stop_time']].drop_duplicates()
    ...
    ff_x, ff_y = ff_real_position_sorted[GUAT_current_ff_info['ff_index'].values].T
    monkey_x, monkey_y = monkey_information.loc[GUAT_current_ff_info['first_stop_point_index'].values, ['monkey_x', 'monkey_y']].values.T
    GUAT_current_ff_info['distance_to_monkey'] = np.sqrt((ff_x - monkey_x)**2 + (ff_y - monkey_y)**2)
    GUAT_current_ff_info['distance_to_stop'] = GUAT_current_ff_info['distance_to_monkey'] # since distance_to_monkey's reference point is the stop
    GUAT_current_ff_info = GUAT_current_ff_info[GUAT_current_ff_info['distance_to_stop'] < max_distance_to_stop].copy()

    GUAT_current_ff_info = GUAT_current_ff_info.drop_duplicates(subset=['point_index', 'ff_index'], keep='first').reset_index(drop=True)

    return GUAT_current_ff_info
...
def find_GUAT_alt_ff_info(GUAT_current_ff_info, ff_dataframe, ff_real_position_sorted, monkey_information, 
    ...
    if include_ff_in_near_future:
        unique_point_index_and_time_df = GUAT_current_ff_info[['point_index', 'time', 'total_stop_time']].drop_duplicates()
        ...
        GUAT_alt_ff_info = GUAT_alt_ff_info.merge(GUAT_current_ff_info[['point_index', 'total_stop_time']], on='point_index', how='left')

        GUAT_alt_ff_info.sort_values(by=['point_index', 'ff_index', 'distance_to_monkey'], inplace=True)
        GUAT_alt_ff_info = GUAT_alt_ff_info.drop_duplicates(subset=['point_index', 'ff_index'], keep='first').reset_index(drop=True)
...
def retain_useful_current_and_alt_info(GUAT_current_ff_info, GUAT_alt_ff_info, eliminate_cases_with_close_alt_ff=True,
                                       min_alt_ff_distance_to_stop=0):
    ## we need to eliminate the info of the ff in GUAT_alt_ff_info that's also in GUAT_current_ff_info at the same point indices
    # GUAT_alt_ff_info = retain_rows_in_df1_that_share_or_not_share_columns_with_df2(GUAT_alt_ff_info, GUAT_current_ff_info, columns=['point_index', 'ff_index'], whether_share=False)
    
    # then, we eliminate the cases where GUAT_alt_ff_info has at least one ff that's within 150 to the current point, because that creates confounding factors
    if eliminate_cases_with_close_alt_ff:
        GUAT_alt_ff_info = GUAT_alt_ff_info[GUAT_alt_ff_info['ff_distance'] > min_alt_ff_distance_to_stop].copy()

    # also eliminate alt_ff if it's at the back of the monkey
    GUAT_alt_ff_info = GUAT_alt_ff_info[GUAT_alt_ff_info['ff_angle_boundary'].between(-90*math.pi/180, 90*math.pi/180)].copy()
    
    # then, we eliminate the info in GUAT_current_ff_info that does not have corresponding info in GUAT_alt_ff_info (with the same point_index)
    GUAT_current_ff_info = retain_rows_in_df1_that_share_or_not_share_columns_with_df2(GUAT_current_ff_info, GUAT_alt_ff_info, columns=['point_index'], whether_share=True)
    return GUAT_current_ff_info, GUAT_alt_ff_info


def make_sure_GUAT_alt_ff_info_and_GUAT_current_ff_info_have_the_same_point_indices(GUAT_current_ff_info, GUAT_alt_ff_info):
    GUAT_current_ff_info = GUAT_current_ff_info[GUAT_current_ff_info['point_index'].isin(GUAT_alt_ff_info['point_index'].values)].copy()
    GUAT_alt_ff_info = GUAT_alt_ff_info[GUAT_alt_ff_info['point_index'].isin(GUAT_current_ff_info['point_index'].values)].copy()
    return GUAT_current_ff_info, GUAT_alt_ff_info
...
def polish_GUAT_alt_ff_info(GUAT_alt_ff_info, GUAT_current_ff_info, ff_real_position_sorted, ff_life_sorted, ff_dataframe, monkey_information, 
    ...
    if take_one_row_for_each_point_and_find_cluster:
        # take the optimal ff from GUAT_alt_ff_info based on columns_to_sort_alt_ff_by
        GUAT_alt_ff_info = take_optimal_row_per_group_based_on_columns(GUAT_alt_ff_info, columns_to_sort_alt_ff_by, groupby_column='point_index')
        
        # now, let's re-find GUAT_alt_ff_info by considering clusters
        GUAT_alt_ff_info_old = GUAT_alt_ff_info.copy()
        GUAT_alt_ff_info = supply_info_of_cluster_to_df(GUAT_alt_ff_info, ff_real_position_sorted, ff_life_sorted, monkey_information, max_cluster_distance=max_cluster_distance)
    else:
        GUAT_alt_ff_info_old = GUAT_alt_ff_info.copy()
...
def _add_num_ff_in_cluster(df):
    num_ff_in_cluster = df.groupby('point_index').size().reset_index(drop=False)
...
def add_curv_diff_and_ff_number_to_GUAT_current_ff_info_and_GUAT_alt_ff_info(GUAT_current_ff_info, GUAT_alt_ff_info, ff_caught_T_new, ff_real_position_sorted, monkey_information, curv_of_traj_df=None,
    ...
    GUAT_alt_ff_info = decision_making_utils.add_curv_diff_to_df(GUAT_alt_ff_info, monkey_information, curv_of_traj_df, ff_real_position_sorted=ff_real_position_sorted)

    GUAT_current_ff_info.sort_values(by=['point_index', ff_priority_criterion], inplace=True)
    GUAT_alt_ff_info.sort_values(by=['point_index', ff_priority_criterion], inplace=True)
    
    # assign a ff_number to each ff within each point_index
    GUAT_current_ff_info['ff_number'] = GUAT_current_ff_info.groupby('point_index').cumcount()+1
    GUAT_alt_ff_info['ff_number'] = GUAT_alt_ff_info.groupby('point_index').cumcount()+101
...
def find_additional_ff_info_for_near_future(unique_point_index_and_time_df, ff_dataframe, ff_real_position_sorted, monkey_information, duration_into_future=0.5,
    ...
    all_available_ff_in_near_future = find_available_ff_in_near_future(unique_point_index_and_time_df, ff_dataframe, duration_into_future=duration_into_future)
...
def find_available_ff_in_near_future(unique_point_index_and_time_df, ff_dataframe, duration_into_future=0.5):
    ...
    for index, row in unique_point_index_and_time_df.iterrows():
        ...
        ff_dataframe_subset0 = ff_dataframe[(ff_dataframe['ff_starting_visible_time'] >= duration[0]) & (ff_dataframe['ff_starting_visible_time'] <= duration[1])].copy()
        ...
        ff_dataframe_subset = ff_dataframe[ff_dataframe['time'].between(duration[0], duration[1] + 5)].copy()
        ff_dataframe_subset = ff_dataframe_subset[ff_dataframe_subset['ff_index'].isin(selected_ff)].copy()
        ff_dataframe_subset['distance_to_monkey'] = ff_dataframe_subset['ff_distance']
        ff_dataframe_subset.sort_values(by=['ff_index', 'distance_to_monkey'], inplace=True)
        available_ff = ff_dataframe_subset.groupby('ff_index').first().reset_index()
        if len(available_ff) > 0:
            available_ff_sub = available_ff[['ff_index', 'ff_distance']].copy()
            available_ff_sub['point_index'] = point_index
            all_available_ff_in_near_future = pd.concat([all_available_ff_in_near_future, available_ff_sub], axis=0)
    all_available_ff_in_near_future['ff_index'] = all_available_ff_in_near_future['ff_index'].astype(int)
    all_available_ff_in_near_future['point_index'] = all_available_ff_in_near_future['point_index'].astype(int)
...
def find_curv_diff_for_ff_info(ff_info, monkey_information, ff_real_position_sorted, curv_of_traj_df=None):
    if curv_of_traj_df is None:
        raise ValueError('curv_of_traj_df is None. Please provide curv_of_traj_df.')
    ...
    ff_info = ff_info.merge(temp_curvature_df[['ff_index', 'point_index', 'curv_diff']].drop_duplicates(), on=['ff_index', 'point_index'], how='left')
...
def update_point_index_of_important_df_in_important_info_func(important_info, new_point_index_start, point_index_column_name='point_index'):
    ...
    point_index_to_new_number_df = pd.DataFrame({point_index_column_name: unique_point_index, 'new_number': range(new_point_index_start, new_point_index_start+len(unique_point_index))})
    # print('The following are the point indices and their new numbers:')
    # print(point_index_to_new_number_df)
    point_index_to_new_number_df.set_index(point_index_column_name, inplace=True)
    for df_name, df in important_info.items():
        if old_point_index_column_name not in important_info[df_name].columns: # then the df has not been updated before
...
def find_ff_aimed_at_through_manual_annotation(relevant_indices, monkey_information, manual_anno):
    ...
    manual_anno_sub = manual_anno[manual_anno['starting_point_index'] <= max(relevant_indices)]
    manual_anno_sub_during_interval = manual_anno_sub[manual_anno_sub['starting_point_index'].between(min(relevant_indices), max(relevant_indices))]
    ...
    manual_anno_sub_earlier = manual_anno_sub[manual_anno_sub['starting_point_index'] < min(relevant_indices)]
    if len(manual_anno_sub_earlier) > 0:
        manual_anno_sub_earlier = manual_anno_sub_earlier[manual_anno_sub_earlier['starting_point_index'] == max(manual_anno_sub_earlier.starting_point_index.values)]
        manual_anno_sub = pd.concat([manual_anno_sub_earlier, manual_anno_sub_during_interval])
    else:
...
def find_alive_ff_close_to_monkey(relevant_indices, monkey_information, ff_life_sorted, ff_real_position_sorted, max_ff_distance_to_include = 75):
    ...
    sampled_indices = relevant_indices[np.linspace(0, len(relevant_indices)-1, 5, dtype=int, endpoint=True)]
    ...
    for position in sampled_monkey_positions:
        ...
        close_ff_indices = np.concatenate([close_ff_indices, alive_ff_indices[ff_distance < max_ff_distance_to_include]])
