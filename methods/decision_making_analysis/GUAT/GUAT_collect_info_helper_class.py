...
class GUATCollectInfoHelperClass(GUAT_helper_class.GUATHelperClass):
    ...
    def find_current_and_alternative_ff_info(self, 
        ...
        ## The below can be replaced now because we're using the original GUAT_current_ff_info + GUAT_alt_ff_info as more_ff_df
    ...
    def set_time_of_eval(self, GUAT_w_ff_df, time_with_respect_to_first_stop=None, time_with_respect_to_second_stop=None, time_with_respect_to_last_stop=None):
        ...
        if time_with_respect_to_first_stop is not None:
            ...
            self.time_of_eval = self.GUAT_w_ff_df['second_stop_time'] + time_with_respect_to_second_stop
        else:
    ...
    def eliminate_crossing_boundary_cases(self, n_seconds_before_crossing_boundary=None, n_seconds_after_crossing_boundary=None):
        ...
        self.GUAT_w_ff_df = self.GUAT_w_ff_df.iloc[non_CB_indices].reset_index(drop=True)          
    ...
    def retrieve_or_make_GUAT_trials_df(self, exists_ok=True):
        ...
        if exists(filepath) & exists_ok:
            ...
            self.GUAT_trials_df = GUAT_utils.make_GUAT_trials_df(self.GUAT_w_ff_df, self.ff_life_sorted, self.ff_real_position_sorted, self.monkey_information)
    ...
    def make_curvature_df(self, curv_of_traj_df=None, clean=True):
        ...
        self.curvature_df = curvature_utils.make_curvature_df(self.ff_dataframe, curv_of_traj_df, clean=clean)
    ...
    def add_curvature_info_to_ff_dataframe(self, column_exists_ok=False):
        ...
        if 'curv_diff' not in self.ff_dataframe.columns or column_exists_ok is False:
            curvature_df_sub = self.curvature_df[['ff_index', 'point_index', 'curv_diff']]
            self.ff_dataframe = pd.merge(self.ff_dataframe, curvature_df_sub, on=['ff_index', 'point_index'], how='left')
    ...
    def get_more_ff_df(self):
        ...
        self.more_ff_df = pd.concat([self.GUAT_current_ff_info, self.GUAT_alt_ff_info], axis=0).reset_index(drop=True)
        self.more_ff_df.drop_duplicates(subset=['point_index', 'ff_index'], inplace=True)
    ...
    def update_curvature_df_with_additional_curvature_df(self):
        self.curvature_df = pd.concat([self.curvature_df, self.additional_curvature_df], axis=0)
    ...
    def get_trajectory_and_stop_info_for_machine_learning(self):
       ...
       '''
        ...
        self.traj_points_df = pd.DataFrame(traj_points, columns=trajectory_feature_names)
        self.traj_stops_df = pd.DataFrame(traj_stops, columns=temp_trajectory_feature_names)
    ...
    def get_more_trajectory_info_for_plotting(self,
        ...
        if time_range_of_trajectory_to_plot is not None:
            ...
            more_traj_stops, temp_trajectory_feature_names = trajectory_info.generate_stops_info(self.time_all, self.monkey_information, time_range_of_trajectory=time_range_of_trajectory_to_plot, num_time_points_for_trajectory=num_time_points_for_trajectory_to_plot)
            self.more_traj_points_df = pd.DataFrame(more_traj_points, columns=trajectory_feature_names)
            self.more_traj_stops_df = pd.DataFrame(more_traj_stops, columns=temp_trajectory_feature_names)
    ...
    def get_traj_ff_info(self):
        ...
        GUAT_current_ff_info = self.GUAT_current_ff_info.copy()
        GUAT_alt_ff_info = self.GUAT_alt_ff_info.copy()

        GUAT_current_ff_info.columns

        GUAT_ff_info_combined = pd.concat([GUAT_current_ff_info[['point_index', 'ff_index', 'ff_number']], GUAT_alt_ff_info[['point_index', 'ff_index', 'ff_number']]], axis=0)
        ...
        for i in range(len(self.point_index_all)):
            ...
            relevant_ff = GUAT_ff_info_combined[GUAT_ff_info_combined['point_index']==current_point_index]

            for index, row in relevant_ff.iterrows():
                ff = row['ff_index']
                ff_number = row['ff_number']
                num_traj_points = len(traj_point_index_array)
                list_of_point_index.extend([current_point_index] * num_traj_points)
                ...
                traj_relative_index = np.arange((ff_number-1)*num_traj_points, ff_number*num_traj_points)
        ...
        try:
            ...
            print('ff_info and new_traj_df have different lengths. ff_info:', ff_info.shape, 'new_traj_df:', new_traj_df.shape)
            print('ff_info head:')
            print(ff_info.head())
        ...
        self.traj_ff_info = ff_info.sort_values(by=['point_index', 'ff_number', 'traj_point_index']).reset_index(drop=True)


    def add_arc_info_to_each_df_of_ff_info(self, curvature_df):
        # add best_arc_info to GUAT_current_ff_info and GUAT_alt_ff_info
        ...
        for column in arc_info:
            if column in self.GUAT_current_ff_info.columns:
                self.GUAT_current_ff_info = self.GUAT_current_ff_info.drop([column], axis=1)
            if column in self.GUAT_alt_ff_info.columns:
                self.GUAT_alt_ff_info = self.GUAT_alt_ff_info.drop([column], axis=1)
