...
def prepare_binned_spikes_df(spike_df, bin_width=0.02):
    ...
    """
    time_bins, binned_spikes_matrix = _make_all_binned_spikes(spike_df, bin_width)
...
def add_lags_to_each_feature(var, lag_numbers, trial_vector=None, rearrange_lag_based_on_abs_value=True):
    ...
    if trial_vector is None:
        ...
        var_lags = var.groupby('lag_segment_id', group_keys=False).apply(lag_group)
...
def get_mapping_table_between_hard_drive_and_local_folders(monkey_name, hdrive_dir, neural_data_folder_name, filter_neural_file_func, save_table=False):
    ...
    if save_table:
        ...
        mapping_table.to_csv(f'/Users/dusiyi/Documents/Multifirefly-Project/multiff_analysis/methods/eye_position_analysis/neural_data_analysis/get_neural_data/MATLAB_processing/{monkey}_mapping_table.csv',
                             index=False)
