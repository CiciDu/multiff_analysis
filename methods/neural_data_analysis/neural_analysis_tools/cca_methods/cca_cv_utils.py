...
def crossvalidated_cca_analysis(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X1)):
        ...
        # Store CCA object and compute canonical correlations
        fold_cca_objects.append(cca)
        tr_corrs, te_corrs = cca.validate(
            [X1_tr, X2_tr]), cca.validate([X1_te, X2_te])

        # Compute mean canonical correlation for this fold (average across components)
        mean_canon_corr = np.mean(tr_corrs[0])  # Using training correlations
        fold_canonical_corrs.append(mean_canon_corr)
        ...
        fold_loadings.append({
            'X1_train': np.corrcoef(X1_tr.T, V_tr.T)[:X1.shape[1], X1.shape[1]:],
            'X2_train': np.corrcoef(X2_tr.T, U_tr.T)[:X2.shape[1], X2.shape[1]:],
            'X1_test': np.corrcoef(X1_te.T, V_te.T)[:X1.shape[1], X1.shape[1]:],
            'X2_test': np.corrcoef(X2_te.T, U_te.T)[:X2.shape[1], X2.shape[1]:]
        })

        can_load_stats['X1_train'].append(fold_loadings[-1]['X1_train'])
        can_load_stats['X2_train'].append(fold_loadings[-1]['X2_train'])
        can_load_stats['X1_test'].append(fold_loadings[-1]['X1_test'])
        can_load_stats['X2_test'].append(fold_loadings[-1]['X2_test'])
    ...
    best_fold_idx = np.argmax(fold_canonical_corrs)
    best_fold_corr = fold_canonical_corrs[best_fold_idx]
    ...
    # Add information about the best fold
    can_load_stats['best_fold_idx'] = best_fold_idx
    can_load_stats['best_fold_canonical_corr'] = best_fold_corr
    can_load_stats['all_fold_canonical_corrs'] = fold_canonical_corrs
    can_load_stats['best_fold_cca'] = fold_cca_objects[best_fold_idx]
