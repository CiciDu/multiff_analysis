...
def concat_new_seg_info(df, new_seg_info, bin_width=0.2):
    df = df.sort_values(by='time')
    new_seg_info = new_seg_info.sort_values(by='new_segment')
    concat_seg_data = []

    for _, row in new_seg_info.iterrows():
        mask = (df['time'] >= row['new_seg_start_time']) & (df['time'] <= row['new_seg_end_time'])
        seg_df = df.loc[mask].copy()

        # Assign new bins relative to segment start
        seg_df['new_bin'] = ((seg_df['time'] - row['new_seg_start_time']) // bin_width).astype(int)
    ...
    result.sort_values(by=['new_segment', 'new_bin', 'time'], inplace=True)
...
def rebin_segment_data(df, new_seg_info, bin_width=0.2):
    ...
    concat_seg_data = concat_new_seg_info(df, new_seg_info, bin_width=bin_width)
...
def rebin_spike_data(spike_df, new_seg_info, bin_width=0.2):
    """
    Re-bin spike data based on new segment definitions and specified bin width.
    
    Args:
        spike_df (pd.DataFrame): Original spike data with 'time' and 'cluster' columns.
    ...
    concat_seg_data = concat_new_seg_info(spike_df, new_seg_info, bin_width=bin_width)
...
def _rebin_spike_data(concat_seg_data):
    ...
    full_index = pd.MultiIndex.from_product([segments, bins], names=['new_segment', 'new_bin'])

    rebinned_spike_data = rebinned_spike_data.reindex(index=full_index, columns=clusters, fill_value=0)

    # Clean column names
    rebinned_spike_data.columns.name = None
    rebinned_spike_data.columns = [f'unit_{i}' for i in range(rebinned_spike_data.shape[1])]
