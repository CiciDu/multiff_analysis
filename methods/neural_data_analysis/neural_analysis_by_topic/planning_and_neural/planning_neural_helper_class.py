...
class PlanningAndNeuralHelper(plan_factors_class.PlanFactors):
    ...
    def get_both_ff_across_time_df(self, exists_ok=True, n_seconds_before_stop=2, save_data=True):

        os.makedirs(self.planning_neural_folder_path, exist_ok=True)
        df_path = os.path.join(
            self.planning_neural_folder_path, 'both_ff_across_time_df.csv')
    ...
    def _check_for_duplicate_bins(self):
        if self.all_planning_info['bin'].duplicated().any():
            # retain the rows with the bigger stop_point_index
            self.all_planning_info = self.all_planning_info.sort_values(
