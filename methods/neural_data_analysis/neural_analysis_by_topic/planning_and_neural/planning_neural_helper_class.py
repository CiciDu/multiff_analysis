...
class PlanningAndNeuralHelper(plan_factors_class.PlanFactors):
    ...
    def __init__(self, test_or_control='test',
                 raw_data_folder_path=None, bin_width=0.02, window_width=0.25,
    ...
    def make_temporary_heading_info_df(self):
        ...
        self.cur_ff_df3 = self.add_d_heading_of_traj_to_df(
            self.cur_ff_df2)
        self.cur_and_nxt_ff_df2 = show_planning_utils.make_cur_and_nxt_ff_df(self.nxt_ff_df3, self.cur_ff_df3, include_arc_info=False)
    ...
    def add_heading_info_to_planning_data_by_point(self):
        ...
        print(f'Note: The following columns are not in the new heading_info_df: {set(all_columns_to_add) - set(self.heading_info_df.columns)}')
        all_columns_to_add = [col for col in all_columns_to_add if col in self.heading_info_df.columns]
    ...
    def get_both_ff_across_time_df(self, n_seconds_before_stop=2):
        ...
        all_info_to_add = self.collect_data_for_each_stop()
        ...
        self.both_ff_across_time_df = planning_neural_utils.add_angle_from_cur_arc_end_to_nxt_ff(
    ...
    def collect_data_for_each_stop(self):
    ...
    def _add_time_info_to_df(self, df):
        ...
        df['segment_start_time'] = self.monkey_information.loc[df['segment_start_point_index'].values, 'time'].values
        df['segment_end_time'] = self.monkey_information.loc[df['segment_end_point_index'].values, 'time'].values
        df['segment_duration'] = df['segment_end_time'] - df['segment_start_time']
        df['time_rel_to_stop'] = df['stop_time'] - df['time']
        time_columns = ['time', 'stop_time', 'segment_start_time',
                        'segment_end_time', 'segment_duration', 'time_rel_to_stop']
    ...
    def _add_basic_ff_info(self, df, which_ff_info):
        ...
        columns_added = [f'{which_ff_info}ff_angle',
                         f'{which_ff_info}ff_distance']
        return df, columns_added

    def _get_info_to_add(self, row):
        segment_start = max(
            self.ff_caught_T_new_point_index[row['cur_ff_index']-1], row['point_index_in_the_past'])
        segment_end = row['stop_point_index']

        # segment_start = max(
        #     self.ff_caught_T_new_point_index[row['cur_ff_index']], row['point_index_in_the_past'])
        # segment_end = row['next_stop_point_index']

        if segment_start > segment_end:
            # if the difference is less than 5, then simply continue. Otherwise, raise an error
        ...
        info_to_add = self.monkey_information[self.monkey_information['point_index'].between(
            segment_start, segment_end)].copy()
        ...
        info_to_add['segment_start_point_index'] = segment_start
        info_to_add['segment_end_point_index'] = segment_end
        return info_to_add
