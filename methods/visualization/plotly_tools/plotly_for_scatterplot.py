...
def make_the_initial_fig_scatter(curv_of_traj_df_in_duration, monkey_hoverdata_value, stop_ff_color, alt_ff_color, use_two_y_axes=True, change_y_ranges=True, add_vertical_line=True, 
    ...
    if trajectory_ref_row is not None:
        fig_scatter = mark_reference_point_in_scatter_plot(fig_scatter, x_column_name, trajectory_ref_row, y_range=y_range_for_v_line)
    if show_visible_segments:
        ...
        plot_lines_to_show_ff_visible_segments_in_fig_scatter(fig_scatter, visible_segments_info['ff_info'], visible_segments_info['monkey_information'], stops_near_ff_row,
                                                                unique_ff_indices=[stops_near_ff_row.stop_ff_index, stops_near_ff_row.alt_ff_index], time_or_distance=time_or_distance, y_range_for_v_line=y_range_for_v_line,
                                                                varying_colors = [stop_ff_color, alt_ff_color], ff_names=['stop ff', 'alt ff'])
    # plot a vertical line at stop point (which is 0)
    fig_scatter = add_vertical_line_for_an_x_value(fig_scatter, x_value=0, y_range=y_range_for_v_line, name='First stop point', color='black')
    if trajectory_next_stop_row is not None:
        fig_scatter = mark_next_stop_in_scatter_plot(fig_scatter, x_column_name, trajectory_next_stop_row, y_range_for_v_line=y_range_for_v_line)

    fig_scatter = add_annotation_to_fig_scatter(fig_scatter, 'stop point', 0, -130)
...
def mark_reference_point_in_scatter_plot(fig_scatter, x_column_name, trajectory_ref_row, y_range=[-200, 200]):
    ...
    vline_df = pd.DataFrame({'x': [ref_point_value, ref_point_value], 'y': y_range})
    fig_traces = px.line(vline_df, 
                        x= 'x', 
                        y= 'y',         
                        )
    ...
    fig_scatter = add_annotation_to_fig_scatter(fig_scatter, 'ref point', ref_point_value, -130)
    return fig_scatter
...
def mark_next_stop_in_scatter_plot(fig_scatter, x_column_name, trajectory_next_stop_row, y_range_for_v_line=[-200, 200]):
    ...
    fig_scatter = add_vertical_line_for_an_x_value(fig_scatter, x_value=next_stop_value, y_range=y_range_for_v_line, name='Next stop point', color='black')
    return fig_scatter
...
def make_new_trace_for_scatterplot(ff_curv_df, name, color='purple', x_column_name='rel_time', y_column_name='curv_to_ff_center', size=5, symbol='circle', showlegend=True):
    plot_to_add = go.Scatter(x=ff_curv_df[x_column_name], y=ff_curv_df[y_column_name],   
                                name=name,
                                legendgroup=name,
                                marker=dict(color=color, size=size, opacity=0.8, symbol=symbol),  
...
def add_to_the_scatterplot(fig, ff_curv_df, name, color='purple', x_column_name='rel_time', y_column_name='curv_to_ff_center', symbol='circle'):
    plot_to_add = make_new_trace_for_scatterplot(ff_curv_df, name, color=color, x_column_name=x_column_name, y_column_name=y_column_name, symbol=symbol)
...
def find_monkey_hoverdata_value_for_both_fig_scatter(hoverdata_column, monkey_hoverdata_value, trajectory_df):
    if hoverdata_column == 'rel_time':
        ...
        trajectory_hover_row = trajectory_df.loc[trajectory_df['rel_time'] >= monkey_hoverdata_value_s]
        ...
        trajectory_hover_row = trajectory_df.loc[trajectory_df['rel_distance'] >= monkey_hoverdata_value_cm]
        if len(trajectory_hover_row) > 0:
            trajectory_hover_row = trajectory_hover_row.iloc[0]
        else:
            trajectory_hover_row = trajectory_df.iloc[-1]
...
def make_fig_scatter_combd(fig_scatter_s, fig_scatter_cm, use_two_y_axes):
    ...
    for data in fig_scatter_s.data:
        ...
        if overall_secondary_y is True: # then we judge again whether we should use secondary_y for this particular trace
            secondary_y = (data['name'] in ['Change in Curvature of Trajectory', 'y2 =5 or -5'])
        fig_scatter_combd.add_trace(data, row=1, col=1, secondary_y=secondary_y)
    ...
    for data in fig_scatter_cm.data:
        ...
        if overall_secondary_y is True: # then we judge again whether we should use secondary_y for this particular trace
            secondary_y = (data['name'] in ['Change in Curvature of Trajectory', 'y2 =5 or -5'])
        fig_scatter_combd.add_trace(data, row=2, col=1, secondary_y=secondary_y)
    ...
    fig_scatter_combd.update_layout(legend=dict(orientation="h", y=1.3, groupclick="togglegroup"),
                                    ...
                                    xaxis2 = dict(title = 'Relative Distance (cm)'),
                                    yaxis=dict(title=dict(text="Curvature of Trajectory (deg/cm)"), side="left"),
...
def update_fig_scatter_natural_y_range(fig_scatter_natural_y_range, df, y_column_name, cap=[-150, 150]):
    new_fig_scatter_natural_y_range = [np.min(df[y_column_name].values), np.max(df[y_column_name].values)]
    fig_scatter_natural_y_range = [np.min([fig_scatter_natural_y_range[0], new_fig_scatter_natural_y_range[0]]), np.max([fig_scatter_natural_y_range[1], new_fig_scatter_natural_y_range[1]])]
...
def plot_lines_to_show_ff_visible_segments_in_fig_scatter(fig_scatter, ff_info, monkey_information, stops_near_ff_row,
    ...
    for i, ff_index in enumerate(unique_ff_indices):
        ...
        all_starting_rel_values = monkey_information.loc[all_starting_points, time_or_distance_var].values - ref_value
        all_ending_rel_values = monkey_information.loc[all_ending_points, time_or_distance_var].values - ref_value  
        if ff_names is None:
        ...
        for j in range(len(all_breaking_points)+1):
            ...
            fig_scatter = add_annotation_to_fig_scatter(fig_scatter, ff_names[i], all_starting_rel_values[j])
