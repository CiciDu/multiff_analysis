...
def prepare_to_plot_a_planning_instance_in_plotly(row, 
    ...
    eliminate_irrelevant_points_beyond_boundaries = monkey_plot_params['eliminate_irrelevant_points_beyond_boundaries']
    ...
    show_connect_path_ff_specific_indices = [int(row.stop_ff_index), int(row.alt_ff_index)]

    if eliminate_irrelevant_points_beyond_boundaries:
        relevant_point_indices = [row.stop_point_index, row.next_stop_point_index]
    ...
    ff_dataframe_in_duration = ff_dataframe[(ff_dataframe['time'] >= duration_to_plot[0]) & (ff_dataframe['time'] <= duration_to_plot[1])].copy()
    ff_dataframe_in_duration.sort_values(by='ff_distance', ascending=False, inplace=True)
    ...
    if show_alive_fireflies:
        alive_ff_indices, alive_ff_position_rotated = plot_behaviors_utils.find_alive_ff(duration_to_plot, ff_life_sorted, ff_real_position_sorted, rotation_matrix=R)
    ...
    if show_connect_path_ff:
        connect_path_ff_df, shown_ff_indices = make_connect_path_ff_df(row, shown_ff_indices, show_connect_path_ff, show_connect_path_ff_specific_indices, ff_dataframe_in_duration_visible, R, connect_path_ff_max_distance=connect_path_ff_max_distance)
    
    # create ff_df
    shown_ff_indices = np.unique(np.array(shown_ff_indices)).astype(int)
    ff_positions_rotated = np.matmul(R, ff_real_position_sorted[shown_ff_indices].T)
    ff_df = pd.DataFrame({'ff_x': ff_positions_rotated[0], 'ff_y': ff_positions_rotated[1], 'ff_index': shown_ff_indices})

    # add ff_number to ff_df
    show_visible_segments_ff_specific_indices = pd.unique(np.concatenate([show_connect_path_ff_specific_indices, ff_dataframe_in_duration_visible.ff_index.unique()])).astype(int)
    ...
    ff_dataframe_in_duration_visible_qualified = ff_dataframe_in_duration_visible.merge(ff_number_df, on='ff_index', how='left')
...
def make_trajectory_df(PlotTrials_args, 
    ...
    eye_positions_columns = ['point_index', 'gaze_world_x', 'gaze_world_y', 'gaze_monkey_view_x', 'gaze_monkey_view_y', 'LDz', 'RDz']
    ...
    try:
        trajectory_df = trajectory_df.merge(monkey_information[eye_positions_columns], on='point_index', how='left')
    except KeyError:
        pass
...
def make_connect_path_ff_df(row, 
    ...
    connect_path_ff_df = ff_dataframe_in_duration_visible_qualified[['ff_x', 'ff_y', 'monkey_x', 'monkey_y', 'point_index', 'time', 'cum_distance']].copy()
    connect_path_ff_df['counter'] = np.arange(connect_path_ff_df.shape[0])
    connect_path_ff_df['rel_time'] = np.round(connect_path_ff_df['time'] - row.stop_time, 2)
    connect_path_ff_df['rel_distance'] = np.round(connect_path_ff_df['cum_distance'] - row.stop_cum_distance, 2)
    # rotated ff_x and ff_y
    ff_positions_rotated = np.matmul(R, connect_path_ff_df[['ff_x', 'ff_y']].T.values)
    connect_path_ff_df[['ff_x', 'ff_y']] = np.vstack([ff_positions_rotated[0], ff_positions_rotated[1]]).T
    # rotated monkey_x and monkey_y
    monkey_positions_rotated = np.matmul(R, connect_path_ff_df[['monkey_x', 'monkey_y']].T.values)
    connect_path_ff_df[['monkey_x', 'monkey_y']] = np.vstack([monkey_positions_rotated[0], monkey_positions_rotated[1]]).T
...
def find_traj_portion_for_traj_curv(trajectory_df, curv_of_traj_current_row):
    traj_portion = trajectory_df[(trajectory_df.point_index >= curv_of_traj_current_row['min_point_index'].item()) & (trajectory_df.point_index <= curv_of_traj_current_row['max_point_index'].item())]
    traj_length = traj_portion['rel_distance'].iloc[-1] - traj_portion['rel_distance'].iloc[0]
...
def find_ff_curv_df(ff_index, current_plotly_plot_key_comp, ff_dataframe, monkey_information, curv_of_traj_df, ff_caught_T_sorted=None):
    ...
    ff_curv_df['curv_to_ff_center'] = ff_curv_df['curv_to_ff_center'] * 180/np.pi * 100 # convert to degree/cm 
    ff_curv_df['optimal_curvature'] = ff_curv_df['optimal_curvature'] * 180/np.pi * 100 # convert to degree/cm 
    ff_curv_df_sub = ff_curv_df[['point_index', 'rel_time', 'rel_distance', 'curv_to_ff_center', 'optimal_curvature']].copy()
