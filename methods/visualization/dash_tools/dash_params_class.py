...
class ParamsDash(params_test_combos_class.ParamsTestCombos):
    ...
    def _put_down_checklist_for_params(self, id_prefix=None):
        ...
        checklist_values = [key for key in checklist_params if self.hyperparameter_dict[key] is True]
    ...
    def prepare_for_dash(self, 
        ...
        self.fig_lines = params_utils.plot_tested_heading_df_or_curv_df2(self.sub_df, self.hyperparameter_dict)



    def make_combo_df_long(self):
        self.heading_df, self.curv_df = params_utils.get_heading_df_and_curv_df(self.tested_combo_df)
        columns_to_include = ['ref_point_mode', 'ref_point_value', 'same_mode',
                            ...
                            'heading_instead_of_curv', 'use_curvature_to_ff_center',
        ...
        self.combo_df_long = pd.concat([self.heading_df[columns_to_include], self.curv_df[columns_to_include]], ignore_index=True)
    ...
    def prepare_dash_for_params_layout(self, id_prefix='params_'):
        ...
        layout = html.Div([self._put_down_checklist_for_params(id_prefix),    
                            ...
                            dash_utils.put_down_the_dropdown_menu_for_ref_point_mode(self.hyperparameter_dict['ref_point_mode'], id=id_prefix+'ref_point_mode'),                            
                            params_utils.make_a_slider_for_reference_point(self.hyperparameter_dict['ref_point_mode'], self.ref_point_info, id=id_prefix+'ref_point_slider'),
                            dash_utils.put_down_the_dropdown_menu_for_curv_of_traj_mode(self.hyperparameter_dict['curv_of_traj_mode'], label='Curvature of Trajectory Window Size', id=id_prefix+'curv_of_traj_mode'),
                            params_utils.make_a_slider_for_window_size(self.unique_window_sizes, id=id_prefix+'window_size_slider'),
                            html.Div([dcc.Graph(id=id_prefix+'scatter_plot', figure = self.fig_lines, style={'width': '70%', 'padding': '0 0 0 0'})])
                            ])
        return layout
    ...
    def make_dash_for_params(self):
        ...
        self.make_function_to_update_ref_point_slider_and_plot_based_on_ref_point_mode(app)
        self.make_function_to_update_window_size_slider_based_on_curv_of_traj_mode(app)
    ...
    def make_function_to_update_window_size_slider_based_on_curv_of_traj_mode(self, app):
        ...
        @app.callback(
            ...
            Output(self.id_prefix + 'scatter_plot', 'figure', allow_duplicate=True),
        ...
        def update_window_size_slider(curv_of_traj_mode):
            ...
            window_size_marks_dict = {i: str(i) for i in self.unique_window_sizes}
    ...
    def make_function_to_update_plot_based_on_new_info(self, app):
        ...
        def update_correlation_plot(ref_point_value, 
            ...
            if (ctx.triggered[0]['prop_id'] == self.id_prefix + 'ref_point_slider.value'):   
                ...
                for param in ['heading_instead_of_curv', 'truncate_curv_of_traj_by_time_of_capture', 'eliminate_outliers', 'use_curvature_to_ff_center']:
    ...
    def find_unique_window_sizes(self):
        self.unique_window_sizes_for_time = np.round(self.combo_df_long[self.combo_df_long['curv_of_traj_mode'] == 'time']['window_size'].unique(), 2)
        self.unique_window_sizes_for_distance = self.combo_df_long[self.combo_df_long['curv_of_traj_mode'] == 'distance']['window_size'].unique().astype(int)
