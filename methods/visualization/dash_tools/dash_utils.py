...
def put_down_the_dropdown_menu_for_ref_point_mode(ref_point_mode, id='ref_point_mode'):
    return html.Div(children=[
                        html.Label(['Reference Point From Stopping Point'], style={'font-weight': 'bold', "text-align": "center"}),
                        dcc.Dropdown(
                            id=id,
...
def put_down_the_input_for_ref_point_descr(ref_point_value, id="ref_point_value"):
    return html.Div(
            ...
            # make it look better
            style={'width': '25%', 'padding': '10px 10px 10px 10px', 'background-color': '#9FD4A3'} 
        )
...
def put_down_the_dropdown_menu_for_curv_of_traj_mode(curv_of_traj_mode, label='Curvature of Trajectory', id='curv_of_traj_mode'):
    return html.Div(children=[
                        html.Label([label], style={'font-weight': 'bold', "text-align": "center"}),
                        dcc.Dropdown(
                            id=id,
...
def put_down_the_input_for_window_lower_end_and_upper_end(window_for_curv_of_traj, ids=['window_lower_end', 'window_upper_end']):
    ...
    return html.Div(
            ...
            dcc.Input(id=ids[1], type="number", placeholder="window upper end", debounce=True, value=upper_end),
            ],
            # make it look better
            style={'width': '25%', 'padding': '10px 10px 10px 10px', 'background-color': '#B5D3E7'}
        )
...
def put_down_empty_plot_that_takes_no_space(id='empty_plot'):
    ...
    return html.Div([
                dcc.Graph(
                    id=id,
                    figure = empty_plot),
            ], style={'height': '0px', 'width': '0px', 'padding': '0 0 0 0', #'display': 'inline-block',
                    })
...
def put_down_monkey_plot(fig, monkey_hoverdata_value, id='monkey_plot'):
    return html.Div([
                dcc.Graph(
                    id=id,
                    figure = fig,
                    hoverData={'points': [{'customdata': monkey_hoverdata_value}]} #['Original-Present', 0]}]}
                )
                ...
                ],  style={'width': '60%', 'padding': '0 0 0 0', #'display': 'inline-block',
                        })
...
def create_error_message_display(id_prefix='main_plots_'):
    return html.Div([
        ...
        style={'width': '25%', 'padding': '10px 10px 10px 10px', 'background-color': 'orange'} 
    )
