        ...
        return html.Div([
            ...
            html.Div([
                plotly_for_correlation.put_down_correlation_plot(
                    self.fig_heading, id='heading_plot', width='50%'),
                plotly_for_correlation.put_down_correlation_plot(
                    self.fig_heading_2, id='heading_plot_2', width='50%')
            ], style=dict(display='flex')),
            html.Button('Refresh correlation plot on the right', id='refresh_fig_corr_2', n_clicks=0,
                        style={'margin': '10px 10px 10px 10px', 'background-color': '#FFC0CB'}),
            html.Div([
                plotly_for_correlation.put_down_correlation_plot(
                    self.fig_corr, id='correlation_plot', width='50%'),
                plotly_for_correlation.put_down_correlation_plot(
                    self.fig_corr_2, id='correlation_plot_2', width='50%')
            ], style=dict(display='flex')),
        ])
        ...
        def update_correlation_plot(curv_of_traj_mode, curv_of_traj_lower_end, curv_of_traj_upper_end,
            ...
            if (ctx.triggered[0]['prop_id'] == 'update_curv_of_traj.nclicks'):
                if (curv_of_traj_lower_end < curv_of_traj_upper_end) or (curv_of_traj_mode == 'now to stop'):
                    ...
                    self.fig_corr, self.fig_corr_2, self.fig_heading, self.fig_heading_2 = self.update_dash_comparison_based_on_curv_of_traj_df(
