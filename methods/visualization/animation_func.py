...
def animate(frame, ax, anim_monkey_info, margin, ff_dataframe_anim, flash_on_ff_dict, alive_ff_dict, 
    ...
    # Plot trajectory
    ax = animation_utils.plot_trajectory_for_animation(anim_monkey_info, frame, show_speed_through_path_color, ax)
    ...
    ax = animation_utils.plot_visible_ff_reward_boundary_for_animation(visible_ffs, ax)
    ...
    if show_in_memory_ff:
        ...
        ax = animation_utils.plot_in_memory_ff_reward_boundary_for_animation(in_memory_ffs, ax)


    # Plot the believed positions of caught fireflies 
    # if believed_ff_dict is not None: 
    if len(believed_ff_dict[point_index]) > 0:
        ax.scatter(believed_ff_dict[point_index][:, 0], believed_ff_dict[point_index][:, 1], alpha=1, c="magenta", s=35)
    ...
    if in_obs_ff_dict is not None:
        ax, markers, marker_labels, _ = animation_utils.plot_circles_around_ff_from_dict(in_obs_ff_dict, point_index, anim_monkey_info['ff_real_position_rotated'], markers, marker_labels, ax)

    # circle anno or pred ff
    if circle_anno_or_pred_ff:
        ff_real_position_rotated = anim_monkey_info['ff_real_position_rotated']
        ax, markers, marker_labels, anno_ff_non_neg_indices, anno_but_not_obs_ff_indices = animation_utils.plot_anno_ff_for_animation(anno_ff_indices_dict, anno_but_not_obs_ff_indices_dict, point_index, ff_real_position_rotated, markers, marker_labels, ax)
        ax, markers, marker_labels = animation_utils.plot_pred_ff_for_animation(pred_ff_indices_dict, pred_ff_colors_dict, point_index, ff_real_position_rotated, markers, marker_labels, ax)
        if plot_show_null_trajectory_for_anno_ff:
            ax = animation_utils.plot_show_null_trajectory_for_anno_ff_for_animation(ax, anno_ff_non_neg_indices, anno_but_not_obs_ff_indices, anim_monkey_info, frame, ff_real_position_rotated)
        
    if len(markers) > 0:
        ax.legend(markers, marker_labels, scatterpoints=1, bbox_to_anchor=(0.8, 0.9), loc=2, borderaxespad=0, fontsize=12)
    ...
    if show_direction_through_triangle:
        ax = animation_utils.plot_triangle_to_show_direction_for_animation(anim_monkey_info, frame, ax)
    

    if plot_time_index:
        ax = animation_utils.plot_time_index_for_animation(anim_monkey_info, frame, ax)

    if set_xy_limits:
        if show_full_arena:
            ax.set_xlim(-1100-anim_monkey_info['x0'], 1100-anim_monkey_info['x0'])
            ax.set_ylim(-1100-anim_monkey_info['y0'], 1100-anim_monkey_info['y0'])
            ...
            mx_min, mx_max, my_min, my_max = animation_utils.find_xy_min_max_for_animation(anim_monkey_info, ff_dataframe_anim)
            ax = plot_behaviors_utils.set_xy_limits_for_axes(ax, mx_min, mx_max, my_min, my_max, margin)
    ax.set_aspect('equal')
