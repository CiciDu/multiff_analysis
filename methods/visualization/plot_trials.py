...
def PlotTrials(duration, 
               ...
               y0 = None,
               player="monkey",
               trail_color_var = None, # None or 'speed' or 'abs_ddw' or 'time'; if not None, then the color of the path will vary by this variable
               ...
               show_points_when_ff_start_being_visible = False, # when this or the next parameter is True, vary_color_for_connecting_path_ff will automatically be set as True
               ...
               show_eye_positions_for_both_eyes = False, # this is only used if show_eye_positions = True
    ...
    if (np.any(cum_r > 949)):
        ...
        if not hitting_arena_edge_ok:
            ...
            return {'whether_plotted': False}  # the three outputs are whether_plotted, axes, R, cum_mxy_rotated, shown_ff_indices
    else:
    ...
    if truncate_part_before_crossing_arena_edge:
        ...
        if len(cross_boundary_points) > 0:
            ...
            if cross_boundary_time - duration[0] < (duration[1]-duration[0])*2/3:
                ...
                print("duration[0] updated to {} to truncate the part of crossing arena edge".format(duration[0]))
    ...
    ff_dataframe_in_duration_visible = ff_dataframe_in_duration.loc[ff_dataframe_in_duration['visible']==1]
    ...
    if show_start:
        ...
        marker = axes.scatter(cum_mxy_rotated[0, 0]-x0, cum_mxy_rotated[1, 0]-y0, marker='s', s=start_size[player], color="green", zorder=3, alpha=0.7)
    ...
    if show_stops:
        ...
        zerospeed_rotated = plot_behaviors_utils.find_stops_for_plotting(cum_mx, cum_my, cum_speeddummy, rotation_matrix=R)
        marker = axes.scatter(zerospeed_rotated[0]-x0, zerospeed_rotated[1]-y0, marker='*', s=stop_size[player], alpha=0.7, color="black", zorder=2)
        legend_markers.append(marker)
    ...
    alive_ff_indices, alive_ff_position_rotated = plot_behaviors_utils.find_alive_ff(duration, ff_life_sorted, ff_real_position_sorted, rotation_matrix=R)
    if show_alive_fireflies:
        marker = axes.scatter(alive_ff_position_rotated[0]-x0, alive_ff_position_rotated[1]-y0, marker='o', s=10, color="magenta", zorder=2)
        ...
        if show_visible_fireflies:
            ff_to_be_plotted_in_a_basic_way.extend(ff_dataframe_in_duration_visible.ff_index.unique())
            marker_name = ('Centers of visible fireflies')
        if show_in_memory_fireflies:
            ff_to_be_plotted_in_a_basic_way.extend(ff_dataframe_in_duration_in_memory.ff_index.unique())
        ...
        ff_to_be_plotted_in_a_basic_way = np.unique(np.array(ff_to_be_plotted_in_a_basic_way)).tolist()
        ff_positions_rotated = np.matmul(R, ff_real_position_sorted[ff_to_be_plotted_in_a_basic_way].T)
        marker = axes.scatter(ff_positions_rotated[0]-x0, ff_positions_rotated[1]-y0, marker='o', s=10, color="magenta", zorder=3)
    ...
    shown_ff_indices = ff_to_be_plotted_in_a_basic_way.copy() # a list of indices of fireflies that will be shown in the plot except alive_ff
    ...
    if show_believed_target_positions:
        ...
        believed_target_positions_rotated = plot_behaviors_utils.find_believed_target_positions(ff_believed_position_sorted, currentTrial, num_trials, rotation_matrix=R)
        marker = axes.scatter(believed_target_positions_rotated[0]-x0, believed_target_positions_rotated[1]-y0, marker=marker[player], s=target_size[player], color="red", alpha=0.75, zorder=4)
        legend_markers.append(marker)
    ...
    if indices_of_ff_to_mark is not None:
        ...
        for ff in indices_of_ff_to_mark:
            ...
            ff_position_rotated = np.matmul(R, np.stack((ff_position[0], ff_position[1])))
            axes.scatter(ff_position_rotated[0]-x0, ff_position_rotated[1]-y0, marker='*', s=target_size[player]+170, color="green", alpha=0.75, zorder=4)
    ...
    if indices_of_ff_to_mark_2nd_kind is not None:
        ...
        for ff in indices_of_ff_to_mark_2nd_kind:
            ff_position = ff_real_position_sorted[ff]
            ff_position_rotated = np.matmul(R, np.stack((ff_position[0], ff_position[1])))
            axes.scatter(ff_position_rotated[0]-x0, ff_position_rotated[1]-y0, marker='s', s=target_size[player], color="purple", alpha=0.75, zorder=5)
    ...
    if index_of_ff_to_show_path_when_ff_visible is not None:
        ...
        # also mark that ff   
        ff_positions_rotated = np.matmul(R, ff_real_position_sorted[index_of_ff_to_show_path_when_ff_visible].T)
        marker = axes.scatter(ff_positions_rotated[0]-x0, ff_positions_rotated[1]-y0, marker='s', s=80, color="aqua", alpha=0.5, zorder=6)
    ...
    if show_connect_path_ff_memory:
        ff_dataframe_in_duration_in_memory_qualified = ff_dataframe_in_duration_in_memory.loc[ff_dataframe_in_duration_in_memory['ff_distance'] <= connect_path_ff_max_distance]
        shown_ff_indices.extend(ff_dataframe_in_duration_in_memory_qualified.ff_index.unique())

        if show_connect_path_ff_specific_indices is not None:
            ff_dataframe_in_duration_in_memory_qualified = ff_dataframe_in_duration_in_memory_qualified.loc[ff_dataframe_in_duration_in_memory_qualified['ff_index'].isin(show_connect_path_ff_specific_indices)]
        
        temp_ff_positions, temp_monkey_positions = plot_behaviors_utils.find_lines_to_connect_path_ff(ff_dataframe_in_duration_in_memory_qualified, target_indices, rotation_matrix=R)
    ...
    if show_visible_segments_ff_indices is not None:
        ff_dataframe_in_duration_visible_qualified = ff_dataframe_in_duration_visible.loc[ff_dataframe_in_duration_visible['ff_index'].isin(show_visible_segments_ff_indices)]
        ...
        if show_connect_path_ff_after_coloring_segments_ff_indices is not None:
            for _ff_index in np.array(show_connect_path_ff_after_coloring_segments_ff_indices):
                ff_dataframe_visible_for_specific_ff = ff_dataframe_in_duration_visible.loc[ff_dataframe_in_duration_visible['ff_index']==_ff_index]
                axes, _, _, _, _, = plot_behaviors_utils.plot_lines_to_connect_path_and_ff(axes, ff_dataframe_visible_for_specific_ff, rotation_matrix, x0, y0, 1, 0.5, vary_color_for_connecting_path_ff=False, line_color=show_visible_segments_of_ff_dict[_ff_index])
    ...
    if show_connect_path_ff or show_connect_path_ff_except_targets or (show_connect_path_ff_specific_indices is not None):
        ff_dataframe_in_duration_visible_qualified = ff_dataframe_in_duration_visible.loc[ff_dataframe_in_duration_visible['ff_distance'] <= connect_path_ff_max_distance]
        if show_connect_path_ff_specific_indices is not None:
            ff_dataframe_in_duration_visible_qualified = ff_dataframe_in_duration_visible_qualified.loc[ff_dataframe_in_duration_visible_qualified['ff_index'].isin(show_connect_path_ff_specific_indices)]
    
        shown_ff_indices.extend(ff_dataframe_in_duration_visible_qualified.ff_index.unique())
        axes, legend_markers, legend_names, temp_ff_positions, temp_monkey_positions = plot_behaviors_utils.plot_lines_to_connect_path_and_ff(axes=axes, \
                ff_info=ff_dataframe_in_duration_visible_qualified, rotation_matrix=R, x0=x0, y0=y0, linewidth=connection_linewidth[player], alpha=connection_alpha[player], \
        ...
        show_dots_marker = Line2D([0], [0], marker='o', markersize=5, color="brown", linestyle='')
        marker_name = "FFs visible"
        if show_connect_path_ff_memory:
            marker_name = marker_name + " or in memory" # As of now, ff_in_memory will not be shown unless ff_visible is shown too
        if show_connect_path_ff_except_targets:
            marker_name = marker_name + " except targets\n(the longer the darker)"
        else:
    ...
    if trial_to_show_cluster is not None:
        ...
        cluster_ff_rotated = plot_behaviors_utils.find_ff_in_cluster(cluster_dataframe_point, ff_real_position_sorted, currentTrial=currentTrial+trial_conversion[trial_to_show_cluster], rotation_matrix=R)
        axes.scatter(cluster_ff_rotated[0]-x0, cluster_ff_rotated[1]-y0, marker='o', c="blue", s=25, zorder=4)
    ...
    if trial_to_show_cluster_around_target is not None:
        ...
        axes.scatter(cluster_around_target_rotated[0]-x0, cluster_around_target_rotated[1]-y0, marker='o', s=30, color="blue", zorder=4)
        if show_path_when_cluster_visible: # Find where on the path the monkey/agent can see any member of the cluster around the target
            ...
            for index in cluster_ff_indices:
                monkey_xy_rotated, ff_position_rotated = plot_behaviors_utils.find_path_when_ff_in_cluster_visible(ff_dataframe_in_duration, index, rotation_matrix=R)
                axes.scatter(monkey_xy_rotated[0]-x0, monkey_xy_rotated[1]-y0, s=path_size[player][0] - path_size[player][1] * i, color=list_of_colors[i], alpha=path_alpha[player], zorder=3+i)
                # Use a circle with the corresponding color to show that ff
                axes.scatter(ff_position_rotated[0]-x0, ff_position_rotated[1]-y0, marker='o', s=ff_size[player], alpha=ff_alpha[player], color=list_of_colors[i], zorder=3)
    ...
    if (rotation_matrix is not None) & (shown_ff_positions_rotated.shape[1]>0):
        shown_ff_positions_rotated = np.matmul(rotation_matrix, shown_ff_positions_rotated)
    ...
    if show_eye_positions:
        if not show_eye_positions_for_both_eyes:
            ...
            both_eyes_info = eye_positions.find_eye_positions_rotated_in_world_coordinates(monkey_information, duration, rotation_matrix=R, separate_left_and_right_eyes=True)
    ...
    if show_eye_positions_on_the_right:
        ...
        axes2 = plot_behaviors_utils.set_xy_limits_for_axes(axes2, mx_min, mx_max, my_min, my_max, minimal_margin, zoom_in)
        fig.tight_layout()
    ...
    if show_colorbar:
        fig, axes = plot_behaviors_utils.plot_colorbar_for_trials(fig, axes, trail_color_var, show_eye_positions=show_eye_positions, show_eye_positions_on_the_right=show_eye_positions_on_the_right, duration=duration, max_value=colorbar_max_value)
    ...
    if adjust_xy_limits:
        if show_eye_positions_on_the_right:
            mx_min, mx_max, my_min, my_max = plot_behaviors_utils.find_xy_min_max_for_plots(gaze_world_xy_rotated[:, overall_valid_indices], x0, y0, temp_ff_positions=None)
            axes = plot_behaviors_utils.set_xy_limits_for_axes(axes, mx_min, mx_max, my_min, my_max, minimal_margin, zoom_in)
        else:
            mx_min, mx_max, my_min, my_max = plot_behaviors_utils.find_xy_min_max_for_plots(cum_mxy_rotated, x0, y0, temp_ff_positions=shown_ff_positions_rotated)
            axes = plot_behaviors_utils.set_xy_limits_for_axes(axes, mx_min, mx_max, my_min, my_max, minimal_margin, zoom_in)
    ...
    if show_eye_world_speed_vs_monkey_speed:
        gaze_world_xy_rotated, overall_valid_indices, monkey_subset = eye_positions.find_eye_positions_rotated_in_world_coordinates(monkey_information, duration, rotation_matrix=R)
        gaze_monkey_view_xy = monkey_subset[['gaze_monkey_view_x', 'gaze_monkey_view_y']].values.T
        eye_positions.plot_eye_world_speed_vs_monkey_speed(gaze_monkey_view_xy, cum_t, overall_valid_indices, monkey_information)          
