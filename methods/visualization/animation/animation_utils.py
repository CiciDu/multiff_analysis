...
def prepare_for_animation(ff_dataframe, ff_caught_T_new, ff_life_sorted, ff_believed_position_sorted, ff_real_position_sorted, 
    ...
    if max_duration is not None:
        if (duration[1]-duration[0]) > max_duration:
            ...
            print("The duration is too long. The animation will be shown for the last 30 seconds.")
        
    # If the duration is too short
    if min_duration is not None:
        if (duration[1]-duration[0]) < min_duration:
            duration = [duration[1]-1, duration[1]]
            print("The duration is too short. The animation will be shown for the last 1 second.")
    ...
    if rotated:
        ...
        anim_monkey_info, x0, y0 = rotated_anim_monkey_info(anim_monkey_info, R)
        anim_monkey_info['anim_angle'] = anim_monkey_info['anim_angle'] + theta
        ff_dataframe_anim.loc[:, ['ff_x_rotated', 'ff_y_rotated']] = (R @ ff_dataframe_anim[['ff_x', 'ff_y']].values.T).T - np.array([x0, y0])
        ff_real_position_rotated = (R @ ff_real_position_sorted.T).T - np.array([x0, y0]) 
        if "ff_x_noisy" in ff_dataframe_anim.columns:
            ff_dataframe_anim.loc[:, ['ff_x_noisy_rotated', 'ff_y_noisy_rotated']] = (R @ ff_dataframe_anim[['ff_x_noisy', 'ff_y_noisy']].values.T).T - np.array([x0, y0])
    else:
        R = np.eye(2)
        x0, y0 = 0, 0
        anim_monkey_info['x0'], anim_monkey_info['y0'] = 0, 0
        ff_real_position_rotated = ff_real_position_sorted
    anim_monkey_info['R'] = R
...
def plot_trajectory_for_animation(anim_monkey_info, frame, show_speed_through_path_color, ax):
    if show_speed_through_path_color:
        color = plt.get_cmap('viridis')(anim_monkey_info['anim_speed'][:frame+1]/200)
    else:
        color = 'royalblue'
    ax.scatter(anim_monkey_info['anim_mx'][:frame+1], anim_monkey_info['anim_my'][:frame+1], s=15, c=color)
    return ax
...
def plot_visible_ff_reward_boundary_for_animation(visible_ffs, ax):
    ...
    if "ff_x_noisy" in visible_ffs.columns: 
        ...
        for k in range(len(visible_ffs)):
          circle = plt.Circle((visible_ffs[ff_x_column].iloc[k], visible_ffs[ff_y_column].iloc[k]), 25, facecolor='yellow', edgecolor='gray', alpha=0.7, zorder=1)
          ax.add_patch(circle)
          circle = plt.Circle((visible_ffs[ff_x_noisy_column].iloc[k], visible_ffs[ff_y_noisy_column].iloc[k]), 25, facecolor='gray', edgecolor='gray', alpha=0.5, zorder=1)
          ax.add_patch(circle)
    else:
...
def plot_in_memory_ff_reward_boundary_for_animation(in_memory_ffs, ax):
    ...
    for j in range(len(in_memory_ffs)):
        circle = plt.Circle((in_memory_ffs[ff_x_column].iloc[j], in_memory_ffs[ff_y_column].iloc[j]), 25, facecolor='purple', edgecolor='orange', alpha=0.3, zorder=1)
        ax.add_patch(circle)
    return ax
...
def show_ff_indices_for_animation(relevant_ff, ax):
    ...
    for k in range(len(selected_ffs)):
        ax.annotate(str(selected_ff_indices[k]), (selected_ff_x[k], selected_ff_y[k]), fontsize=12)
    return ax
...
def plot_a_triangle_to_show_direction(ax, monkey_x, monkey_y, monkey_angle):
    # Plot a triangular shape to indicate the direction of the agent or monkey
    left_end_x, left_end_y, right_end_x, right_end_y = find_triangle_to_show_direction(monkey_x, monkey_y, monkey_angle)
    
    ax.plot(np.array([monkey_x, left_end_x]), np.array([monkey_y , left_end_y]), linewidth = 2)
    ax.plot(np.array([monkey_x, right_end_x]), np.array([monkey_y , right_end_y]), linewidth = 2)
    return ax


def plot_triangle_to_show_direction_for_animation(anim_monkey_info, frame, ax):
    ax = plot_a_triangle_to_show_direction(ax, anim_monkey_info['anim_mx'][frame], anim_monkey_info['anim_my'][frame], anim_monkey_info['anim_angle'][frame])
    # # Plot a triangular shape to indicate the direction of the agent or monkey
...
def plot_circles_around_ff(ff_indices, ff_real_position_rotated, circle_size, edgecolor, ax, lw=2):
    ...
    if len(ff_indices) > 0:
        for k in ff_indices:
            circle = plt.Circle((ff_real_position_rotated[k, 0], ff_real_position_rotated[k, 1]), circle_size, facecolor='None', edgecolor=edgecolor, alpha=0.8, zorder=1, lw=lw)
            ax.add_patch(circle)
    return ax
...
def plot_anno_ff_for_animation(anno_ff_indices_dict, anno_but_not_obs_ff_indices_dict, point_index, ff_real_position_rotated, markers, marker_labels, ax):
    ...
    if anno_ff_indices_dict is not None:
        if point_index in anno_ff_indices_dict.keys():
            ...
            if len(anno_ff_non_neg_indices) > 0:
                ax = plot_circles_around_ff(anno_ff_non_neg_indices, ff_real_position_rotated, circle_size=30, edgecolor='red', ax=ax)
            else:
            ...
            if anno_but_not_obs_ff_indices_dict is not None: 
                if point_index not in anno_but_not_obs_ff_indices_dict.keys(): # otherwise, it will plotted below (in the next if else statement)
    ...
    marker1 = Line2D([0], [0], marker='o', color='r', label='Circle', markerfacecolor='w', lw=0, markeredgewidth=2, markersize=15)
    markers.append(marker1)
    marker_labels.append('Annotated')


    ax, markers, marker_labels, anno_but_not_obs_ff_indices = plot_circles_around_ff_from_dict(anno_but_not_obs_ff_indices_dict, point_index, ff_real_position_rotated, markers, marker_labels, ax, edgecolor='green', circle_size=33, legend_name='Annotated but not observed')
...
def plot_pred_ff_for_animation(pred_ff_indices_dict, pred_ff_colors_dict, point_index, ff_real_position_rotated, markers, marker_labels, ax):
    ...
    ax, markers, marker_labels, _ = plot_circles_around_ff_from_dict(pred_ff_indices_dict, point_index, ff_real_position_rotated, markers, marker_labels, ax, edgecolor=edgecolor, circle_size=37, legend_name='Predicted')
...
def plot_circles_around_ff_from_dict(ff_indices_dict, 
    ...
    if ff_indices_dict is not None:
        if point_index in ff_indices_dict.keys():
            ...
            ax = plot_circles_around_ff(in_obs_ff_indices, ff_real_position_rotated, circle_size=circle_size, edgecolor=edgecolor, ax=ax, lw=lw)

        marker2 = Line2D([0], [0], marker='o', color=edgecolor, label='Circle', markerfacecolor='w', lw=0, markeredgewidth=2, markersize=15)
        #marker2,_ = ax.plot([], [], 50, color='blue', marker='o', alpha=1, zorder=1)
        markers.append(marker2)
...
def plot_show_null_trajectory_for_anno_ff_for_animation(ax, anno_ff_non_neg_indices, anno_but_not_obs_ff_indices, anim_monkey_info, frame, ff_real_position_rotated,
    ...
    union_anno_ff_indices = np.union1d(anno_ff_non_neg_indices, anno_but_not_obs_ff_indices).astype(int)
    ...
    if reaching_boundary_ok:
        ff_xy = find_best_arc.find_point_on_ff_boundary_with_smallest_angle_to_monkey(ff_x, ff_y, monkey_x, monkey_y, monkey_angle)
        ff_x = ff_xy[:, 0]
        ff_y = ff_xy[:, 1]

    # get the null trajectory info
    ff_xy, ff_distance, ff_angle, ff_angle_boundary, arc_length, arc_radius = show_null_trajectory.find_arc_length_and_radius(ff_x, ff_y, monkey_x, monkey_y, monkey_angle)
    whether_ff_behind = (np.abs(ff_angle) > math.pi/2)
    center_x, center_y, arc_starting_angle, arc_ending_angle = show_null_trajectory.find_cartesian_arc_center_and_angle_for_arc_to_center(monkey_xy, monkey_angle, ff_distance, ff_angle, arc_radius, ff_xy, np.sign(ff_angle), whether_ff_behind=whether_ff_behind)
    ...
    arc_xy_rotated = show_null_trajectory.find_arc_xy_rotated(center_x[arc_ff], center_y[arc_ff], arc_radius[arc_ff], arc_starting_angle[arc_ff], arc_ending_angle[arc_ff], rotation_matrix=None)  
    ax.plot(arc_xy_rotated[0], arc_xy_rotated[1], linewidth=2.5, color=arc_color, zorder=4)

    # then take out the ff that will be reached by straight lines
    line_ff = np.where(arc_radius == 0)[0] 
    for ff in (line_ff):
        line_xy_rotated = np.stack((np.array([monkey_x, ff_xy[ff][0]]), np.array([monkey_y, ff_xy[ff][1]])))
        ax.plot(line_xy_rotated[0], line_xy_rotated[1], linewidth=2.5, color=line_color, zorder=4)
