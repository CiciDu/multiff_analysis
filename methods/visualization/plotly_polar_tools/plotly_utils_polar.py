...
def make_one_plotly_polar_plot(ff_df=None, current_traj_df=None, current_more_traj_df=None, real_label=None, predicted_label=None,
    ...
    if show_more_ff is False:
        fig.update_traces(visible='legendonly', selector={"marker_symbol": "diamond-open-dot"})
    ...
    if add_trajectory:
        ...
        fig = plotly_for_trajectory_polar.plot_trajectory_data(fig, current_more_traj_df, color_discrete_sequence=['black', 'gold'], additional_update_kwargs={'marker': {'size': 4}})
    
    if monkey_info_for_ff_in_past_or_future is not None:
        fig = plotly_for_trajectory_polar.plot_monkey_info_for_ff_in_past_or_future(fig, monkey_info_for_ff_in_past_or_future)
    ...
    if add_real_and_predicted_labels:
        fig = annotate_real_and_predicted_labels_in_numbers(fig, real_label, predicted_label)
...
def annotate_real_and_predicted_labels_in_numbers(fig, real_label, predicted_label):
    ...
    fig.add_annotation(x=0.1, y=1.08, text='Real: '+str(real_label+2), showarrow=False, font=dict(size=12, color='green'))
    fig.add_annotation(x=0.1, y=1.05, text='Predicted: '+str(predicted_label+2), showarrow=False, font=dict(size=12, color=predicted_label_color))
    return fig
...
def prepare_to_make_one_plotly_polar_plot(i, polar_plots_kwargs, point_index_array, GUAT_joined_ff_info, all_traj_feature_names, more_ff_df=None, trajectory_features=['monkey_distance', 'monkey_angle_to_origin'],
    ...
    all_ff_dict = plotly_for_ff_polar.make_all_ff_dict_from_GUAT_joined_ff_df_and_more_ff_df(i, GUAT_joined_ff_info, more_ff_df, point_index_array)
    current_traj_df, current_more_traj_df = plotly_for_trajectory_polar.prepare_trajectory_data_for_plotting(current_traj_points, current_stops, all_traj_feature_names, current_more_traj_points=current_more_traj_points, current_more_traj_stops=current_more_traj_stops, trajectory_features=trajectory_features)
    present_ff_df = all_ff_dict['combined_ff_df'][all_ff_dict['combined_ff_df']['time_label']=='Present'].copy()
