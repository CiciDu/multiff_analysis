...
def update_firefly_hovertemplate_based_on_group(fig, ff_df, num_added_traces, additional_update_kwargs={}, color='group', symbol='group',
    ...
    for counter in range(len(columns_for_annotation)):
        if (columns_for_annotation[counter] == 'subgroup') & (counter == 0): # if subgroup is the first column, then we bold it and not add the annotation name
            hovertemplate = hovertemplate + '<b>%{customdata[' + f'{counter}' + ']}</b><br>'
    ...
    if (ff_df[symbol].dtype != 'float') & (ff_df[symbol].dtype != 'int'): # then symbol is used to define levels
    ...
    if (ff_df[color].dtype != 'float') & (ff_df[color].dtype != 'int'): # then color defines levels too
        ...
        if color == symbol:
            ...
            if symbol not in columns_for_levels:
                ...
                for j in range(len(levels_color)):
                    for i in range(len(symbol_levels)): # make outer product of symbol_levels and levels_color
                        levels.append([levels_color[j], symbol_levels[i]])
                        new_level_name = levels_color[j] + ', ' + symbol_levels[i]
    ...
    for i in range(num_added_traces):
        ...
        if current_data_name in level_names:
            ...
            for j in range(len(columns_for_levels)):
                ff_df_sub = ff_df_sub[(ff_df_sub[columns_for_levels[j]]==current_levels[j])].copy()
            ...
            raise ValueError('The current level name ' + current_data_name +  ' is not in level_names.')
...
def separate_ff_info_from_past_present_and_future(i, original_ff_df, point_index_array, time_since_last_vis_cap=3, time_till_next_visible_cap=3):
    ...
    current_ff_df = current_ff_df[current_ff_df['ff_distance'] < 400].copy()  # eliminate placeholders
    if 'ff_number' not in current_ff_df.columns:
        current_ff_df['ff_number'] = np.arange(current_ff_df.shape[0])   # assign each ff a number
    ...
    monkey_info_for_ff_in_past_or_future = plotly_for_trajectory_polar.find_monkey_info_for_ff_in_past_or_future(current_ff_df, last_seen_ff_df, next_seen_ff_df)
