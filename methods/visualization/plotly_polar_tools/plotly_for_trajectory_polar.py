...
def prepare_trajectory_data_for_plotting(current_traj_points, current_stops, all_traj_feature_names, current_more_traj_points=None, current_more_traj_stops=None, trajectory_features=['monkey_distance', 'monkey_angle_to_origin']):
    current_traj_df = make_current_traj_df(current_traj_points, current_stops, trajectory_features=trajectory_features)
    current_traj_df['time'] = [item.split('_')[-1] for item in all_traj_feature_names['traj_stops']]
    current_more_traj_df = make_current_traj_df(current_more_traj_points, current_more_traj_stops, trajectory_features=trajectory_features)
    current_more_traj_df['time'] = [item.split('_')[-1] for item in all_traj_feature_names['more_traj_stops']]
...
def plot_trajectory_data(fig, traj_df_to_use, color_discrete_sequence=['red', 'blue'], additional_update_kwargs={},
    ...
    fig.add_traces(
        list(px.scatter_polar(traj_df_to_use, 
                            ...
                            color_discrete_map={'has stop': color_discrete_sequence[0], 'no stop': color_discrete_sequence[1]},
    ...
    for counter in range(len(columns_for_annotation)):
        if columns_for_annotation[counter] in ['whether_stopped', 'time']:
            hovertemplate = hovertemplate + annotation_names[columns_for_annotation[counter]] + ': %{customdata[' + f'{counter}' + ']}<br>'
            ...
            hovertemplate = hovertemplate + annotation_names[columns_for_annotation[counter]] + ': %{customdata[' + f'{counter}' + ']:.2f}<br>'
    ...
    for i, level in enumerate(whether_stopped_levels):
        if fig.data[-(len(whether_stopped_levels)-i)].name == level:
            ...
            fig.update_traces(marker=dict(size=10, opacity=0.7), selector = ({'name': level}))
            #fig.for_each_trace(
...
def plot_monkey_info_for_ff_in_past_or_future(fig, monkey_info_for_ff_in_past_or_future,
                                              columns_for_annotation=['distance_from_monkey_now_to_ff_then', 'angle_from_monkey_now_to_ff_then', 'time_from_now'],
                                              additional_update_kwargs={}):
    

    for index, row in monkey_info_for_ff_in_past_or_future.iterrows():
        fig_traces = px.line_polar(row, 
                                r= np.array([row['distance_from_monkey_now_to_ff_then'], row['distance_from_monkey_now_to_monkey_then']]), 
                                theta= np.array([row['angle_from_monkey_now_to_ff_then'], row['angle_from_monkey_now_to_monkey_then']]),            
                                )
...
def make_current_traj_df(current_traj_points, current_stops, trajectory_features):
    current_traj_df = pd.DataFrame(current_traj_points.T, columns=trajectory_features)
    ...
    current_traj_df.loc[current_traj_df['whether_stopped']==1, 'whether_stopped'] = 'has stop'
    current_traj_df.loc[current_traj_df['whether_stopped']==0, 'whether_stopped'] = 'no stop'
...
def find_monkey_info_when_ff_next_seen(current_ff_df, next_seen_ff_numbers):
    current_ff_df = current_ff_df[current_ff_df['ff_number'].isin(next_seen_ff_numbers)].copy()
...
def find_monkey_info_for_ff_in_past_or_future(current_ff_df, last_seen_ff_df, next_seen_ff_df):
    ...
    monkey_info_when_ff_last_seen_df = find_monkey_info_when_ff_last_seen(current_ff_df, last_seen_ff_numbers)
    next_seen_ff_numbers = next_seen_ff_df['ff_number'].values
    monkey_info_when_ff_next_seen_df = find_monkey_info_when_ff_next_seen(current_ff_df, next_seen_ff_numbers)
    monkey_info_for_ff_in_past_or_future = pd.concat([monkey_info_when_ff_last_seen_df, monkey_info_when_ff_next_seen_df], axis=0).reset_index(drop=True)
