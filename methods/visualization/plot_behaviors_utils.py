...
def show_trajectory_func(axes, player, cum_iloc_indices, cum_mxy_rotated, cum_t, cum_speed, monkey_information, 
    ...
    if show_eye_positions:
        axes.scatter(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, marker='o', s=trail_size[player], alpha=trail_alpha[player], c=cum_t, cmap='gist_ncar', zorder=3)
        ...
        axes.scatter(cum_mxy_rotated[0, points_to_mark]-x0, cum_mxy_rotated[1, points_to_mark]-y0, marker='*', s=160, alpha=trail_alpha[player], c="orange", zorder=1)
    elif trail_color_var == 'target_visibility': 
            axes.scatter(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, marker='o', s=trail_size[player], alpha=trail_alpha[player], c="orange", zorder=1)          
    elif trail_color_var == 'time': 
            axes.scatter(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, marker='o', s=trail_size[player], alpha=trail_alpha[player], c=cum_t-min(cum_t), cmap='viridis', zorder=3)   
    elif trail_color_var is None:
        if not hitting_arena_edge:
            axes.plot(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, alpha=trail_alpha[player], c="black", zorder=3, linewidth=2)
        else: 
            axes.scatter(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, marker='o', s=2, alpha=trail_alpha[player], c="black", zorder=3)
    else: # then trail_color_var will be considered as a color
        if not hitting_arena_edge:
            axes.plot(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, alpha=trail_alpha[player], c=trail_color_var, zorder=3, linewidth=2)
        else: 
            axes.scatter(cum_mxy_rotated[0]-x0, cum_mxy_rotated[1]-y0, marker='o', s=2, alpha=trail_alpha[player], c=trail_color_var, zorder=3)        
    return axes
    ...
    if temp_ff_positions.shape[1] > 0:
        for j in range(temp_ff_positions.shape[1]):
            ...
            if show_dots:
                ...
                axes.scatter(temp_ff_positions[0, j]-x0, temp_ff_positions[1, j]-y0, alpha=0.15, s=15, color=dot_color, zorder=3)
    return axes     
...
def find_alive_ff(duration, ff_life_sorted, ff_real_position_sorted, rotation_matrix=None):
    ...
    alive_ff_position_rotated = np.stack((alive_ff_positions.T[0], alive_ff_positions.T[1]))
    if (rotation_matrix is not None) & (alive_ff_position_rotated.shape[1] > 0):
        alive_ff_position_rotated = np.matmul(rotation_matrix, alive_ff_position_rotated)
...
def find_believed_target_positions(ff_believed_position_sorted, currentTrial, num_trials, rotation_matrix=None):
    ...
    believed_target_positions_rotated = np.stack((believed_target_positions.T[0], believed_target_positions.T[1]))
    if (rotation_matrix is not None) & (believed_target_positions_rotated.shape[1]>0):
        believed_target_positions_rotated = np.matmul(rotation_matrix, believed_target_positions_rotated)
...
def find_lines_to_connect_path_ff(ff_dataframe_in_duration, target_indices, rotation_matrix=None, target_excluded = False):
    ...
    if (rotation_matrix is not None) & (temp_ff_positions_rotated.shape[1]>0):
        temp_ff_positions_rotated = np.matmul(rotation_matrix, temp_ff_positions_rotated)
        temp_monkey_positions_rotated = np.matmul(rotation_matrix, temp_monkey_positions_rotated)
...
def plot_lines_to_connect_path_and_ff(axes, ff_info, rotation_matrix, x0, y0, linewidth, alpha, vary_color_for_connecting_path_ff=False, line_color="#a940f5", show_connect_path_ff_except_targets=False, 
    ...
    if vary_color_for_connecting_path_ff:
        ...
        for i in range(len(unique_ff_index)):
            ...
            color = np.append(varying_colors[i % 9], 0.5)  # take out a color from Set2
            temp_df = ff_info[ff_info['ff_index'] == ff_index]
            temp_ff_positions, temp_monkey_positions = find_lines_to_connect_path_ff(temp_df, target_indices, rotation_matrix=rotation_matrix, target_excluded = show_connect_path_ff_except_targets)
            if temp_monkey_positions.shape[1] > 0:
                ...
                if show_points_when_ff_start_being_visible or show_points_when_ff_stop_being_visible:
                    if show_points_when_ff_start_being_visible:
                        marker = axes.scatter(temp_monkey_positions[0, 0]-x0, temp_monkey_positions[1, 0]-y0, alpha=0.7, marker="X", s=80, color=color, zorder=4)
        ...
        temp_ff_positions, temp_monkey_positions = find_lines_to_connect_path_ff(ff_info, target_indices, rotation_matrix=rotation_matrix, target_excluded = show_connect_path_ff_except_targets)
...
def find_dict_of_perpendicular_lines_to_monkey_trajectory_at_certain_points(all_point_index, all_breaking_points, monkey_information, rotation_matrix):
    all_starting_points = np.insert(all_point_index[all_breaking_points], 0, all_point_index[0])
    all_ending_points = np.append(all_point_index[all_breaking_points-1], all_point_index[-1])

    starting_left_xy_rotated, starting_right_xy_rotated = find_perpendicular_lines_to_monkey_trajectory_at_certain_points(all_starting_points, monkey_information, rotation_matrix)
    ending_left_xy_rotated, ending_right_xy_rotated = find_perpendicular_lines_to_monkey_trajectory_at_certain_points(all_ending_points, monkey_information, rotation_matrix)
...
def find_perpendicular_lines_to_monkey_trajectory_at_certain_points(certain_points, monkey_information, rotation_matrix, line_half_length=50):
        ...
        right_xy_rotated = np.matmul(rotation_matrix, np.vstack((right_x, right_y)))
...
def plot_horizontal_lines_to_show_ff_visible_segments(axes, ff_info, monkey_information, rotation_matrix, x0, y0, legend_markers=[], legend_names=[],
    ...
    for i, ff_index in enumerate(unique_ff_indices):
        ...
        perp_dict = find_dict_of_perpendicular_lines_to_monkey_trajectory_at_certain_points(all_point_index, all_breaking_points, monkey_information, rotation_matrix)

        # Show firefly position
        if how_to_show_ff == 'square':
            axes.scatter(ff_position_rotated[0]-x0, ff_position_rotated[1]-y0, marker='s', alpha=0.75, s=170, color=color, zorder=3)
        elif how_to_show_ff == 'circle':
            circle = plt.Circle((ff_position_rotated[0]-x0, ff_position_rotated[1]-y0), 25, facecolor=color, edgecolor=None, alpha=0.75, zorder=1)
            axes.add_patch(circle)

        # Find and plot beginning and end of each visible segment
        for j in range(len(all_breaking_points)+1):
            one_perp_dict = find_one_pair_of_perpendicular_lines(perp_dict, j, x0, y0)
            ...
            if not legend_markers:
                ...
                legend_names.extend(['Points when fireflies start being visible', 'Points when fireflies stop being visible'])
...
def find_ff_in_cluster(cluster_dataframe_point, ff_real_position_sorted, currentTrial, rotation_matrix=None):
    ...
    cluster_ff_rotated = np.stack((cluster_ff_positions.T[0], cluster_ff_positions.T[1]))
...
def find_ff_in_cluster_around_target(cluster_around_target_indices, ff_real_position_sorted, currentTrial, rotation_matrix=None):
    ...
    cluster_around_target_rotated = np.stack((cluster_ff_positions.T[0], cluster_ff_positions.T[1]))
    if (rotation_matrix is not None) & (cluster_around_target_rotated.shape[1] > 0):
        cluster_around_target_rotated = np.matmul(rotation_matrix, cluster_around_target_rotated)
...
def set_xy_limits_for_axes(axes, mx_min, mx_max, my_min, my_max, minimal_margin=50, max_margin=200, zoom_in=False):
    ...
    if zoom_in is True:
        if minimal_margin <= 40:
            ...
            axes.set_ylim((ymin - minimal_margin*2/3, ymax + minimal_margin*4/3))
    else:
...
def readjust_xy_limits_for_axes(axes, cum_mxy_rotated_1, cum_mxy_rotated_2, shown_ff_indices_1, shown_ff_indices_2, R, ff_real_position_sorted, minimal_margin=50):
    cum_mxy_rotated_all = np.concatenate((cum_mxy_rotated_1, cum_mxy_rotated_2), axis=1)
    ...
    shown_ff_positions_rotated_all = np.concatenate((shown_ff_positions_rotated_1, shown_ff_positions_rotated_2), axis=1)
    mx_min, mx_max, my_min, my_max = find_xy_min_max_for_plots(cum_mxy_rotated_all, x0=0, y0=0, temp_ff_positions=shown_ff_positions_rotated_all)
...
def update_plot_limits(xmin, ymin, xmax, ymax, cum_mxy_rotated):
    ...
    temp_xmin, temp_xmax = np.min(cum_mxy_rotated[0])-x0, np.max(cum_mxy_rotated[0])-x0
    temp_ymin, temp_ymax = np.min(cum_mxy_rotated[1])-y0, np.max(cum_mxy_rotated[1])-y0
...
def _show_eye_positions(axes, x0, y0, marker, cum_iloc_indices, overall_valid_indices, gaze_world_xy_rotated):
    ...
    return axes, gaze_world_xy_rotated_valid

def _show_connect_path_eye_positions(gaze_world_xy_rotated_valid, cum_mxy_rotated, overall_valid_indices, axes, x0, y0, player, connection_alpha, connection_linewidth, sample_ratio=4):
    sample = np.arange(1, gaze_world_xy_rotated_valid.shape[1], sample_ratio)
    axes = connect_points_to_points(axes, gaze_world_xy_rotated_valid[:, sample], cum_mxy_rotated[:,overall_valid_indices][:, sample], 
