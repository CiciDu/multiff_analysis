...
class PlanningAndNeural(plan_factors_class.PlanFactors):
    ...
    def get_all_planning_info(self, time_bins, both_ff_across_time_df_exists_ok=True):
        ...
        heading_columns_to_add = ['stop_point_index', 'angle_from_m_before_stop_to_stop_ff', 'angle_from_m_before_stop_to_alt_ff',
                                'angle_from_stop_ff_landing_to_alt_ff']
        self.all_planning_info = self.all_planning_info.merge(self.heading_info_df[heading_columns_to_add], on='stop_point_index', how='left')
    ...
    def get_both_ff_across_time_df(self, exists_ok=True):
        ...
        if exists_ok and os.path.exists(df_path):
            ...
            self._get_point_index_based_on_some_time_before_stop(n_seconds_before_stop=2)
            for i, row in self.stops_near_ff_df.iterrows():
                if i % 10 == 0:
                    print(f'Having processed {i} rows out of {len(self.stops_near_ff_df)} of the stops_near_ff_df for both_ff_across_time_df.')
                info_to_add = self._get_info_to_add(row)
                self.both_ff_across_time_df = pd.concat([self.both_ff_across_time_df, info_to_add], axis=0)
    ...
    def _get_info_to_add(self, row):
        info_to_add = self.mid_bins[self.mid_bins['point_index'].between(row['point_index_in_the_past'], row['next_stop_point_index'])].copy()
        ...
        for which_ff_info in ['alt_', 'stop_']:
            info_to_add, columns_added = self._add_ff_info_to_info_to_add(info_to_add, row, which_ff_info)
    ...
    def _check_for_duplicate_bins(self):
        if self.both_ff_across_time_df['bin'].duplicated().any():
            ...
            self.both_ff_across_time_df = self.both_ff_across_time_df.sort_values(by='stop_point_index', ascending=True).drop_duplicates(subset='bin', keep='last')

    def _find_ff_info(self, row, all_point_index):
        self.alt_ff_df2 = find_stops_near_ff_utils.find_ff_info(np.repeat(row.alt_ff_index, len(all_point_index)), all_point_index, self.monkey_information, self.ff_real_position_sorted)
        self.stop_ff_df2 = find_stops_near_ff_utils.find_ff_info(np.repeat(row.stop_ff_index, len(all_point_index)), all_point_index, self.monkey_information, self.ff_real_position_sorted)
