...
def make_target_df(monkey_information, ff_caught_T_new, ff_real_position_sorted, ff_life_sorted, ff_dataframe):
    target_df = _initialize_target_df(monkey_information, ff_caught_T_new, ff_real_position_sorted)
    target_df = _calculate_target_distance_and_angle(target_df)
    target_df, nearby_alive_ff_indices = _add_target_and_target_cluster_last_seen_info(target_df, ff_real_position_sorted, ff_caught_T_new, ff_life_sorted, ff_dataframe)
    target_df = _add_disappeared_for_last_time_dummy(target_df, ff_caught_T_new, ff_dataframe, nearby_alive_ff_indices)
    target_df = _add_visible_dummy(target_df)
    target_df = _find_time_since_last_capture(target_df, ff_caught_T_new)
    target_df = _add_while_last_seeing_target_cluster_dummy(target_df)
    return target_df
...
def _initialize_target_df(monkey_information, ff_caught_T_new, ff_real_position_sorted):
    """
...
def _add_target_and_target_cluster_last_seen_info(target_df, ff_real_position_sorted, ff_caught_T_new, ff_life_sorted, ff_dataframe):
    """
...
def _add_disappeared_for_last_time_dummy(target_df, ff_caught_T_new, ff_dataframe, nearby_alive_ff_indices):
    ...
    for target in range(len(ff_caught_T_new)):
        target_visible_info = ff_dataframe[(ff_dataframe['ff_index']==target) & (ff_dataframe['visible']==1)]
    ...
    for target in range(len(ff_caught_T_new)):
        ...
        target_visible_info = ff_dataframe[(ff_dataframe['ff_index'].isin(target_cluster_indices)) & (ff_dataframe['visible']==1)]
        target_last_vis_time = target_visible_info['time'].max()
...
def add_target_last_seen_info_to_target_df(target_df, ff_dataframe, nearby_alive_ff_indices=None, use_target_cluster=False, include_frozen_info=False):
    """
...
def _update_info_for_current_target_index(target_df, ff_dataframe, target_index, nearby_alive_ff_indices, use_target_cluster, include_frozen_info):
    ...
    target_sub_row_indices = np.array((target_df['target_index'] == target_index)).nonzero()[0]
    ...
    target_visible_info = _get_target_info_when_visible_for_current_target_index(ff_dataframe, target_index, nearby_alive_ff_indices, use_target_cluster)
    ...
    if len(target_df_sub) > 0:
        ...
        unique_time_points = target_df_sub[['point_index', 'time', 'monkey_x', 'monkey_y', 'monkey_angle']].drop_duplicates().sort_values(by='point_index')
        # Adjust the duration contained in unique_time_points
        unique_time_points, target_visible_info, target_df_sub, target_sub_row_indices = _adjust_duration_of_unique_time_points(unique_time_points, target_df_sub, target_visible_info, target_sub_row_indices)
        target_visible_info = _polish_target_info(target_visible_info)

        if len(target_visible_info) > 0:
            unique_time_points = _get_last_seen_info(unique_time_points, target_visible_info, include_frozen_info)
            # put back the updated part of target_df
            target_df = _update_target_df_for_current_target_index(target_df, target_df_sub, target_sub_row_indices, unique_time_points)
    return target_df
...
def _get_target_info_when_visible_for_current_target_index(ff_dataframe, target_index, nearby_alive_ff_indices, use_target_cluster):
    if use_target_cluster:
        ...
        target_visible_info = ff_dataframe[(ff_dataframe['ff_index'].isin(target_cluster_indices)) & (ff_dataframe['visible'] == 1)].copy()
    else:
        target_visible_info = ff_dataframe[(ff_dataframe['ff_index'] == target_index) & (ff_dataframe['visible'] == 1)].copy()
...
def _adjust_duration_of_unique_time_points(unique_time_points, target_df_sub, target_visible_info, target_sub_row_indices):
    ...
    if len(target_visible_info) > 0:
        ...
        if len(target_info_before) > 0:
            target_visible_info = target_visible_info[target_visible_info['point_index'] >= target_info_before.iloc[-1].point_index]
            ...
            unique_time_points = unique_time_points.drop_duplicates(subset=['point_index'], keep='first')
            ...
            valid_points = np.where(np.array(unique_time_points['point_index']) >= target_visible_info.point_index.min())[0]
...
def _polish_target_info(target_visible_info):
    target_visible_info = target_visible_info.sort_values(by=['point_index', 'ff_index']).drop_duplicates(subset=['point_index'], keep='first')
    target_visible_info = target_visible_info[['point_index', 'time', 'ff_x', 'ff_y', 'monkey_x', 'monkey_y', 'monkey_angle']]
...
def _get_last_seen_info(unique_time_points, target_visible_info, include_frozen_info):
    ...
    """
    unique_time_points = unique_time_points.merge(target_visible_info, how='left', on='point_index')
...
def _update_target_df_for_current_target_index(target_df, target_df_sub, target_sub_row_indices, unique_time_points):
    ...
    """
    target_df_sub_new = target_df_sub[['point_index']].merge(unique_time_points, on='point_index', how='left')
    target_df.iloc[target_sub_row_indices, target_df.columns.get_indexer(target_df_sub_new.columns)] = target_df_sub_new.values
    return target_df
