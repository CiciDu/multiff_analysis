...
from non_behavioral_analysis.neural_data_analysis.neural_vs_behavioral import prep_monkey_data, prep_monkey_data, prep_monkey_data, prep_target_data, neural_data_modeling, reduce_multicollinearity
...
class NeuralVsBehavioralClass(further_processing_class.FurtherProcessing):
    ...
    def prep_behavioral_data_for_neural_data_modeling(self):
        ...
        self.make_final_behavioral_data()

    def make_final_behavioral_data(self):
        self.final_behavioral_data = prep_monkey_data.make_final_behavioral_data(self.rebinned_monkey_info_essential, self.binned_features)
    ...
    def make_or_retrieve_y_var_lr_result_df(self, exists_ok=True):
        df_path = os.path.join(self.processed_neural_data_folder_path, 'lr_result_df', 'y_var_lr_result_df.csv')
    ...
    def match_binned_spikes_to_range_of_behavioral_data(self):
        self.max_bin = self.final_behavioral_data['bin'].max()
        self.binned_spikes_matrix, self.binned_spikes_df = neural_data_processing.prepare_binned_spikes_matrix_and_df(self.max_bin, self.all_binned_spikes)

    def get_x_and_y_var(self):
        self.match_binned_spikes_to_range_of_behavioral_data()
        self.x_var = self.binned_spikes_df.copy()
        self.y_var = self.final_behavioral_data.copy()

    def get_x_and_y_var_lags(self, max_lag_number=3):
    ...
    def make_or_retrieve_y_var_lags_reduced(self, exists_ok=True):
        df_path = os.path.join(self.processed_neural_data_folder_path, 'y_var_lags', f'y_var_lags_{self.max_lag_number}_reduced.csv')
