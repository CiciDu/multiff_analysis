...
fname = fullfile(original_file_path, strcat(file_name, '.plx'));

[~, ts, sv, freq] = plx_event_ts_modified(fname, 257); 
ts_s = ts/freq; ts_s = ts_s.'; sv = sv.'; ts = ts.';
for_aligning_data = table(sv, ts, ts_s);
writetable(for_aligning_data, fullfile(path, strcat(file_name, '.txt')))
type(fullfile(path, strcat(file_name, '.txt')))
