...
def run_segment_split_regression_cv(x_var, y_var, columns_of_interest, num_folds=5, segment_column='new_segment', verbose=False):
    results = regress_by_variable_type_cv(
        x_var, y_var, columns_of_interest, num_folds=num_folds, segment_column=segment_column, verbose=verbose)
    return results
...
def make_barplot_to_compare_results(results_df, metric, targets=None):
    wide_df = (
        results_df
        .pivot_table(
...
def _make_barplot_to_compare_results(wide_df, metric, targets=None):
    # Example: compare 'Accuracy_Mean' for each Target across 3 test_or_control groups

    if targets is None:
        targets = wide_df['Target'].unique()
    groups = wide_df['test_or_control'].unique()

    # Prepare data matrix: rows=targets, cols=groups
    data = []
    for t in targets:
        vals = []
        for g in groups:
    ...
    for i, group in enumerate(groups):
        ax.bar(x + i*bar_width, data[:, i], width=bar_width, label=group)

    ax.set_xticks(x + bar_width)
    ax.set_xticklabels(targets, rotation=45, ha='right')
    ax.set_ylabel(metric)
    ax.set_title(f'Comparison of {metric} across test_or_control groups')
