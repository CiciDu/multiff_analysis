...
class LSTMforMultifirefly(rl_for_multiff_class._RLforMultifirefly):
    ...
    def make_agent(self,
        ...
        self.agent_params = {
            ...
            "action_range": 1.0,
            "replay_buffer_size": kwargs.get('replay_buffer_size', 500), # used 100 before
        ...
        self.replay_buffer = LSTM_functions.ReplayBufferLSTM2(self.agent_params['replay_buffer_size'])
    ...
    def _make_initial_env_for_curriculum_training(self, initial_dt=0.25, initial_flash_on_interval=3.3, initial_angular_terminal_vel=1):
        ...
        self._update_env_flash_on_interval(flash_on_interval=initial_flash_on_interval)
    ...
    def _update_env_dt(self, dt=0.25):
        self.env.dt = dt
        self.sac_model.gamma = rl_for_multiff_utils.calculate_model_gamma(self.env.dt)
        self.env.visible_time_range = self.env.flash_on_interval + self.env.dt
    ...
    def _change_env_after_meeting_reward_threshold(self):
        flash_on_interval = max(self.env.flash_on_interval - 0.3, self.env_kwargs['flash_on_interval'])
    ...
    def _use_while_loop_for_curriculum_training(self, eval_eps_freq=20):
        ...
        while self.env.flash_on_interval > self.env_kwargs['flash_on_interval']:
            ...
            num_eval_episodes = 1
            reward_threshold = rl_for_multiff_utils.calculate_reward_threshold_for_curriculum_training(self.env, n_eval_episodes=num_eval_episodes, ff_caught_rate_threshold=1/5)
    ...
    def regular_training(self, num_train_episodes=10000, eval_eps_freq=15, num_eval_episodes=2, 
        ...
        if dir_name is not None:
            if env_params_to_save is None:
                ...
                self.store_env_params(model_folder_name=dir_name, env_params_to_save=env_params_to_save)
    ...
    def save_agent(self, whether_save_replay_buffer=True, dir_name=None):
        ...
        LSTM_functions.save_best_model(self.sac_model, whether_save_replay_buffer=whether_save_replay_buffer, dir_name=dir_name)
    ...
    def load_agent(self, load_replay_buffer=True, dir_name=None, model_name=''): 
        ...
        try:
            ...
            print(f"There's a problem retrieving existing agent or replay_buffer found in {dir_name}. Need to train a new agent. Error message {e}")
            raise ValueError(f"There's a problem retrieving existing agent or replay_buffer found in {dir_name}. Need to train a new agent.")
