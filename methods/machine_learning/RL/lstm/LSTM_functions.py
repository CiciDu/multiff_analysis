...
class SAC_Trainer():
    ...
    def __init__(self, **kwargs):
        ...
        self.device = kwargs.get('device', "mps" if torch.backends.mps.is_available() else "cpu",)  # Default to 'cpu' if not provided
        ...
        self.soft_q_net1 = QNetworkLSTM2(state_space, action_space, self.hidden_dim).to(device)
        self.soft_q_net2 = QNetworkLSTM2(state_space, action_space, self.hidden_dim).to(device)
        self.target_soft_q_net1 = QNetworkLSTM2(state_space, action_space, self.hidden_dim).to(device)
        self.target_soft_q_net2 = QNetworkLSTM2(state_space, action_space, self.hidden_dim).to(device)
        self.policy_net = SAC_PolicyNetworkLSTM(state_space, action_space, self.hidden_dim, action_range).to(device)
    ...
    def update(self, device=device):
        ...
        hidden_in, hidden_out, state, action, last_action, reward, next_state, done = self.replay_buffer.sample(self.batch_size)
    ...
    def load_model(self, path, device='cpu'):
        self.soft_q_net1.load_state_dict(torch.load(path + '/lstm_q1', map_location=torch.device(device)))
        self.soft_q_net2.load_state_dict(torch.load(path + '/lstm_q2', map_location=torch.device(device)))
        self.policy_net.load_state_dict(torch.load(path + '/lstm_policy', map_location=torch.device(device)))
...
def train_LSTM_agent(env, sac_model, 
    ...
    for eps in range(num_train_episodes):
        ...
        if eps % eval_eps_freq == 0 and eps > 0:
            ...
            avg_reward = evaluate_agent(env, sac_model, max_steps_per_eps, num_eval_episodes, deterministic=True)
            print(f"Best average reward: {best_avg_reward}, Current average reward: {avg_reward}")
        ...
        alpha_dict = {'epi': list_of_epi_for_alpha, 'alpha': list_of_alpha, 'rewards': list_of_epi_rewards}
...
def _train_episode(env, sac_model, max_steps_per_eps):
    ...
    hidden_out = _initialize_hidden_state(sac_model.hidden_dim, sac_model.device)
