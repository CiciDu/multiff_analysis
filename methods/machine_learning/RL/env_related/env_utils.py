...
def make_ff_flash_from_random_sampling(num_alive_ff, duration, non_flashing_interval_mean=3, flash_on_interval=0.3):
    ...
    for i in range(num_alive_ff):
        ...
        if max(t1) < duration:
            raise ValueError('The flashing-on duration is too short for the given duration')
    return ff_flash
...
def update_noisy_ffxy(ffx_noisy, ffy_noisy, ffx, ffy, ff_uncertainty_all, visible_ff_indices):
    ...
    ffx_noisy = ffx_noisy + torch.normal(torch.zeros([num_alive_ff, ]), ff_uncertainty_all)
    ffy_noisy = ffy_noisy + torch.normal(torch.zeros([num_alive_ff, ]), ff_uncertainty_all)
