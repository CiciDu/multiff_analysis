...
class EnvForSB3(base_env.MultiFF):
    ...
    def _get_ff_array_for_belief(self):
        self._get_ff_array_for_belief_common(self.ff_in_memory_indices, add_memory=True, add_ff_time_since_start_visible=False)
...
class EnvForSB3_2(more_envs.MultiFF_2):  
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.visible_time_range = self.flash_on_interval + self.max_in_memory_time + self.dt
    ...
    def _update_ff_time_since_start_visible(self):
        self.not_in_memory_ff_indices = torch.tensor(list(set(range(self.num_alive_ff)) - set(self.ff_in_memory_indices.tolist())), dtype=torch.int)
        self._update_ff_time_since_start_visible_base_func(self.not_in_memory_ff_indices)
