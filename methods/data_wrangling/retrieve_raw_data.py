    ...
    for index, file_name in enumerate(smr_file_paths):  # loop 2 files one by one
        ...
        for C_index, C_data in enumerate(seg_reader.analogsignals[:-1]): # -1 indicates we discard 'Raw' channel   
            ...
            for i in range(shape):
                Channel_index.append(str(seg_reader.analogsignals[C_index].name))
...
def get_trimmed_monkey_information(raw_data_folder_path):
    ...
    monkey_information = _trim_monkey_information(raw_monkey_information, smr_markers_start_time, smr_markers_end_time)
...
def make_or_retrieve_ff_info_from_txt_data(raw_data_folder_path, exists_ok=True, save_data=True):
    ...
    processed_data_folder_path = raw_data_folder_path.replace('raw_data', 'processed_data')
    ...
    if exists_ok:
        try:
            ...
            print(f"Failed to retrieve ff info from txt data. Will get the data anew. Error: {e}")
...
def make_or_retrieve_ff_flash_sorted_from_txt_data(raw_data_folder_path, exists_ok=True, save_data=True):
    ...
    if exists_ok:
        try:
            ff_flash_sorted = _retrieve_ff_flash_sorted_in_npz_from_txt_data(processed_data_folder_path)
        except FileNotFoundError as e:
            print("Failed to retrieve ff_flash_sorted from txt data. Will get the data anew. Error: ", e)
            ff_flash_sorted = _get_ff_flash_sorted_from_txt_data(raw_data_folder_path, save_data=save_data)
    else:
        ff_flash_sorted = _get_ff_flash_sorted_from_txt_data(raw_data_folder_path, save_data=save_data)
...
def _get_ff_info_from_txt_data(raw_data_folder_path, save_data=True):
    ...
    smr_markers_start_time, smr_markers_end_time = time_calib_utils.find_smr_markers_start_and_end_time(raw_data_folder_path, ff_caught_T_sorted=ff_caught_T_sorted)
...
def _get_ff_flash_sorted_from_txt_data(raw_data_folder_path, save_data=True):
    ...
    if save_data:
        processed_data_folder_path = raw_data_folder_path.replace('raw_data', 'processed_data')
        npz_flash = os.path.join(os.path.join(processed_data_folder_path, 'ff_flash_sorted.npz'))
...
def _retrieve_ff_flash_sorted_in_npz_from_txt_data(processed_data_folder_path):
    npz_file_for_flash = os.path.join(os.path.join(processed_data_folder_path, 'ff_flash_sorted.npz'))
