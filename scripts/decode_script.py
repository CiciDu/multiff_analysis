...
def main():
    ...
    # Load comparison definitions (robust to empty/invalid JSON)
    comps = None
    if os.path.exists(args.comparisons):
        try:
            with open(args.comparisons, "r") as f:
                raw = f.read()
            if raw.strip():
                all_comps = json.loads(raw)
                comps = [all_comps[args.idx]
                         ] if args.idx is not None else all_comps
            else:
                print(
                    f"[warning] {args.comparisons} is empty; using defaults from dataset.")
        except json.JSONDecodeError as e:
            print(
                f"[warning] Could not parse {args.comparisons} as JSON ({e}); using defaults from dataset.")
    else:
        print(
            f"[warning] {args.comparisons} not found; using defaults from dataset.")
    ...
    try:
        ...
        print(
            f"[warning] Could not parse model_kwargs JSON: {args.model_kwargs}")
        model_kwargs = {}
