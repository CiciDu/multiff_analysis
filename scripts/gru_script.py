...
print(f"[device check] CUDA available: {torch.cuda.is_available()}")
print(f"[device check] Using device: {torch.cuda.get_device_name(0)}")
import time
start_time = time.time()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train or evaluate GRU agent on Multifirefly env")
    # ------------------------------
    ...
    # ------------------------------
    parser.add_argument("--overall-folder", type=str, default='multiff_analysis/RL_models/GRU_stored_models/all_agents/oct_18_3/',
    ...
    try:
        # Optionally, include a simple metric at end if available
        metrics = {
            'best_avg_reward': getattr(rl, 'best_avg_reward', None)
        }
        run_logger.log_run_end(overall_folder, agent_type='gru', sweep_params=sweep_params, status='finished', metrics=metrics)
    except Exception as e:
        print('[logger] failed to log run end:', e)

    # Collect model directories from this run into a per-job folder
    try:
        # Agent root with manifests
