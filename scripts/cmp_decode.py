...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Compare decoding results across models")
    ap.add_argument(
        "--base",
        type=str,
...
def _summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"window_start", "window_end", "mean_auc"}.issubset(df.columns):
        df = df.assign(window_center=(df["window_start"] + df["window_end"]) / 2.0)

    group_cols = [
        c for c in ("a_label", "b_label", "align_by_stop_end", "model_name") if c in df.columns
    ]
    if not group_cols:
        raise ValueError("Input dataframe lacks necessary columns to summarize.")
    ...
    summary = df.groupby(group_cols, as_index=False).apply(_agg_peak, include_groups=False)
