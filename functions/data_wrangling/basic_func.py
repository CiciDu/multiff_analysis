...
def calculate_change_in_abs_ff_angles(current_ff_index, angles_to_ff, angles_to_boundaries, ff_real_position_sorted, monkey_x_array, monkey_y_array, monkey_angles_array, in_memory_indices):
...
def distance_traveled(currentTrial, ff_caught_T_sorted, monkey_information):
    ...
    if len(cum_indices) > 1:
        ...
        cum_speed = np.array(monkey_information['monkey_speed'].iloc[cum_indices])
...
def find_stops(currentTrial, ff_caught_T_sorted, monkey_information, player = "monkey", return_index = False, 
    ...
    if len(cum_indices) > 5:
      cum_mx, cum_my = np.array(monkey_information['monkey_x'].iloc[cum_indices]), np.array(monkey_information['monkey_y'].iloc[cum_indices])
      cum_speeddummy = np.array(monkey_information['monkey_speeddummy'].iloc[cum_indices])
...
def put_stops_into_clusters(currentTrial, distance_between_points, ff_caught_T_sorted, monkey_information, player = "monkey"):
    """
    ...
    distinct_stops = find_stops(currentTrial, ff_caught_T_sorted, monkey_information, player = player)
...
def retrieve_and_merge(df_name = 'pattern_frequencies', data_directory_list = ['all_monkey_data/individual_monkey_data']):
...
def find_currentTrial_or_num_trials_or_duration(ff_caught_T_sorted, currentTrial=None, num_trials=None, duration=None):
    ...
    if num_trials is None:
        ...
        if len(trials_after_first_capture) > 0:
            num_trials = max(1, currentTrial-trials_after_first_capture[0])
        else:
            num_trials = 1
    if duration is None:
