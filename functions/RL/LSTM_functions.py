...
class SAC_Trainer():
    def __init__(self, replay_buffer, state_space, action_space, hidden_dim, action_range, gamma, soft_q_lr, policy_lr,
                 alpha_lr, batch_size, update_itr, reward_scale, target_entropy, soft_tau, train_freq, device=device):
    ...
    def update(self, batch_size, reward_scale=10., auto_entropy=True, target_entropy=-2, gamma=0.975, soft_tau=1e-2, device=device):
...
def train_LSTM_agent(env, sac_model, device, log_dir = None, 
                     action_dim=2, hidden_dim=128, train_freq=100, batch_size=10, update_itr=1,
                     num_train_episodes=10000, eval_freq=15, max_steps=1024, AUTO_ENTROPY=True, 
                     DETERMINISTIC=False, num_eval_episodes=3, best_avg_reward=-9999,
                     best_avg_reward_record=-9999, print_episode_reward = False):
    eval_rewards = []
    ...
    for eps in range(num_train_episodes):
        ...
        for step in range(max_steps):
            ...
            action, hidden_out = sac_model.policy_net.get_action(state, last_action, hidden_in, deterministic = False)
            ...
            if step % train_freq == 0:
                if len(sac_model.replay_buffer) > batch_size:
                    for i in range(update_itr):
                        # Train the networks
                        _=sac_model.update(batch_size, reward_scale=10., auto_entropy=AUTO_ENTROPY, target_entropy=-1.*action_dim)
            if done:
                break
        sac_model.replay_buffer.push(ini_hidden_in, ini_hidden_out, episode_state, episode_action, episode_last_action, episode_reward, episode_next_state, episode_done)


        # For every 15 episodes of training, we will do a few episodes (num_eval_episodes) of evaluation
        if eps % 15 == 0 and eps > 0: # plot and model saving interval
            cum_reward = 0     
            for eval_eps in range(num_eval_episodes):
                state =  env.reset()
                last_action = env.action_space.sample()
                hidden_out = (torch.zeros([1, 1, hidden_dim], dtype=torch.float).to(device), torch.zeros([1, 1, hidden_dim], dtype=torch.float).to(device))              
                for step in range(max_steps):
                    ...
                    state = next_state
                    last_action = action
            avg_reward = cum_reward/num_eval_episodes
            print("                      Best average reward:", best_avg_reward, "Current average reward", avg_reward)
            if avg_reward > best_avg_reward:
                best_avg_reward = avg_reward
                if best_avg_reward > best_avg_reward_record:
                    best_avg_reward_record = best_avg_reward
                if log_dir is not None:
                    sac_model.save_model(log_dir)
            ...
            # If curriculum training has not finished yet:
            if env.flash_on_interval > 0.31:
                if avg_reward >= 1000:
                    env.decrease_flash_on_interval = True
                    # Since the task has become harder, we decrease the threshdold of reward above which the model will be saved
                    best_avg_reward = best_avg_reward/2
            else:
                # Stop curriculum training 
                env.distance2center_cost = 0

                # Also begin to plot the avg_rewards for all the instances (or the past 100 instances) of evaluation since the curriculum training has finished
