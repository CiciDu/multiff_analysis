from multiff_analysis.functions.data_wrangling import basic_func, find_patterns, make_ff_dataframe
...
def collect_agent_data_func(env, sac_model, n_steps = 15000, LSTM = False, hidden_dim= 128, device = "cpu", deterministic = True, first_obs = None):
    ...
    for step in range(n_steps):
        ...
        for ff in range(len(env.topk_indices)):
            index = env.topk_indices[ff]
            ...
            ff_noisy_xy_in_obs.append([last_corresponding_ff_index, step, env.ffxy_topk_noisy[ff][0].item(), env.ffxy_topk_noisy[ff][1].item()])
    ...
    monkey_information = pack_monkey_information(monkey_t, monkey_x, monkey_y, monkey_speed, monkey_angles, env.dt)
    ...
    ff_noisy_xy_in_obs = pd.DataFrame(ff_noisy_xy_in_obs, columns =['ff_index', 'point_index', 'ff_x_noisy', 'ff_y_noisy'], dtype = float) 
    ff_noisy_xy_in_obs['ff_index'] = reversed_sorting[np.array(ff_noisy_xy_in_obs['ff_index']).astype(int)]
    ff_noisy_xy_in_obs = ff_noisy_xy_in_obs.astype({'ff_index':'int', 'point_index':'int'})
...
def pack_monkey_information(monkey_t, monkey_x, monkey_y, monkey_speed, monkey_angles, dt):
    """
...
def find_corresponding_info_of_agent(info_of_monkey, currentTrial, num_trials, sac_model, agent_dt, LSTM=False, env_kwargs=None):
    ...
    env_kwargs['num_ff'] = len(alive_ffs)
