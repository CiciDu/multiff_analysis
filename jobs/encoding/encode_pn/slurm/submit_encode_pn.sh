...
JOB_DIR=$PROJECT_ROOT/multiff_analysis/jobs/encoding/encode_pn
...
usage() {
  cat <<EOF
Usage:
  submit_encode_pn.sh --monkey MONKEY_NAME [sbatch args]

Example:
  submit_encode_pn.sh --monkey Bruno
  submit_encode_pn.sh --monkey Schro -p cpu --time=12:00:00
EOF
}
...
# ------------------------------
sbatch \
  --job-name="encode_pn_${MONKEY_NAME}" \
  --array=0-"$ARRAY_MAX"%$MAX_CONCURRENT \
  "${FORWARD_ARGS[@]}" \
  "$JOB_DIR/slurm/encode_pn_job.slurm" \
  "$MONKEY_NAME"
