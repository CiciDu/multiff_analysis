...
JOB_DIR=$PROJECT_ROOT/multiff_analysis/jobs/encoding/encode_vis
...
usage() {
  cat <<EOF
Usage:
  submit_encode_vis.sh --monkey MONKEY_NAME [sbatch args]

Example:
  submit_encode_vis.sh --monkey Bruno
  submit_encode_vis.sh --monkey Schro -p cpu --time=12:00:00
EOF
}
...
# ------------------------------
sbatch \
  --job-name="encode_vis_${MONKEY_NAME}" \
  --array=0-"$ARRAY_MAX"%$MAX_CONCURRENT \
  "${FORWARD_ARGS[@]}" \
  "$JOB_DIR/slurm/encode_vis_job.slurm" \
  "$MONKEY_NAME"
