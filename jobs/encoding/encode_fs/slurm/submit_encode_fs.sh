...
JOB_DIR=$PROJECT_ROOT/multiff_analysis/jobs/encoding/encode_fs
...
usage() {
  cat <<EOF
Usage:
  submit_encode_fs.sh --monkey MONKEY_NAME [sbatch args]

Example:
  submit_encode_fs.sh --monkey Bruno
  submit_encode_fs.sh --monkey Schro -p cpu --time=12:00:00
EOF
}
...
# ------------------------------
sbatch \
  --job-name="encode_fs_${MONKEY_NAME}" \
  --array=0-"$ARRAY_MAX"%$MAX_CONCURRENT \
  "${FORWARD_ARGS[@]}" \
  "$JOB_DIR/slurm/encode_fs_job.slurm" \
  "$MONKEY_NAME"
