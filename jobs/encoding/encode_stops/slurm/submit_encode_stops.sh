...
JOB_DIR=$PROJECT_ROOT/multiff_analysis/jobs/encoding/encode_stops
...
usage() {
  cat <<EOF
Usage:
  submit_encode_stops.sh --monkey MONKEY_NAME [sbatch args]

Example:
  submit_encode_stops.sh --monkey Bruno
  submit_encode_stops.sh --monkey Schro -p cpu --time=12:00:00
EOF
}
