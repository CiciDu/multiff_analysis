...
SLURM_SCRIPT=$JOB_DIR/slurm/one_ff_pen_tune_job.slurm

LOG_DIR=$JOB_DIR/logs/pen_tune
mkdir -p "$LOG_DIR"

usage() {
  cat <<EOF
Usage:
  submit_one_ff_pen_tune.sh [options]
...
Examples:
  submit_one_ff_pen_tune.sh
  submit_one_ff_pen_tune.sh --units 96
  submit_one_ff_pen_tune.sh --units 128 --max-parallel 8
  submit_one_ff_pen_tune.sh -p cpu --time=24:00:00
EOF
}
...
echo "[SUBMIT] One-FF Penalty Tuning"
