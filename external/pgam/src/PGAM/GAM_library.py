...
class GAM_result(object):
    def __init__(
        ...
        for var in var_list:
            ...
            if (type(mdl_matrix) is sparse.coo_matrix) or (
                type(mdl_matrix) is sparse.csr_matrix
            ):
        ...
        self.adj_pseudo_r2 = 1 - (n_obs - 1) / (n_obs - self.edf1) * (
            1 - self.pseudo_r2
        )
    ...
    def smooth_compute(
        ...
        if seWithMean:
            ...
            se_y = np.sqrt(
                np.sum(np.dot(fX, self.cov_beta[index, :][:, index]) * fX, axis=1)
            )
        else:
            se_y = np.sqrt(
                np.sum(np.dot(fX, self.cov_beta[index, :][:, index]) * fX, axis=1)
    ...
    def compute_p_values_covariate(self, var_name, diagF, diagFF, sm_handler):
        ...
        if (r - np.floor(r) > 0.99) and (r - np.floor(r) < 1):
            ...
            k = int(
                np.floor(r)
            )  # consider also the constant term that has been removed forcing the identifiability constraint
            nu = r - k  # + 1
        ...
        if k == 1:
            ...
            block_mat = np.block(
                [
                    [D, np.zeros((D.shape[0], N - D.shape[0]))],
                    [np.zeros((N - D.shape[0], N))],
                ]
            )
        ...
        if k > 2 and k < len(eig):
            if T_correct < 10**-13:
                ...
                if nu1 < 10**-3:
                    p_val = wSumChisq_cdf(
                        [T_correct], [k - 2, 1], [1, nu2]
                    )  # chi square of df =1 multiplied by 10**-8 does not contibute
                elif nu2 < 10**-3:
...
class general_additive_model(object):
    ...
    def optim_gam(
        ...
        f_weights_and_data = weights_and_data(
        ...
        gam_results = GAM_result(
            ...
            post_trial_dur=post_trial_dur,
            time_bin=time_bin,
            compute_mutual_info=compute_MI,
    ...
    def k_fold_crossval(
        ...
        for test_idx in range(k):
            ...
            print(
                "train set size: %d - test set size: %d "
                % (sum(bool_train), sum(bool_test))
            )
            model_fit = self.optim_gam(
                var_list,
                ...
                fit_initial_beta=fit_initial_beta,
                filter_trials=bool_train,
                compute_MI=compute_MI,
                saveBetaHist=saveBetaHist,
                WLS_solver=WLS_solver,
            )
            ...
            null_dev_t = self.family.resid_dev(
                self.y[bool_test], [null_mu] * self.y[bool_test].shape[0]
            )
    ...
    def fit_full_and_reduced(
        ...
        if (not k_fold) or k_fold_reducedOnly:
            ...
            full_model, test_bool = self.k_fold_crossval(
                fold_num,
                trial_num_vec,
                var_list,
                max_iter=max_iter,
            ...
            if not reducedAdaptive:
                ...
                for var in sub_list:
                    ...
                    if sm_handler[var].is_temporal_kernel:
                        ...
                        (
                            sm_handler[var].basis_kernel,
                            sm_handler[var].B_list,
                            sm_handler[var].S_list,
                            sm_handler[var].basis_dim,
                        ) = basisAndPenalty(
                            xx,
                            sm_handler[var].knots,
                            is_cyclic=sm_handler[var].is_cyclic,
                            ord=sm_handler[var]._ord,
                            penalty_type=sm_handler[var].penalty_type,
                            xmin=sm_handler[var].xmin,
                            xmax=sm_handler[var].xmax,
                            der=sm_handler[var].der,
                            measure=sm_handler[var].measure,
                            ord_AD=ord_AD,
                            ad_knots=ad_knots,
                        )
                        sm_handler[var].X = sm_handler[var]._eval_basis_temporal(
                            sm_handler[var]._x,
                            sm_handler[var].trial_idx,
                    ...
                    sm_handler[var].lam = [reset_smooth[var_list_reset == var][0]] * (
                        len(sm_handler[var].B_list)
                    )
                    new_smooth = np.hstack((new_smooth, sm_handler[var].lam))
                smooth_pen = new_smooth
            if not k_fold:
                reduced_model = self.optim_gam(
                    sub_list,
                    max_iter=max_iter,
                    tol=tol,
                    conv_criteria=conv_criteria,
                ...
                reduced_model, test_bool = self.k_fold_crossval(
                    fold_num,
                    trial_num_vec,
                    sub_list,
                    max_iter=max_iter,
        ...
        if pseudoR2_per_variable and (not reduced_model is None):
            reduced_model.variable_expl_variance = np.zeros(
                len(reduced_model.var_list) + 1,
                dtype={
                    "names": ("variable", "pseudo-R2", "var_expl"),
                    "formats": ("U50", float, float),
                },
            )
            reduced_model.variable_expl_variance["variable"][0] = "all"
            reduced_model.variable_expl_variance["pseudo-R2"][
                0
            ] = reduced_model.pseudo_r2
            ...
            for var in reduced_model.var_list:
                if len(reduced_model.var_list) == 1:
                    break
                var_use = np.array(reduced_model.var_list)[
                    np.array(reduced_model.var_list) != var
                ]
                ...
                null_dev_t = self.family.resid_dev(
                    self.y[test_bool], [null_mu] * np.sum(test_bool)
                )
                null_deviance = np.sum(null_dev_t**2)
...
if __name__ == "__main__":
    ...
    nobs = 2 * 10**5
    x1, x2, x3 = (
        np.random.uniform(0.05, 1, size=nobs),
        np.random.uniform(0, 1, size=nobs),
        np.random.uniform(0, 1, size=nobs),
    )
    xs = [x1, x2, x3]
    ...
    gam_model = general_additive_model(
        sm_handler, var_list, y, poissFam, fisher_scoring=False
    )
