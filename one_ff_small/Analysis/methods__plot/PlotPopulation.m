    ...
    switch plot_type
        case 'GAM'
            ...
            for i=1:nunits
                ...
                % Extract coupling weights (handle both struct and numeric formats)
                coupling = coupledmodel.marginaltunings{end};
                if isstruct(coupling)
                    Jij(i,[1:i-1 i+1:end]) = coupling.mean;
                else
                    Jij(i,[1:i-1 i+1:end]) = coupling;
                end
                if ~isnan(bestmodel)
                    ...
                    % Filter to only valid subplot indices (1-8 for 2x4 grid)
                    varindx = varindx(varindx <= 8);
                    %% tuning of this unit
                    figure(i); hold on; %set(gcf,'Position',[85 -276 700 1000]);
                    ymin = inf; ymax = -inf;
                    for j=varindx
                        subplot(2,4,j); hold on;
                        % Extract tuning curve (handle both struct and numeric formats)
                        tuning = uncoupledmodel.marginaltunings{bestmodel}{j};
                        if isstruct(tuning), tuning = tuning.mean; end
                        plot(uncoupledmodel.x{j},tuning,'Color',cmap(j,:));
                        ymin = min(ymin,min(tuning));
                        ymax = max(ymax,max(tuning));
                    end
                    ...
                    for k=1:nvars
                        ...
                        if any(varindx==k)
                            ...
                            % Extract tuning curve (handle both struct and numeric formats)
                            tuning = uncoupledmodel.marginaltunings{bestmodel}{k};
                            if isstruct(tuning), tuning = tuning.mean; end
                            plot(uncoupledmodel.x{k},tuning,'Color',cmap(k,:));
                            if k==6 || k==8
