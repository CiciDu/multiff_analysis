function PlotUnits(prs,behv,units,plot_type,trial_type)
    ...
    %         plot(t,zr{end}.mean - zr{1}.mean,'k','linewidth',2); hline(0,'k');
            ...
            h = findobj(gca);
            idx = 4*(nconds:-1:1);
            if nconds == 1 || any(idx > numel(h))
                idx = round(linspace(numel(h), 1, nconds));
            end
            legend(h(idx), condnames, 'Location', 'best');
        ...
        case 'gam'
            ...
            for i=1:nunits
                ...
                if ~isnan(bestmodel)
                    ...
                    for k=1:nvars
                        figure(50+k); hold on;
                        if i==nunits % wait until the last unit to create sgtitle
                            s = sgtitle(strrep(['Tuning to ' prs.varlookup(num2str(models.xname{k}))],'_','\_'));
