    ...
    for i=1% if i=1, fit model using data from all trials rather than separately to data from each condition
        ...
        for j=1:nconds
            if copystats % if only one condition present, no need to recompute stats --- simply copy them from 'all' trials
                ...
                fprintf('GAM_prs.varchoose: %s\n', mat2str(GAM_prs.varchoose));
    ...
    for i=1% if i=1, fit model using data from all trials rather than separately to data from each condition
        ...
        for j=1:nconds
            if copystats % if only one condition present, no need to recompute stats --- simply copy them from 'all' trials
                ...
                [messenger,model] = system(['python ' 'C:\Users\jklakshm\Documents\GitHub\one_ff\Analyse\FitNNmodel.py']); % switch to command line to run Python code
