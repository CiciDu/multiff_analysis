    ...
    for i=1% if i=1, fit model using data from all trials rather than separately to data from each condition
        ...
        for j=1:nconds
            if copystats % if only one condition present, no need to recompute stats --- simply copy them from 'all' trials
                ...
                %% Check for NaN values in predictor variables
                fprintf('**********Checking for NaN values in predictors**********\n');
                % Summary statistics per variable
                for k=1:length(varname)
                    num_nans = sum(isnan(xt(:,k)));
                    if num_nans > 0
                        fprintf('Variable "%s": %d NaN values (%.2f%%)\n', ...
                            varname{k}, num_nans, 100*num_nans/size(xt,1));
                    end
                end
                % Total NaN count
                total_nans = sum(isnan(xt(:)));
                fprintf('Total NaN values: %d out of %d (%.2f%%)\n', ...
                    total_nans, numel(xt), 100*total_nans/numel(xt));
                % Check if any entire column is NaN
                all_nan_vars = find(all(isnan(xt)));
                if ~isempty(all_nan_vars)
                    warning('These variables are entirely NaN: %s', strjoin(varname(all_nan_vars), ', '));
                end
                ...
                if any(strcmp(varname,'phase'))
                    ...
                    % If no 'phase' variable, use xt directly for all units
                    for k=1:nunits
                        xt2{k} = xt;
                    end
                end
                ...
                % fit in parallel
                disp(['Number of units to fit GAM model: ' num2str(nunits)]);

                % can temporarily change parfor k=1:nunits to for k=1:nunits to debug
                % for k=1:nunits
                parfor k=1:nunits
                    fprintf(['k = ' num2str(k) '\n']);
                    % replace xt(:,'phase') with the unit-specific LFP channel
%                     xt(:,strcmp(varname,'phase')) = ConcatenateTrials(var_phase{k},[],{trials_spks_temp.tspk},{continuous_temp.ts},timewindow_full);
                    ...
                    % ====== ADD THIS SECTION HERE ======
                    % Check for NaN in xt_k before fitting
                    fprintf('  Unit %d: Checking xt_k for NaN...\n', k);
                    has_nan = false;
                    for v=1:length(xt_k)
                        num_nans = sum(isnan(xt_k{v}(:)));
                        if num_nans > 0
                            fprintf('    Variable %d (%s): %d NaN values (%.2f%%)\n', ...
                                v, varname{v}, num_nans, 100*num_nans/numel(xt_k{v}));
                            has_nan = true;
                        end
                    end
                    if ~has_nan
                        fprintf('    No NaN values detected in xt_k.\n');
                    end
                    % ====== END OF ADDED SECTION ======
