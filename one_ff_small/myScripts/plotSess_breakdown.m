...
%     units = [units sessions(i).populations.(unit_type).stats.trialtype.all.models.log.units(unitindx{i})];
    % Use the last (most recent) population if multiple exist
    pop_idx = length(sessions(i).populations);
...
for hl = 0:0.2:1
    line([0 10], [hl hl], 'Color', [0.8 0.8 0.8], 'LineStyle', '-');
end
...
%             if isstruct(rate), rate = rate.mean; end
...
%     if nunits>0, s = sgtitle(strrep(['Tuning to ' prs.varlookup(num2str(units(1).Uncoupledmodel.xname{i}))],'_','\_')); 
...
% % Handle different return types from shadedErrorBar
% if isstruct(h1) && isfield(h1,'mainLine')
%     legend([h1.mainLine h2.mainLine], {'Uncoupled model', 'Coupled model'}, 'Location', 'SE','Fontsize', 10);
% else
%     legend([h1 h2], {'Uncoupled model', 'Coupled model'}, 'Location', 'SE','Fontsize', 10);
% end
