...
%% ====== PRINT DIMENSIONS OF INPUT xt ======
fprintf('**********Input dimensions (raw xt)**********\n');
for i=1:nvars
    [nrows, ncols] = size(xt{i});
    fprintf('  xt{%d}: size=[%d x %d]\n', i, nrows, ncols);
end
fprintf('  yt: size=[%d x %d]\n', size(yt,1), size(yt,2));
fprintf('******************************************************\n');
...
%% Check for NaN in raw input variables
fprintf('**********Checking for NaN in raw inputs (BuildGAM)**********\n');
has_nan = false;
for i=1:nvars
    num_nans = sum(isnan(xt{i}(:)));
    if num_nans > 0
        fprintf('  Variable %d: %d NaN values (%.2f%%)\n', ...
            i, num_nans, 100*num_nans/numel(xt{i}));
        has_nan = true;
    end
end
num_nans_y = sum(isnan(yt));
if num_nans_y > 0
    fprintf('  Response (yt): %d NaN values (%.2f%%)\n', ...
        num_nans_y, 100*num_nans_y/numel(yt));
    has_nan = true;
end
if ~has_nan
    fprintf('  No NaN values in raw inputs.\n');
end
%% ====== END OF ADDED SECTION ======
...
for i=1:nvars
    ...
    % ---- NA diagnostic (for breakpoint / logging) ----
    if any(isnan(xt{i}(:)))
        pct_na = 100 * mean(isnan(xt{i}(:)));
        fprintf('Var %d: %.2f%% NA\n', i, pct_na);
...
%% ====== ADD THIS SECTION HERE ======
%% Check for NaN in encoded design matrix
fprintf('**********Checking for NaN after encoding (BuildGAM)**********\n');
has_nan_encoded = false;
for i=1:nvars
    num_nans = sum(isnan(x{i}(:)));
    if num_nans > 0
        fprintf('  Encoded variable %d: %d NaN values (%.2f%%)\n', ...
            i, num_nans, 100*num_nans/numel(x{i}));
        has_nan_encoded = true;
    end
end
if ~has_nan_encoded
    fprintf('  No NaN values in encoded design matrix.\n');
end
% Warn if NaN was in raw input but not in encoded (means it got binned somewhere)
if has_nan && ~has_nan_encoded
    warning('BuildGAM: NaN values were present in raw inputs but disappeared after encoding - they may have been incorrectly binned!');
end
%% ====== END OF ADDED SECTION ======


%% ====== PRINT MIN/MAX OF DESIGN MATRIX ======
fprintf('**********Design matrix statistics (before fitting)**********\n');
for i=1:nvars
    x_min = min(x{i}(:));
    x_max = max(x{i}(:));
    x_mean = mean(x{i}(:));
    x_std = std(x{i}(:));
    [nrows, ncols] = size(x{i});
    fprintf('  Variable %d (%s): size=[%d x %d], min=%.4f, max=%.4f, mean=%.4f, std=%.4f\n', ...
        i, xname{i}, nrows, ncols, x_min, x_max, x_mean, x_std);
end
fprintf('  Response (yt): size=[%d x %d], min=%.4f, max=%.4f, mean=%.4f, std=%.4f\n', ...
    size(yt,1), size(yt,2), min(yt), max(yt), mean(yt), std(yt));
fprintf('**************************************************************\n');

% %% ====== SAVE DESIGN MATRIX ======
% % Save design matrix with variable names
% design_matrix_data.xt = xt;            % Raw input variables (before RecodeInput)
% design_matrix_data.x = x;              % Cell array of encoded design matrices
% design_matrix_data.xname = xname;      % Variable names
% design_matrix_data.yt = yt;            % Response variable
% design_matrix_data.basis = basis;      % Basis functions / bin centres
% design_matrix_data.xtype = xtype;      % Variable types
% design_matrix_data.nprs = nprs;        % Number of parameters per variable

% out_dir = '/Users/dusiyi/Documents/Multifirefly-Project/all_monkey_data/one_ff_data';
% if ~exist(out_dir, 'dir')
%     mkdir(out_dir);
% end
% save(fullfile(out_dir, 'design_matrix.mat'), 'design_matrix_data', '-v6')
% fprintf('Design matrix saved to: design_matrix.mat\n');
