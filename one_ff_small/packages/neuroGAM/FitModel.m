...
%     fprintf('\t\t- Cross validation fold %d of %d\n', k, nfolds);
    ...
    % Check for NaN in predictor variables before fitting
    if k == 1  % Only report once per model to avoid spam
        num_nan_rows_train = sum(any(isnan(train_X), 2));
        num_nan_total_train = sum(isnan(train_X(:)));
        num_nan_rows_test = sum(any(isnan(test_X), 2));
        num_nan_total_test = sum(isnan(test_X(:)));
        
        if num_nan_total_train > 0 || num_nan_total_test > 0
            if num_nan_total_train > 0
                warning('FitModel: Training data contains %d NaN values in %d rows (%.2f%% of rows)', ...
                    num_nan_total_train, num_nan_rows_train, 100*num_nan_rows_train/size(train_X,1));
                fprintf('  NaN count per predictor: ');
                for p = 1:size(train_X,2)
                    fprintf('%d ', sum(isnan(train_X(:,p))));
                end
                fprintf('\n');
            end
            if num_nan_total_test > 0
                warning('FitModel: Test data contains %d NaN values in %d rows (%.2f%% of rows)', ...
                    num_nan_total_test, num_nan_rows_test, 100*num_nan_rows_test/size(test_X,1));
            end
        else
            fprintf('  FitModel: No NaN values detected in predictor variables. Proceeding with model fitting.\n');
        end
    end
    ...
    smooth_fr_hat_train = convn(fr_hat_train,h,'same'); %returns vector same size as original
